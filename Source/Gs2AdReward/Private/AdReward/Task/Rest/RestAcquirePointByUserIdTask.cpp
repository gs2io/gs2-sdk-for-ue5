...
#include "AdReward/Task/Rest/AcquirePointByUserIdTask.h"
...
{
    FAcquirePointByUserIdTask::FAcquirePointByUserIdTask(
    ...
    FAcquirePointByUserIdTask::FAcquirePointByUserIdTask(
        const FAcquirePointByUserIdTask& From
    ): TGs2Future(From), Session(From.Session), Request(From.Request)
    {
    }

    Core::Model::FGs2ErrorPtr FAcquirePointByUserIdTask::Action(
        const TSharedPtr<Result::FAcquirePointByUserIdResultPtr> Result
    )
    {
        ...
        FString ResponseBody;
        {
            ...
            auto Url = Core::FGs2Constant::EndpointHost
                .Replace(TEXT("{service}"), TEXT("ad-reward"))
                .Replace(TEXT("{region}"), *this->Session->RegionName())
                .Append("/{namespaceName}/user/{userId}/point/acquire");
            ...
            if (this->Request->GetPoint().IsSet())
            {
                JsonRootObject->SetStringField(TEXT("point"), FString::Printf(TEXT("%lld"), this->Request->GetPoint().GetValue()));
            }
        ...
        if (ResponseCode / 100 == 2)
        {
                ...
                FJsonSerializer::Deserialize(JsonReader, JsonRootObject))
            {
                ...
                *Result = Result::FAcquirePointByUserIdResult::FromJson(JsonRootObject);
                return nullptr;
            }
            const auto Details = MakeShared<TArray<TSharedPtr<Core::Model::FGs2ErrorDetail>>>();
