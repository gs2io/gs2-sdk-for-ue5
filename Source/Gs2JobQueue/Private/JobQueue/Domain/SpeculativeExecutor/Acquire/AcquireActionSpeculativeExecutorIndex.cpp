...
namespace Gs2::JobQueue::Domain::SpeculativeExecutor
{
    ...
    )
    {
        ...
        if (FPushByUserIdSpeculativeExecutor::Action() == NewAcquireAction->GetAction()) {
            ...
            auto Request = Request::FPushByUserIdRequest::FromJson(RequestModelJson);
            Request = FPushByUserIdSpeculativeExecutor::Rate(Request, Rate);
            auto Future = FPushByUserIdSpeculativeExecutor::Execute(
                Domain,
                Service,
                AccessToken,
                Request
