...
#include "JobQueue/Task/Rest/PushByUserIdTask.h"
...
{
    FPushByUserIdTask::FPushByUserIdTask(
    ...
    FPushByUserIdTask::FPushByUserIdTask(
        const FPushByUserIdTask& From
    ): TGs2Future(From), Session(From.Session), Request(From.Request)
    {
    }

    Core::Model::FGs2ErrorPtr FPushByUserIdTask::Action(
        const TSharedPtr<Result::FPushByUserIdResultPtr> Result
    )
    {
        ...
        FString ResponseBody;
        {
            ...
            auto Url = Core::FGs2Constant::EndpointHost
                ...
                .Append("/{namespaceName}/user/{userId}/job");
            ...
            if (this->Request->GetJobs() != nullptr && this->Request->GetJobs().IsValid())
            {
                TArray<TSharedPtr<FJsonValue>> v;
                for (auto JsonObjectValue : *this->Request->GetJobs())
                {
                    v.Add(MakeShared<FJsonValueObject>(JsonObjectValue->ToJson()));
                }
                JsonRootObject->SetArrayField(TEXT("jobs"), v);
            }
        ...
        if (ResponseCode / 100 == 2)
        {
                ...
                FJsonSerializer::Deserialize(JsonReader, JsonRootObject))
            {
                ...
                *Result = Result::FPushByUserIdResult::FromJson(JsonRootObject);
                return nullptr;
            }
            const auto Details = MakeShared<TArray<TSharedPtr<Core::Model::FGs2ErrorDetail>>>();
