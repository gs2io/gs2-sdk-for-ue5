...
#include "JobQueue/Task/Rest/GetJobResultByUserIdTask.h"
...
{
    FGetJobResultByUserIdTask::FGetJobResultByUserIdTask(
    ...
    FGetJobResultByUserIdTask::FGetJobResultByUserIdTask(
        const FGetJobResultByUserIdTask& From
    ): TGs2Future(From), Session(From.Session), Request(From.Request)
    {
    }

    Core::Model::FGs2ErrorPtr FGetJobResultByUserIdTask::Action(
        const TSharedPtr<Result::FGetJobResultByUserIdResultPtr> Result
    )
    {
        ...
        FString ResponseBody;
        {
            ...
            auto Url = Core::FGs2Constant::EndpointHost
                .Replace(TEXT("{service}"), TEXT("job-queue"))
                .Replace(TEXT("{region}"), *this->Session->RegionName())
                .Append("/{namespaceName}/user/{userId}/job/{jobName}/result");
            ...
            Url = Url.Replace(
                TEXT("{jobName}"),
                !this->Request->GetJobName().IsSet() || this->Request->GetJobName().GetValue().Len() == 0 ?
                    TEXT("null") : ToCStr(*this->Request->GetJobName())
            );
            ...
            if (this->Request->GetTryNumber().IsSet()) {
                queryStrings.Add("tryNumber=" + this->Request->GetTryNumberString());
            }
        ...
        if (ResponseCode / 100 == 2)
        {
                ...
                FJsonSerializer::Deserialize(JsonReader, JsonRootObject))
            {
                ...
                *Result = Result::FGetJobResultByUserIdResult::FromJson(JsonRootObject);
                return nullptr;
            }
            const auto Details = MakeShared<TArray<TSharedPtr<Core::Model::FGs2ErrorDetail>>>();
