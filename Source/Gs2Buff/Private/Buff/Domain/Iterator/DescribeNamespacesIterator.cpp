...
namespace Gs2::Buff::Domain::Iterator
{
    ...
    FDescribeNamespacesIterator::FIterator& FDescribeNamespacesIterator::FIterator::operator++()
    {
        ...
        {
            const auto ListParentKey = Gs2::Buff::Model::Cache::FNamespaceCache::CreateCacheParentKey(
                TOptional<int32>()
            );
            if (!RangeIteratorOpt)
            {
            ...
            Range = R->GetItems().IsValid() ? R->GetItems() : MakeShared<TArray<Gs2::Buff::Model::FNamespacePtr>>();
