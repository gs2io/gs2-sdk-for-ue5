...
namespace Gs2::Buff::Domain::Iterator
{
    ...
    Gs2::Core::Model::FGs2ErrorPtr FDescribeNamespacesIterator::FIteratorNextTask::Action(TSharedPtr<TSharedPtr<Gs2::Buff::Model::FNamespace>> Result)
    {
        ++Iterator;
    ...
    FDescribeNamespacesIterator::FIterator& FDescribeNamespacesIterator::FIterator::operator++()
    {
        ...
        {
            const auto ListParentKey = "buff:Namespace";

            if (!RangeIteratorOpt)
            {
                Range = Self->Gs2->Cache->TryGetList<Gs2::Buff::Model::FNamespace>(ListParentKey);

                if (Range)
                {
                    Range->RemoveAll([this](const Gs2::Buff::Model::FNamespacePtr& Item) { return Self->NamePrefix && Item->GetNamePrefix() != Self->NamePrefix; });
                    bLast = true;
            ...
            const auto Future = Self->Client->DescribeNamespaces(
                MakeShared<Gs2::Buff::Request::FDescribeNamespacesRequest>()
            ...
            for (auto Item : *R->GetItems())
            {
                Self->Gs2->Cache->Put(
                    ...
                    ListParentKey,
                    Gs2::Buff::Domain::Model::FNamespaceDomain::CreateCacheKey(
                        Item->GetName()
                    ),
                    Item,
