...
#include "Buff/Task/Rest/UpdateBuffEntryModelMasterTask.h"
...
{
    FUpdateBuffEntryModelMasterTask::FUpdateBuffEntryModelMasterTask(
    ...
    Core::Model::FGs2ErrorPtr FUpdateBuffEntryModelMasterTask::Action(
        const TSharedPtr<Result::FUpdateBuffEntryModelMasterResultPtr> Result
    )
    {
        ...
        FString ResponseBody;
        {
            ...
            auto Url = Core::FGs2Constant::EndpointHost
                .Replace(TEXT("{service}"), TEXT("buff"))
                .Replace(TEXT("{region}"), *this->Session->RegionName())
                .Append("/{namespaceName}/master/model/{buffEntryName}");
            ...
            Url = Url.Replace(
                TEXT("{buffEntryName}"),
                !this->Request->GetBuffEntryName().IsSet() || this->Request->GetBuffEntryName().GetValue().Len() == 0 ?
                    TEXT("null") : ToCStr(*this->Request->GetBuffEntryName())
            );
        ...
        if (ResponseCode / 100 == 2)
        {
                ...
                FJsonSerializer::Deserialize(JsonReader, JsonRootObject))
            {
                ...
                *Result = Result::FUpdateBuffEntryModelMasterResult::FromJson(JsonRootObject);
                return nullptr;
            }
            const auto Details = MakeShared<TArray<TSharedPtr<Core::Model::FGs2ErrorDetail>>>();
