...
namespace Gs2::Ranking2::Domain::Model
{
    ...
    {
        const FString CacheKey = Gs2::Ranking2::Domain::Model::FGlobalRankingModelDomain::CreateCacheKey(
            Self->RankingName
        );
            ...
            [this, Result, CacheKey]() -> Gs2::Core::Model::FGs2ErrorPtr
            {
                ...
                if (!bCacheHit) {
                    ...
                    if (Future->GetTask().IsError())
                    {
                        ...
                        if (!Error->GetErrors().IsValid() || Error->Count() == 0 || !Error->Detail(0).IsValid() || Error->Detail(0)->GetComponent() != "globalRankingModel")
                        {
                            return Error;
                        }
                    }
    ...
    )
    {
        ...
        return Gs2->Cache->Subscribe(
            ...
            [WeakGs2, WeakService, RegisteredParentKey, QueryNamespaceName, QueryRankingName]()
            {
                ...
                const auto Domain = MakeShared<FGlobalRankingModelDomain>(
                    Owner,
                    WeakService.Pin(),
                    QueryNamespaceName,
                    QueryRankingName
