...
namespace Gs2::Ranking2::Domain::Model
{
    ...
    )
    {
        Request
            ...
            ->WithAccessToken(Self->AccessToken->GetToken())
            ->WithTargetUserId(Self->TargetUserId);
        const auto Future = Self->Client->DeleteSubscribe(
            Request
        );
        Future->StartSynchronousTask();
        if (Future->GetTask().IsError())
    ...
    )
    {
        ...
        return Gs2->Cache->Subscribe(
            ...
            [WeakGs2, WeakService, RegisteredParentKey, QueryNamespaceName, QueryRankingName, QueryTargetUserId, SourceToken, RegisteredUserId, RegisteredTimeOffset]()
            {
