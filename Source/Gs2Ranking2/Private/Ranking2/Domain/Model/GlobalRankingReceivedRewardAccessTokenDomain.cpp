...
namespace Gs2::Ranking2::Domain::Model
{
    ...
    )
    {
        ...
        if (SpeculativeExecute) {
            const auto SpeculativeExecuteFuture = Transaction::SpeculativeExecutor::FReceiveGlobalRankingReceivedRewardByUserIdSpeculativeExecutor::Execute(
                Self->Gs2,
                Self->Service,
                Self->AccessToken,
                Request::FReceiveGlobalRankingReceivedRewardByUserIdRequest::FromJson(Request->ToJson())
            );
        ...
        const auto Future = Self->Client->ReceiveGlobalRankingReceivedReward(
            Request
        );
        Future->StartSynchronousTask();
        if (Future->GetTask().IsError())
