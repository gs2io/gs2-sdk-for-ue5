...
namespace Gs2::Ranking2::Domain::Iterator
{
    ...
    FDescribeSubscribeRankingModelsIterator::FIterator& FDescribeSubscribeRankingModelsIterator::FIterator::operator++()
    {
        ...
        {
            const auto ListParentKey = Gs2::Ranking2::Model::Cache::FSubscribeRankingModelCache::CreateCacheParentKey(
                Self->NamespaceName,
                TOptional<int32>()
            );
            if (!RangeIteratorOpt)
            ...
            ;
            const auto Future = Self->Client->DescribeSubscribeRankingModels(Request);
            ...
            Range = R->GetItems().IsValid() ? R->GetItems() : MakeShared<TArray<Gs2::Ranking2::Model::FSubscribeRankingModelPtr>>();
            ...
            if (Range.IsValid())
            {
                for (const auto& Item : *Range)
                {
                    ...
                    Gs2::Ranking2::Model::Cache::FSubscribeRankingModelCache::Put(
                        Self->Gs2->Cache,
