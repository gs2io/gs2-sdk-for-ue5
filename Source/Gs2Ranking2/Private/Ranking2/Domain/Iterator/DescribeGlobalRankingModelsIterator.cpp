...
namespace Gs2::Ranking2::Domain::Iterator
{
    ...
    FDescribeGlobalRankingModelsIterator::FIterator& FDescribeGlobalRankingModelsIterator::FIterator::operator++()
    {
        ...
        {
            const auto ListParentKey = Gs2::Ranking2::Model::Cache::FGlobalRankingModelCache::CreateCacheParentKey(
                Self->NamespaceName,
                TOptional<int32>()
            );
            if (!RangeIteratorOpt)
            ...
            ;
            const auto Future = Self->Client->DescribeGlobalRankingModels(Request);
            ...
            Range = R->GetItems().IsValid() ? R->GetItems() : MakeShared<TArray<Gs2::Ranking2::Model::FGlobalRankingModelPtr>>();
