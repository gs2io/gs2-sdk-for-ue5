...
namespace Gs2::Chat::Domain::Iterator
{
    ...
    FDescribeCategoryModelsIterator::FIterator& FDescribeCategoryModelsIterator::FIterator::operator++()
    {
        ...
        {
            const auto ListParentKey = Gs2::Chat::Model::Cache::FCategoryModelCache::CreateCacheParentKey(
                Self->NamespaceName,
                TOptional<int32>()
            );
            if (!RangeIteratorOpt)
            ...
            Range = R->GetItems().IsValid() ? R->GetItems() : MakeShared<TArray<Gs2::Chat::Model::FCategoryModelPtr>>();
