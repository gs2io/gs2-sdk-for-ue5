...
namespace Gs2::Chat::Domain::Iterator
{
    ...
    FDescribeRoomsIterator::FIterator& FDescribeRoomsIterator::FIterator::operator++()
    {
        ...
        {
            const auto ListParentKey = Gs2::Chat::Model::Cache::FRoomCache::CreateCacheParentKey(
                Self->NamespaceName,
                TOptional<FString>(),
                TOptional<int32>()
            );
            if (!RangeIteratorOpt)
            {
                ...
                if (Range)
                {
                    Range->RemoveAll([this](const Gs2::Chat::Model::FRoomPtr& Item) { return Self->NamePrefix && !Item->GetName()->StartsWith(*Self->NamePrefix); });
                    bLast = true;
            ...
            const auto Request =
                MakeShared<Gs2::Chat::Request::FDescribeRoomsRequest>()
                    ...
                    ->WithLimit(FetchSize)
            ;
            const auto Future = Self->Client->DescribeRooms(Request);
            ...
            Range = R->GetItems().IsValid() ? R->GetItems() : MakeShared<TArray<Gs2::Chat::Model::FRoomPtr>>();
            ...
            if (Range)
            {
                Range->RemoveAll([this](const Gs2::Chat::Model::FRoomPtr& Item) { return Self->NamePrefix.IsSet() && !Item->GetName()->StartsWith(*Self->NamePrefix); });
            }
            RangeIteratorOpt = Range->CreateIterator();
