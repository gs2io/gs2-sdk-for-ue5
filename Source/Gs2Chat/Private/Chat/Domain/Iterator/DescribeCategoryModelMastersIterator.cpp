...
namespace Gs2::Chat::Domain::Iterator
{
    ...
    Gs2::Core::Model::FGs2ErrorPtr FDescribeCategoryModelMastersIterator::FIteratorNextTask::Action(TSharedPtr<TSharedPtr<Gs2::Chat::Model::FCategoryModelMaster>> Result)
    {
        ++Iterator;
    ...
    FDescribeCategoryModelMastersIterator::FIterator& FDescribeCategoryModelMastersIterator::FIterator::operator++()
    {
        ...
        {
            const auto ListParentKey = Gs2::Chat::Model::Cache::FCategoryModelMasterCache::CreateCacheParentKey(
                Self->NamespaceName,
                TOptional<int32>()
            );
            if (!RangeIteratorOpt)
            {
                Range = Self->Gs2->Cache->TryGetList<Gs2::Chat::Model::FCategoryModelMaster>(ListParentKey);
            ...
            const auto Request =
                MakeShared<Gs2::Chat::Request::FDescribeCategoryModelMastersRequest>()
            ...
            Range = R->GetItems().IsValid() ? R->GetItems() : MakeShared<TArray<Gs2::Chat::Model::FCategoryModelMasterPtr>>();
            ...
            if (Range.IsValid())
            {
                for (const auto& Item : *Range)
                {
                    ...
                    Gs2::Chat::Model::Cache::FCategoryModelMasterCache::Put(
                        Self->Gs2->Cache,
                        Request->GetNamespaceName(), Item->GetCategory(),
