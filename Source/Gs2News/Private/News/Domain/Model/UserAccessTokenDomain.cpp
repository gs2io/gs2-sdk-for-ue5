...
namespace Gs2::News::Domain::Model
{
    ...
    class FUserAccessTokenDomain::FCollectNewsesTask : public Gs2::Core::Util::TGs2Future<TArray<Gs2::News::Model::FNewsPtr>>, public TSharedFromThis<FCollectNewsesTask>
    {
        const TSharedPtr<FUserAccessTokenDomain> Self;
        const TFunction<void(TArray<Gs2::News::Model::FNewsPtr>)> OnCollected;

    public:
        explicit FCollectNewsesTask(const TSharedPtr<FUserAccessTokenDomain>& Self, TFunction<void(TArray<Gs2::News::Model::FNewsPtr>)> OnCollected) : Self(Self), OnCollected(OnCollected) {}
        FCollectNewsesTask(const FCollectNewsesTask& From) : TGs2Future(From), Self(From.Self), OnCollected(From.OnCollected) {}
        virtual Gs2::Core::Model::FGs2ErrorPtr Action(TSharedPtr<TSharedPtr<TArray<Gs2::News::Model::FNewsPtr>>> Result) override
        {
            ...
            auto Iterator = Self->Newses()->begin();
    ...
    Gs2::Core::Domain::CallbackID FUserAccessTokenDomain::SubscribeNewses(
        TFunction<void(TArray<Gs2::News::Model::FNewsPtr>)> Callback
    )
    {
        ...
        return Gs2->Cache->ListSubscribeTyped(
            ...
            [WeakGs2, WeakService, Callback, QueryNamespaceName, SourceToken, RegisteredUserId, RegisteredTimeOffset]()
            {
                ...
                const auto Task = Gs2::Core::Util::New<FAsyncTask<FCollectNewsesTask>>(Domain, Callback);
                Task->StartBackgroundTask();
            }
        );
    }

    void FUserAccessTokenDomain::InvalidateNewses()
    {
    ...
    FUserAccessTokenDomain::FSubscribeNewsesWithInitialCallTask::FSubscribeNewsesWithInitialCallTask(const TSharedPtr<FUserAccessTokenDomain>& Self, TFunction<void(TArray<Gs2::News::Model::FNewsPtr>)> Callback) : Self(Self), Callback(Callback) {}
    FUserAccessTokenDomain::FSubscribeNewsesWithInitialCallTask::FSubscribeNewsesWithInitialCallTask(const FSubscribeNewsesWithInitialCallTask& From) : TGs2Future(From), Self(From.Self), Callback(From.Callback) {}
    Gs2::Core::Model::FGs2ErrorPtr FUserAccessTokenDomain::FSubscribeNewsesWithInitialCallTask::Action(TSharedPtr<TSharedPtr<Gs2::Core::Domain::CallbackID>> Result)
    {
        const auto Task = Gs2::Core::Util::New<FAsyncTask<FCollectNewsesTask>>(Self, TFunction<void(TArray<Gs2::News::Model::FNewsPtr>)>());
        Task->StartSynchronousTask(); Task->EnsureCompletion();
        if (Task->GetTask().IsError()) return Task->GetTask().Error();
        const auto Values = Task->GetTask().Result();
        const auto CallbackId = Self->SubscribeNewses(Callback);
        Callback(*Values); *Result = MakeShared<Gs2::Core::Domain::CallbackID>(CallbackId);
        return nullptr;
    }
    TSharedPtr<FAsyncTask<FUserAccessTokenDomain::FSubscribeNewsesWithInitialCallTask>> FUserAccessTokenDomain::SubscribeNewsesWithInitialCall(TFunction<void(TArray<Gs2::News::Model::FNewsPtr>)> Callback)
    {
        return Gs2::Core::Util::New<FAsyncTask<FSubscribeNewsesWithInitialCallTask>>(this->AsShared(), Callback);
    }
