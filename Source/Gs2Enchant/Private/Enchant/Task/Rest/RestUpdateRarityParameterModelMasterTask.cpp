...
#include "Enchant/Task/Rest/UpdateRarityParameterModelMasterTask.h"
...
{
    FUpdateRarityParameterModelMasterTask::FUpdateRarityParameterModelMasterTask(
    ...
    Core::Model::FGs2ErrorPtr FUpdateRarityParameterModelMasterTask::Action(
        const TSharedPtr<Result::FUpdateRarityParameterModelMasterResultPtr> Result
    )
    {
        ...
        FString ResponseBody;
        {
            ...
            auto Url = Core::FGs2Constant::EndpointHost
                ...
                .Append("/{namespaceName}/master/model/rarity/{parameterName}");
            ...
            if (this->Request->GetMaximumParameterCount().IsSet())
            {
                JsonRootObject->SetNumberField(TEXT("maximumParameterCount"), this->Request->GetMaximumParameterCount().GetValue());
            }
            if (this->Request->GetParameterCounts() != nullptr && this->Request->GetParameterCounts().IsValid())
            {
                TArray<TSharedPtr<FJsonValue>> v;
                for (auto JsonObjectValue : *this->Request->GetParameterCounts())
                {
        ...
        if (ResponseCode / 100 == 2)
        {
                ...
                FJsonSerializer::Deserialize(JsonReader, JsonRootObject))
            {
                ...
                *Result = Result::FUpdateRarityParameterModelMasterResult::FromJson(JsonRootObject);
                return nullptr;
            }
            const auto Details = MakeShared<TArray<TSharedPtr<Core::Model::FGs2ErrorDetail>>>();
