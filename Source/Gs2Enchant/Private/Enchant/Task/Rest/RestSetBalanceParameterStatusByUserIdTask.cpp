...
#include "Enchant/Task/Rest/SetBalanceParameterStatusByUserIdTask.h"
...
{
    FSetBalanceParameterStatusByUserIdTask::FSetBalanceParameterStatusByUserIdTask(
    ...
    FSetBalanceParameterStatusByUserIdTask::FSetBalanceParameterStatusByUserIdTask(
        const FSetBalanceParameterStatusByUserIdTask& From
    ): TGs2Future(From), Session(From.Session), Request(From.Request)
    {
    }

    Core::Model::FGs2ErrorPtr FSetBalanceParameterStatusByUserIdTask::Action(
        const TSharedPtr<Result::FSetBalanceParameterStatusByUserIdResultPtr> Result
    )
    {
        ...
        FString ResponseBody;
        {
            ...
            auto Url = Core::FGs2Constant::EndpointHost
                ...
                .Append("/{namespaceName}/user/{userId}/status/balance/{parameterName}/{propertyId}");
            ...
            if (this->Request->GetParameterValues() != nullptr && this->Request->GetParameterValues().IsValid())
            {
                TArray<TSharedPtr<FJsonValue>> v;
                for (auto JsonObjectValue : *this->Request->GetParameterValues())
                {
        ...
        if (ResponseCode / 100 == 2)
        {
                ...
                FJsonSerializer::Deserialize(JsonReader, JsonRootObject))
            {
                ...
                *Result = Result::FSetBalanceParameterStatusByUserIdResult::FromJson(JsonRootObject);
                return nullptr;
            }
            const auto Details = MakeShared<TArray<TSharedPtr<Core::Model::FGs2ErrorDetail>>>();
