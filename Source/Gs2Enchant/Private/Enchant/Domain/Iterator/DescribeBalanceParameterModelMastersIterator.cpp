...
namespace Gs2::Enchant::Domain::Iterator
{
    ...
    FDescribeBalanceParameterModelMastersIterator::FIterator& FDescribeBalanceParameterModelMastersIterator::FIterator::operator++()
    {
        ...
        {
            const auto ListParentKey = Gs2::Enchant::Model::Cache::FBalanceParameterModelMasterCache::CreateCacheParentKey(
                Self->NamespaceName,
                TOptional<int32>()
            );
            if (!RangeIteratorOpt)
            ...
            const auto Request =
                MakeShared<Gs2::Enchant::Request::FDescribeBalanceParameterModelMastersRequest>()
                    ...
                    ->WithLimit(FetchSize)
            ;
            const auto Future = Self->Client->DescribeBalanceParameterModelMasters(Request);
            ...
            Range = R->GetItems().IsValid() ? R->GetItems() : MakeShared<TArray<Gs2::Enchant::Model::FBalanceParameterModelMasterPtr>>();
            ...
            if (Range)
            {
                Range->RemoveAll([this](const Gs2::Enchant::Model::FBalanceParameterModelMasterPtr& Item) { return Self->NamePrefix.IsSet() && !Item->GetName()->StartsWith(*Self->NamePrefix); });
            }
            RangeIteratorOpt = Range->CreateIterator();
