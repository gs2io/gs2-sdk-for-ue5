...
namespace Gs2::Enchant::Domain::Iterator
{
    ...
    FDescribeRarityParameterModelsIterator::FIterator& FDescribeRarityParameterModelsIterator::FIterator::operator++()
    {
        ...
        {
            const auto ListParentKey = Gs2::Enchant::Model::Cache::FRarityParameterModelCache::CreateCacheParentKey(
                Self->NamespaceName,
                TOptional<int32>()
            );
            if (!RangeIteratorOpt)
            ...
            ;
            const auto Future = Self->Client->DescribeRarityParameterModels(Request);
            ...
            Range = R->GetItems().IsValid() ? R->GetItems() : MakeShared<TArray<Gs2::Enchant::Model::FRarityParameterModelPtr>>();
