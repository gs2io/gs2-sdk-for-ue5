...
#include "Datastore/Task/Rest/PrepareDownloadByGenerationAndUserIdTask.h"
...
{
    FPrepareDownloadByGenerationAndUserIdTask::FPrepareDownloadByGenerationAndUserIdTask(
    ...
    FPrepareDownloadByGenerationAndUserIdTask::FPrepareDownloadByGenerationAndUserIdTask(
        const FPrepareDownloadByGenerationAndUserIdTask& From
    ): TGs2Future(From), Session(From.Session), Request(From.Request)
    {
    }

    Core::Model::FGs2ErrorPtr FPrepareDownloadByGenerationAndUserIdTask::Action(
        const TSharedPtr<Result::FPrepareDownloadByGenerationAndUserIdResultPtr> Result
    )
    {
        ...
        FString ResponseBody;
        {
            ...
            auto Url = Core::FGs2Constant::EndpointHost
                ...
                .Append("/{namespaceName}/user/{userId}/file/generation/{generation}");
            ...
            Url = Url.Replace(
                TEXT("{generation}"),
                !this->Request->GetGeneration().IsSet() || this->Request->GetGeneration().GetValue().Len() == 0 ?
                    TEXT("null") : ToCStr(*this->Request->GetGeneration())
            );
            ...
            if (this->Request->GetDataObjectId().IsSet() && !this->Request->GetDataObjectId().GetValue().IsEmpty())
            {
        ...
        if (ResponseCode / 100 == 2)
        {
                ...
                FJsonSerializer::Deserialize(JsonReader, JsonRootObject))
            {
                ...
                *Result = Result::FPrepareDownloadByGenerationAndUserIdResult::FromJson(JsonRootObject);
                return nullptr;
            }
            const auto Details = MakeShared<TArray<TSharedPtr<Core::Model::FGs2ErrorDetail>>>();
