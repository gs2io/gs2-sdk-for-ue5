...
#include "Datastore/Task/WebSocket/RestoreDataObjectTask.h"
...
{
    FRestoreDataObjectTask::FRestoreDataObjectTask(
    ...
    Core::Model::FGs2ErrorPtr FRestoreDataObjectTask::Action(
        const TSharedPtr<Result::FRestoreDataObjectResultPtr> Result
    )
    {
        ...
        UE_LOG(Gs2Log, VeryVerbose, TEXT("[%s:%s:%s] %s"), TEXT("datastore"), TEXT("dataObject"), TEXT("restoreDataObject"), ToCStr(RequestPayload->Payload()));
        ...
        *Result = Result::FRestoreDataObjectResult::FromJson(WebSocketResult->Body());
