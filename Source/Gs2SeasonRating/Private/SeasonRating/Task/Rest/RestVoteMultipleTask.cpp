...
#include "SeasonRating/Task/Rest/VoteMultipleTask.h"
...
{
    FVoteMultipleTask::FVoteMultipleTask(
    ...
    Core::Model::FGs2ErrorPtr FVoteMultipleTask::Action(
        const TSharedPtr<Result::FVoteMultipleResultPtr> Result
    )
    {
        ...
        FString ResponseBody;
        {
            ...
            auto Url = Core::FGs2Constant::EndpointHost
                ...
                .Append("/{namespaceName}/action/vote/multiple");
            ...
            if (this->Request->GetSignedBallots() != nullptr && this->Request->GetSignedBallots().IsValid())
            {
                TArray<TSharedPtr<FJsonValue>> v;
                for (auto JsonObjectValue : *this->Request->GetSignedBallots())
                {
            ...
            {
                TArray<TSharedPtr<FJsonValue>> v;
                for (auto JsonObjectValue : *this->Request->GetGameResults())
                {
                    v.Add(MakeShared<FJsonValueObject>(JsonObjectValue->ToJson()));
                }
        ...
        if (ResponseCode / 100 == 2)
        {
                ...
                FJsonSerializer::Deserialize(JsonReader, JsonRootObject))
            {
                ...
                *Result = Result::FVoteMultipleResult::FromJson(JsonRootObject);
                return nullptr;
            }
            const auto Details = MakeShared<TArray<TSharedPtr<Core::Model::FGs2ErrorDetail>>>();
