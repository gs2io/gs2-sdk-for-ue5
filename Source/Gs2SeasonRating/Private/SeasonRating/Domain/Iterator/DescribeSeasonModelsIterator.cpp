...
namespace Gs2::SeasonRating::Domain::Iterator
{
    ...
    FDescribeSeasonModelsIterator::FIterator& FDescribeSeasonModelsIterator::FIterator::operator++()
    {
        ...
        {
            const auto ListParentKey = Gs2::SeasonRating::Model::Cache::FSeasonModelCache::CreateCacheParentKey(
                Self->NamespaceName,
                TOptional<int32>()
            );
            if (!RangeIteratorOpt)
            ...
            Range = R->GetItems().IsValid() ? R->GetItems() : MakeShared<TArray<Gs2::SeasonRating::Model::FSeasonModelPtr>>();
