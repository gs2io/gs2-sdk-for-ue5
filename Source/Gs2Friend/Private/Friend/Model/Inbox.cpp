...
namespace Gs2::Friend::Model
{
    ...
    TSharedPtr<FInbox> FInbox::WithUserId(
    ...
    TOptional<FString> FInbox::GetUserId() const
    {
        return UserIdValue;
    }
    ...
    TOptional<FString> FInbox::GetUserIdFromGrn(const FString Grn)
    {
        const auto Pattern = FRegexPattern(TEXT("grn:gs2:(?<region>.+):(?<ownerId>.+):friend:(?<namespaceName>.+):user:(?<userId>.+)"));
        FRegexMatcher Matcher(Pattern, Grn);
        while (Matcher.FindNext())
    ...
    TSharedPtr<FInbox> FInbox::FromJson(const TSharedPtr<FJsonObject> Data)
    {
        ...
        return MakeShared<FInbox>()
            ...
            ->WithFromUserIds(Data->HasField(ANSI_TO_TCHAR("fromUserIds")) ? [Data]() -> TSharedPtr<TArray<FString>>
                {
                    if (!Data->HasTypedField<EJson::Array>(ANSI_TO_TCHAR("fromUserIds")))
                    {
                        return nullptr;
                    }
    ...
    TSharedPtr<FJsonObject> FInbox::ToJson() const
    {
        ...
        {
            JsonRootObject->SetStringField(TEXT("inboxId"), InboxIdValue.GetValue());
        }
        ...
        if (FromUserIdsValue != nullptr && FromUserIdsValue.IsValid())
        {
            ...
            JsonRootObject->SetArrayField(TEXT("fromUserIds"), v);
        }
        if (CreatedAtValue.IsSet())
        {
...
{
    FString FInboxCache::CreateCacheParentKey(
    ...
    )
    {
        return FString("friend:")
            ...
            + FString::FromInt(CacheOwnerArgumentTimeOffset.Get(0)) + FString(":Inbox");
    }

    FString FInboxCache::CreateCacheKey(
    )
    {
        return FString("Singleton");
    }

    bool FInboxCache::TryGet(
        const Gs2::Core::Domain::FCacheDatabasePtr& CacheOwnerArgumentCache,
        TOptional<FString> CacheOwnerArgumentNamespaceName,
        TOptional<FString> CacheOwnerArgumentUserId,
        TOptional<int32> CacheOwnerArgumentTimeOffset,
        Gs2::Friend::Model::FInboxPtr* CacheOwnerArgumentOutItem
    )
    {
        ...
        Gs2::Friend::Model::FInboxPtr CacheOwnerValue;
        const bool CacheOwnerFound = CacheSnapshot->TryGet<Gs2::Friend::Model::FInbox>(
    ...
    void FInboxCache::Put(
        const Gs2::Core::Domain::FCacheDatabasePtr& CacheOwnerArgumentCache,
        TOptional<FString> CacheOwnerArgumentNamespaceName,
        TOptional<FString> CacheOwnerArgumentUserId,
        TOptional<int32> CacheOwnerArgumentTimeOffset,
        const Gs2::Friend::Model::FInboxPtr& CacheOwnerArgumentItem
    )
    {
        const auto CacheSnapshot = CacheOwnerArgumentCache;
        ...
        Gs2::Friend::Model::FInboxPtr CacheOwnerExisting;
        if (CacheSnapshot->TryGet<Gs2::Friend::Model::FInbox>(CacheOwnerParentKey, CacheOwnerKey, &CacheOwnerExisting))
        {
        ...
        CacheSnapshot->Put(Gs2::Friend::Model::FInbox::TypeName, CacheOwnerParentKey, CacheOwnerKey, CacheOwnerValue,
            FDateTime::Now() + FTimespan::FromMinutes(Gs2::Core::Domain::DefaultCacheMinutes)
        );
    }

    FString FInboxCache::PutUserData(
        const Gs2::Core::Domain::FCacheDatabasePtr& Cache,
        TOptional<FString> NamespaceName,
        TOptional<FString> UserId,
        TOptional<int32> TimeOffset,
        const Gs2::Friend::Model::FInboxPtr& Item
    )
    {
    ...
    )
    {
        ...
        if (!CacheSnapshot.IsValid()) return;
        if (!CacheOwnerArgumentUserId.IsSet()) return;
        CacheSnapshot->Delete(Gs2::Friend::Model::FInbox::TypeName, CreateCacheParentKey(
            CacheOwnerArgumentNamespaceName,
            CacheOwnerArgumentUserId,
            CacheOwnerArgumentTimeOffset
        ), CreateCacheKey(
    ...
    Gs2::Core::Model::FGs2ErrorPtr FInboxCache::Fetch(
        const Gs2::Core::Domain::FCacheDatabasePtr& CacheOwnerArgumentCache,
        TOptional<FString> CacheOwnerArgumentNamespaceName,
        TOptional<FString> CacheOwnerArgumentUserId,
        TOptional<int32> CacheOwnerArgumentTimeOffset,
        const TFunction<Gs2::Core::Model::FGs2ErrorPtr(Gs2::Friend::Model::FInboxPtr*)>& CacheOwnerArgumentFetchImpl,
        Gs2::Friend::Model::FInboxPtr* CacheOwnerArgumentOutItem
    )
    {
        const auto CacheSnapshot = CacheOwnerArgumentCache;
        ...
        Gs2::Friend::Model::FInboxPtr CacheOwnerFetchedItem;
        ...
        if (CacheOwnerDetails.IsValid() && CacheOwnerDetails->Num() > 0 && (*CacheOwnerDetails)[0].IsValid() && (*CacheOwnerDetails)[0]->GetComponent() == TEXT("inbox"))
        {
            return nullptr;
        }
        if (CacheOwnerArgumentOutItem) *CacheOwnerArgumentOutItem = nullptr;
    ...
    Gs2::Core::Domain::CallbackID FInboxCache::ListSubscribe(
        const Gs2::Core::Domain::FCacheDatabasePtr& CacheOwnerArgumentCache,
        TOptional<FString> CacheOwnerArgumentNamespaceName,
        TOptional<FString> CacheOwnerArgumentUserId,
        TOptional<int32> CacheOwnerArgumentTimeOffset,
        TFunction<void(TArray<Gs2::Friend::Model::FInboxPtr>)> CacheOwnerArgumentCallback
    )
    {
        const auto CacheSnapshot = CacheOwnerArgumentCache;
        if (!CacheSnapshot.IsValid()) return 0;
        return CacheSnapshot->ListSubscribeTyped(Gs2::Friend::Model::FInbox::TypeName, CreateCacheParentKey(
            CacheOwnerArgumentNamespaceName,
            CacheOwnerArgumentUserId,
            CacheOwnerArgumentTimeOffset
        ), [CacheOwnerArgumentCallback](const TArray<FGs2ObjectPtr>& CacheOwnerValues)
        {
            TArray<Gs2::Friend::Model::FInboxPtr> CacheOwnerTypedValues;
            for (const auto& CacheOwnerValue : CacheOwnerValues) if (CacheOwnerValue) CacheOwnerTypedValues.Add(StaticCastSharedPtr<Gs2::Friend::Model::FInbox>(CacheOwnerValue));
            if (CacheOwnerArgumentCallback) CacheOwnerArgumentCallback(CacheOwnerTypedValues);
        });
    }

    void FInboxCache::ListUnsubscribe(
        const Gs2::Core::Domain::FCacheDatabasePtr& CacheOwnerArgumentCache,
        TOptional<FString> CacheOwnerArgumentNamespaceName,
        TOptional<FString> CacheOwnerArgumentUserId,
        TOptional<int32> CacheOwnerArgumentTimeOffset,
    ...
    )
    {
        const auto CacheSnapshot = CacheOwnerArgumentCache;
        if (!CacheSnapshot.IsValid()) return;
        CacheSnapshot->ListUnsubscribe(Gs2::Friend::Model::FInbox::TypeName, CreateCacheParentKey(
            CacheOwnerArgumentNamespaceName,
            CacheOwnerArgumentUserId,
            CacheOwnerArgumentTimeOffset
