...
#include "Money/Task/Rest/CreateNamespaceTask.h"
...
namespace Gs2::Money::Task::Rest
{
    ...
    )
    {
        ...
        FString ResponseBody;
        {
            ...
            if (this->Request->GetShareFree().IsSet())
            {
                JsonRootObject->SetBoolField(TEXT("shareFree"), this->Request->GetShareFree().GetValue());
            }
            if (this->Request->GetCurrency().IsSet())
            {
                JsonRootObject->SetStringField(TEXT("currency"), this->Request->GetCurrency().GetValue());
            }
