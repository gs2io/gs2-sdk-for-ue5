...
#include "Money/Task/Rest/RecordReceiptTask.h"
...
{
    FRecordReceiptTask::FRecordReceiptTask(
    ...
    Core::Model::FGs2ErrorPtr FRecordReceiptTask::Action(
        const TSharedPtr<Result::FRecordReceiptResultPtr> Result
    )
    {
        ...
        FString ResponseBody;
        {
            ...
            auto Url = Core::FGs2Constant::EndpointHost
                .Replace(TEXT("{service}"), TEXT("money"))
                .Replace(TEXT("{region}"), *this->Session->RegionName())
                .Append("/{namespaceName}/user/{userId}/receipt");
            ...
            if (this->Request->GetContentsId().IsSet() && !this->Request->GetContentsId().GetValue().IsEmpty())
            {
                JsonRootObject->SetStringField("contentsId", this->Request->GetContentsId().GetValue());
            }
            if (this->Request->GetReceipt().IsSet() && !this->Request->GetReceipt().GetValue().IsEmpty())
            {
        ...
        if (ResponseCode / 100 == 2)
        {
                ...
                FJsonSerializer::Deserialize(JsonReader, JsonRootObject))
            {
                ...
                *Result = Result::FRecordReceiptResult::FromJson(JsonRootObject);
                return nullptr;
            }
            const auto Details = MakeShared<TArray<TSharedPtr<Core::Model::FGs2ErrorDetail>>>();
