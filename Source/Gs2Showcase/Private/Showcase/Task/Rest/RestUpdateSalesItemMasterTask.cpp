...
#include "Showcase/Task/Rest/UpdateSalesItemMasterTask.h"
...
{
    FUpdateSalesItemMasterTask::FUpdateSalesItemMasterTask(
    ...
    FUpdateSalesItemMasterTask::FUpdateSalesItemMasterTask(
        const FUpdateSalesItemMasterTask& From
    ): TGs2Future(From), Session(From.Session), Request(From.Request)
    {
    }

    Core::Model::FGs2ErrorPtr FUpdateSalesItemMasterTask::Action(
        const TSharedPtr<Result::FUpdateSalesItemMasterResultPtr> Result
    )
    {
        ...
        FString ResponseBody;
        {
            ...
            auto Url = Core::FGs2Constant::EndpointHost
                ...
                .Append("/{namespaceName}/master/salesItem/{salesItemName}");
            ...
            Url = Url.Replace(
                TEXT("{salesItemName}"),
                !this->Request->GetSalesItemName().IsSet() || this->Request->GetSalesItemName().GetValue().Len() == 0 ?
                    TEXT("null") : ToCStr(*this->Request->GetSalesItemName())
            );
        ...
        if (ResponseCode / 100 == 2)
        {
                ...
                FJsonSerializer::Deserialize(JsonReader, JsonRootObject))
            {
                ...
                *Result = Result::FUpdateSalesItemMasterResult::FromJson(JsonRootObject);
                return nullptr;
            }
            const auto Details = MakeShared<TArray<TSharedPtr<Core::Model::FGs2ErrorDetail>>>();
