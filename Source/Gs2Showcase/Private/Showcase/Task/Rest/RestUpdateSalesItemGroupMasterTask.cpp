...
#include "Showcase/Task/Rest/UpdateSalesItemGroupMasterTask.h"
...
{
    FUpdateSalesItemGroupMasterTask::FUpdateSalesItemGroupMasterTask(
    ...
    Core::Model::FGs2ErrorPtr FUpdateSalesItemGroupMasterTask::Action(
        const TSharedPtr<Result::FUpdateSalesItemGroupMasterResultPtr> Result
    )
    {
        ...
        FString ResponseBody;
        {
            ...
            auto Url = Core::FGs2Constant::EndpointHost
                ...
                .Append("/{namespaceName}/master/group/{salesItemGroupName}");
            ...
            Url = Url.Replace(
                TEXT("{salesItemGroupName}"),
                !this->Request->GetSalesItemGroupName().IsSet() || this->Request->GetSalesItemGroupName().GetValue().Len() == 0 ?
                    TEXT("null") : ToCStr(*this->Request->GetSalesItemGroupName())
            );
            ...
            if (this->Request->GetSalesItemNames() != nullptr && this->Request->GetSalesItemNames().IsValid())
            {
                TArray<TSharedPtr<FJsonValue>> v;
                for (auto JsonObjectValue : *this->Request->GetSalesItemNames())
                {
        ...
        if (ResponseCode / 100 == 2)
        {
                ...
                FJsonSerializer::Deserialize(JsonReader, JsonRootObject))
            {
                ...
                *Result = Result::FUpdateSalesItemGroupMasterResult::FromJson(JsonRootObject);
                return nullptr;
            }
            const auto Details = MakeShared<TArray<TSharedPtr<Core::Model::FGs2ErrorDetail>>>();
