...
namespace Gs2::Showcase::Domain::Iterator
{
    ...
    FDescribeShowcaseMastersIterator::FIterator& FDescribeShowcaseMastersIterator::FIterator::operator++()
    {
        ...
        {
            const auto ListParentKey = Gs2::Showcase::Model::Cache::FShowcaseMasterCache::CreateCacheParentKey(
                Self->NamespaceName,
                TOptional<int32>()
            );
            if (!RangeIteratorOpt)
            {
                Range = Self->Gs2->Cache->TryGetList<Gs2::Showcase::Model::FShowcaseMaster>(ListParentKey);

                if (Range)
                {
                    Range->RemoveAll([this](const Gs2::Showcase::Model::FShowcaseMasterPtr& Item) { return Self->NamePrefix && !Item->GetName()->StartsWith(*Self->NamePrefix); });
                    bLast = true;
            ...
            const auto Request =
                MakeShared<Gs2::Showcase::Request::FDescribeShowcaseMastersRequest>()
                    ...
                    ->WithLimit(FetchSize)
            ;
            const auto Future = Self->Client->DescribeShowcaseMasters(Request);
            ...
            Range = R->GetItems().IsValid() ? R->GetItems() : MakeShared<TArray<Gs2::Showcase::Model::FShowcaseMasterPtr>>();
            ...
            if (Range)
            {
                Range->RemoveAll([this](const Gs2::Showcase::Model::FShowcaseMasterPtr& Item) { return Self->NamePrefix.IsSet() && !Item->GetName()->StartsWith(*Self->NamePrefix); });
            }
            RangeIteratorOpt = Range->CreateIterator();
