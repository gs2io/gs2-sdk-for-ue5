...
namespace Gs2::Showcase::Domain::Iterator
{
    ...
    FDescribeRandomShowcaseMastersIterator::FIterator& FDescribeRandomShowcaseMastersIterator::FIterator::operator++()
    {
        ...
        {
            const auto ListParentKey = Gs2::Showcase::Model::Cache::FRandomShowcaseMasterCache::CreateCacheParentKey(
                Self->NamespaceName,
                TOptional<int32>()
            );
            if (!RangeIteratorOpt)
            ...
            const auto Request =
                MakeShared<Gs2::Showcase::Request::FDescribeRandomShowcaseMastersRequest>()
                    ...
                    ->WithLimit(FetchSize)
            ;
            const auto Future = Self->Client->DescribeRandomShowcaseMasters(Request);
            ...
            Range = R->GetItems().IsValid() ? R->GetItems() : MakeShared<TArray<Gs2::Showcase::Model::FRandomShowcaseMasterPtr>>();
            ...
            if (Range)
            {
                Range->RemoveAll([this](const Gs2::Showcase::Model::FRandomShowcaseMasterPtr& Item) { return Self->NamePrefix.IsSet() && !Item->GetName()->StartsWith(*Self->NamePrefix); });
            }
            RangeIteratorOpt = Range->CreateIterator();
