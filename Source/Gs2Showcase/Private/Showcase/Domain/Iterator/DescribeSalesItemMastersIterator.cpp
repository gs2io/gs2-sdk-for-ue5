...
namespace Gs2::Showcase::Domain::Iterator
{
    ...
    FDescribeSalesItemMastersIterator::FIterator& FDescribeSalesItemMastersIterator::FIterator::operator++()
    {
        ...
        {
            const auto ListParentKey = Gs2::Showcase::Model::Cache::FSalesItemMasterCache::CreateCacheParentKey(
                Self->NamespaceName,
                TOptional<int32>()
            );
            if (!RangeIteratorOpt)
            {
                ...
                if (Range)
                {
                    Range->RemoveAll([this](const Gs2::Showcase::Model::FSalesItemMasterPtr& Item) { return Self->NamePrefix && !Item->GetName()->StartsWith(*Self->NamePrefix); });
                    bLast = true;
            ...
            const auto Request =
                MakeShared<Gs2::Showcase::Request::FDescribeSalesItemMastersRequest>()
                    ...
                    ->WithLimit(FetchSize)
            ;
            const auto Future = Self->Client->DescribeSalesItemMasters(Request);
            ...
            Range = R->GetItems().IsValid() ? R->GetItems() : MakeShared<TArray<Gs2::Showcase::Model::FSalesItemMasterPtr>>();
            ...
            if (Range)
            {
                Range->RemoveAll([this](const Gs2::Showcase::Model::FSalesItemMasterPtr& Item) { return Self->NamePrefix.IsSet() && !Item->GetName()->StartsWith(*Self->NamePrefix); });
            }
            RangeIteratorOpt = Range->CreateIterator();
