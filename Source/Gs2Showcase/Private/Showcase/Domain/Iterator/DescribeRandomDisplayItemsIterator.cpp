...
namespace Gs2::Showcase::Domain::Iterator
{
    ...
    FDescribeRandomDisplayItemsIterator::FIterator& FDescribeRandomDisplayItemsIterator::FIterator::operator++()
    {
        ...
        if (!RangeIteratorOpt || (!*RangeIteratorOpt && !bLast))
        {
            ...
            ;
            const auto Future = Self->Client->DescribeRandomDisplayItems(Request);
            ...
            if (Range.IsValid())
            {
                for (const auto& Item : *Range)
                {
                    ...
                    Gs2::Showcase::Model::Cache::FRandomDisplayItemCache::Put(
                        Self->Gs2->Cache,
                        Request->GetNamespaceName(), CacheOwnerSnapshotUserId, Request->GetShowcaseName(), Item->GetName(),
