...
namespace Gs2::Idle::Domain::Iterator
{
    ...
    Gs2::Core::Model::FGs2ErrorPtr FDescribeStatusesByUserIdIterator::FIteratorNextTask::Action(TSharedPtr<TSharedPtr<Gs2::Idle::Model::FStatus>> Result)
    {
        ++Iterator;
    ...
    FDescribeStatusesByUserIdIterator::FIterator& FDescribeStatusesByUserIdIterator::FIterator::operator++()
    {
        ...
        {
            const auto ListParentKey = Gs2::Idle::Model::Cache::FStatusCache::CreateCacheParentKey(
                Self->NamespaceName,
                Self->UserId,
                TOptional<int32>()
            );
            ...
            Range = R->GetItems().IsValid() ? R->GetItems() : MakeShared<TArray<Gs2::Idle::Model::FStatusPtr>>();
            ...
            if (Range.IsValid())
            {
                for (const auto& Item : *Range)
                {
                    ...
                    Gs2::Idle::Model::Cache::FStatusCache::Put(
                        Self->Gs2->Cache,
                        Request->GetNamespaceName(), Request->GetUserId(), Item->GetCategoryName(),
