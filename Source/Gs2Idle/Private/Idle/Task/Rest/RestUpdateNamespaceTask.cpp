...
#include "Idle/Task/Rest/UpdateNamespaceTask.h"
...
namespace Gs2::Idle::Task::Rest
{
    ...
    )
    {
        ...
        FString ResponseBody;
        {
            ...
            if (this->Request->GetReceiveScript() != nullptr && this->Request->GetReceiveScript().IsValid())
            {
                JsonRootObject->SetObjectField(TEXT("receiveScript"), this->Request->GetReceiveScript()->ToJson());
            }
            if (this->Request->GetOverrideAcquireActionsScriptId().IsSet())
            {
                JsonRootObject->SetStringField(TEXT("overrideAcquireActionsScriptId"), this->Request->GetOverrideAcquireActionsScriptId().GetValue());
            }
