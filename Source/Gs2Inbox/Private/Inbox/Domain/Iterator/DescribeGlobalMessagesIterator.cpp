...
namespace Gs2::Inbox::Domain::Iterator
{
    ...
    FDescribeGlobalMessagesIterator::FDescribeGlobalMessagesIterator(
        const FDescribeGlobalMessagesIterator& From
    ):
        Gs2(From.Gs2),
        Client(From.Client),
    ...
    FDescribeGlobalMessagesIterator::FIterator& FDescribeGlobalMessagesIterator::FIterator::operator++()
    {
        ...
        {
            const auto ListParentKey = Gs2::Inbox::Model::Cache::FGlobalMessageCache::CreateCacheParentKey(
                Self->NamespaceName,
                TOptional<int32>()
            );
            if (!RangeIteratorOpt)
            {
                Range = Self->Gs2->Cache->TryGetList<Gs2::Inbox::Model::FGlobalMessage>(ListParentKey);
            ...
            ;
            const auto Future = Self->Client->DescribeGlobalMessages(Request);
            ...
            Range = R->GetItems().IsValid() ? R->GetItems() : MakeShared<TArray<Gs2::Inbox::Model::FGlobalMessagePtr>>();
