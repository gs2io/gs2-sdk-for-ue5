...
#include "Inbox/Task/WebSocket/ReadMessageByUserIdTask.h"
...
{
    FReadMessageByUserIdTask::FReadMessageByUserIdTask(
    ...
    Core::Model::FGs2ErrorPtr FReadMessageByUserIdTask::Action(
        const TSharedPtr<Result::FReadMessageByUserIdResultPtr> Result
    )
    {
        ...
        UE_LOG(Gs2Log, VeryVerbose, TEXT("[%s:%s:%s] %s"), TEXT("inbox"), TEXT("message"), TEXT("readMessageByUserId"), ToCStr(RequestPayload->Payload()));
        ...
        *Result = Result::FReadMessageByUserIdResult::FromJson(WebSocketResult->Body());
