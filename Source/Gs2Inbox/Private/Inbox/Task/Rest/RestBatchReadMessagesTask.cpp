...
#include "Inbox/Task/Rest/BatchReadMessagesTask.h"
...
{
    FBatchReadMessagesTask::FBatchReadMessagesTask(
    ...
    Core::Model::FGs2ErrorPtr FBatchReadMessagesTask::Action(
        const TSharedPtr<Result::FBatchReadMessagesResultPtr> Result
    )
    {
        ...
        FString ResponseBody;
        {
            ...
            auto Url = Core::FGs2Constant::EndpointHost
                ...
                .Append("/{namespaceName}/user/me/messages/read/batch");
            ...
            if (this->Request->GetMessageNames() != nullptr && this->Request->GetMessageNames().IsValid())
            {
                TArray<TSharedPtr<FJsonValue>> v;
                for (auto JsonObjectValue : *this->Request->GetMessageNames())
                {
                    v.Add(MakeShared<FJsonValueString>(JsonObjectValue));
                }
                JsonRootObject->SetArrayField(TEXT("messageNames"), v);
            }
        ...
        if (ResponseCode / 100 == 2)
        {
                ...
                FJsonSerializer::Deserialize(JsonReader, JsonRootObject))
            {
                ...
                *Result = Result::FBatchReadMessagesResult::FromJson(JsonRootObject);
                return nullptr;
            }
            const auto Details = MakeShared<TArray<TSharedPtr<Core::Model::FGs2ErrorDetail>>>();
    ...
    void FBatchReadMessagesTask::OnError(Core::Model::FGs2ErrorPtr Error)
    {
        if (Error->Count() > 0 && Error->Detail(0)->Code() == "inbox.message.expired") {
            TGs2Future<Result::FBatchReadMessagesResult>::OnError(MakeShared<Inbox::Error::FMessageExpiredError>(Error));
        }
        else {
            TGs2Future<Result::FBatchReadMessagesResult>::OnError(Error);
        }
    }
}