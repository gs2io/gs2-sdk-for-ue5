...
namespace Gs2::Ranking::Domain::Model
{
    ...
    )
    {
        ...
        if (ResultModel != nullptr) {
            ...
            if (ResultModel->GetItem() != nullptr)
            {
                ...
                Self->Gs2->Cache->Delete(Gs2::Ranking::Model::FCategoryModelMaster::TypeName, ParentKey, Key);
            }
        }
        auto Domain = Self;
    ...
    {
        const FString CacheKey = Gs2::Ranking::Domain::Model::FCategoryModelMasterDomain::CreateCacheKey(
            Self->CategoryName
        );
        return Self->Gs2->Cache->ExecuteWithKeyLock(
            ...
            [this, Result, CacheKey]() -> Gs2::Core::Model::FGs2ErrorPtr
            {
                ...
                auto bCacheHit = Self->Gs2->Cache->TryGet<Gs2::Ranking::Model::FCategoryModelMaster>(
                    Self->ParentKey,
                    CacheKey,
                    &Value
                );
                if (!bCacheHit) {
                    ...
                    if (Future->GetTask().IsError())
                    {
                        ...
                        if (!Error->GetErrors().IsValid() || Error->Count() == 0 || !Error->Detail(0).IsValid() || Error->Detail(0)->GetComponent() != "categoryModelMaster")
                        {
                            return Error;
                        }
                    }
    ...
    )
    {
        ...
        return Gs2->Cache->Subscribe(
            ...
            [WeakGs2, WeakService, RegisteredParentKey, QueryNamespaceName, QueryCategoryName]()
            {
