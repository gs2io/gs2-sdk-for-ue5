...
namespace Gs2::Ranking::Domain::Iterator
{
    ...
    Gs2::Core::Model::FGs2ErrorPtr FDescribeCategoryModelsIterator::FIteratorNextTask::Action(TSharedPtr<TSharedPtr<Gs2::Ranking::Model::FCategoryModel>> Result)
    {
        ++Iterator;
    ...
    FDescribeCategoryModelsIterator::FIterator& FDescribeCategoryModelsIterator::FIterator::operator++()
    {
        ...
        {
            const auto ListParentKey = Gs2::Ranking::Model::Cache::FCategoryModelCache::CreateCacheParentKey(
                Self->NamespaceName,
                TOptional<int32>()
            );
            if (!RangeIteratorOpt)
            {
                Range = Self->Gs2->Cache->TryGetList<Gs2::Ranking::Model::FCategoryModel>(ListParentKey);
            ...
            ;
            const auto Future = Self->Client->DescribeCategoryModels(Request);
            ...
            Range = R->GetItems().IsValid() ? R->GetItems() : MakeShared<TArray<Gs2::Ranking::Model::FCategoryModelPtr>>();
            ...
            if (Range.IsValid())
            {
                for (const auto& Item : *Range)
                {
                    ...
                    Gs2::Ranking::Model::Cache::FCategoryModelCache::Put(
                        Self->Gs2->Cache,
