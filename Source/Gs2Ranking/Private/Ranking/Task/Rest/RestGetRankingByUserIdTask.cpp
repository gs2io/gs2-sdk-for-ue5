...
#include "Ranking/Task/Rest/GetRankingByUserIdTask.h"
...
{
    FGetRankingByUserIdTask::FGetRankingByUserIdTask(
    ...
    Core::Model::FGs2ErrorPtr FGetRankingByUserIdTask::Action(
        const TSharedPtr<Result::FGetRankingByUserIdResultPtr> Result
    )
    {
        ...
        FString ResponseBody;
        {
            ...
            auto Url = Core::FGs2Constant::EndpointHost
                ...
                .Append("/{namespaceName}/user/{userId}/category/{categoryName}/ranking/scorer/{scorerUserId}/score/{uniqueId}");
            ...
            Url = Url.Replace(
                TEXT("{scorerUserId}"),
                !this->Request->GetScorerUserId().IsSet() || this->Request->GetScorerUserId().GetValue().Len() == 0 ?
                    TEXT("null") : ToCStr(*this->Request->GetScorerUserId())
            );
            Url = Url.Replace(
                TEXT("{uniqueId}"),
                !this->Request->GetUniqueId().IsSet() || this->Request->GetUniqueId().GetValue().Len() == 0 ?
                    TEXT("null") : ToCStr(*this->Request->GetUniqueId())
            );
        ...
        if (ResponseCode / 100 == 2)
        {
                ...
                FJsonSerializer::Deserialize(JsonReader, JsonRootObject))
            {
                ...
                *Result = Result::FGetRankingByUserIdResult::FromJson(JsonRootObject);
                return nullptr;
            }
            const auto Details = MakeShared<TArray<TSharedPtr<Core::Model::FGs2ErrorDetail>>>();
