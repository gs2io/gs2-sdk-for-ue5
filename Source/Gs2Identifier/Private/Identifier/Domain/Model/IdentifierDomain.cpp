...
namespace Gs2::Identifier::Domain::Model
{
    ...
    {
        const FString CacheKey = Gs2::Identifier::Domain::Model::FIdentifierDomain::CreateCacheKey(
            Self->ClientId
        );
            ...
            [this, Result, CacheKey]() -> Gs2::Core::Model::FGs2ErrorPtr
            {
                ...
                if (!bCacheHit) {
                    ...
                    if (Future->GetTask().IsError())
                    {
                        ...
                        if (!Error->GetErrors().IsValid() || Error->Count() == 0 || !Error->Detail(0).IsValid() || Error->Detail(0)->GetComponent() != "identifier")
                        {
                            return Error;
                        }
                    }
    ...
    )
    {
        ...
        const TOptional<FString> QueryClientId = ClientId;
        return Gs2->Cache->Subscribe(
            ...
            [WeakGs2, WeakService, RegisteredParentKey, QueryUserName, QueryClientId]()
            {
                ...
                const auto Domain = MakeShared<FIdentifierDomain>(
                    Owner,
                    WeakService.Pin(),
                    QueryUserName,
                    QueryClientId
