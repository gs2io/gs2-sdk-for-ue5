...
#include "Identifier/Task/Rest/UpdateSecurityPolicyTask.h"
...
{
    FUpdateSecurityPolicyTask::FUpdateSecurityPolicyTask(
    ...
    FUpdateSecurityPolicyTask::FUpdateSecurityPolicyTask(
        const FUpdateSecurityPolicyTask& From
    ): TGs2Future(From), Session(From.Session), Request(From.Request)
    {
    }

    Core::Model::FGs2ErrorPtr FUpdateSecurityPolicyTask::Action(
        const TSharedPtr<Result::FUpdateSecurityPolicyResultPtr> Result
    )
    {
        ...
        FString ResponseBody;
        {
            ...
            auto Url = Core::FGs2Constant::EndpointHost
                ...
                .Append("/securityPolicy/{securityPolicyName}");

            Url = Url.Replace(
                TEXT("{securityPolicyName}"),
                !this->Request->GetSecurityPolicyName().IsSet() || this->Request->GetSecurityPolicyName().GetValue().Len() == 0 ?
                    TEXT("null") : ToCStr(*this->Request->GetSecurityPolicyName())
            );
            ...
            if (this->Request->GetPolicy().IsSet() && !this->Request->GetPolicy().GetValue().IsEmpty())
            {
                JsonRootObject->SetStringField("policy", this->Request->GetPolicy().GetValue());
            }
        ...
        if (ResponseCode / 100 == 2)
        {
                ...
                FJsonSerializer::Deserialize(JsonReader, JsonRootObject))
            {
                ...
                *Result = Result::FUpdateSecurityPolicyResult::FromJson(JsonRootObject);
                return nullptr;
            }
            const auto Details = MakeShared<TArray<TSharedPtr<Core::Model::FGs2ErrorDetail>>>();
