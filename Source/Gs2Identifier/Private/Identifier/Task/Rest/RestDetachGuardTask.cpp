...
#include "Identifier/Task/Rest/DetachGuardTask.h"
...
{
    FDetachGuardTask::FDetachGuardTask(
    ...
    Core::Model::FGs2ErrorPtr FDetachGuardTask::Action(
        const TSharedPtr<Result::FDetachGuardResultPtr> Result
    )
    {
        ...
        FString ResponseBody;
        {
            ...
            auto Url = Core::FGs2Constant::EndpointHost
                ...
                .Append("/user/{userName}/identifier/{clientId}/guard/{guardNamespaceId}");
            ...
            Url = Url.Replace(
                TEXT("{clientId}"),
                !this->Request->GetClientId().IsSet() || this->Request->GetClientId().GetValue().Len() == 0 ?
                    TEXT("null") : ToCStr(*this->Request->GetClientId())
            );
            Url = Url.Replace(
                TEXT("{guardNamespaceId}"),
                !this->Request->GetGuardNamespaceId().IsSet() || this->Request->GetGuardNamespaceId().GetValue().Len() == 0 ?
                    TEXT("null") : ToCStr(*this->Request->GetGuardNamespaceId())
            );
        ...
        if (ResponseCode / 100 == 2)
        {
                ...
                FJsonSerializer::Deserialize(JsonReader, JsonRootObject))
            {
                ...
                *Result = Result::FDetachGuardResult::FromJson(JsonRootObject);
                return nullptr;
            }
            const auto Details = MakeShared<TArray<TSharedPtr<Core::Model::FGs2ErrorDetail>>>();
