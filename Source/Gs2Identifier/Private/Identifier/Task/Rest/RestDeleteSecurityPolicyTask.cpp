...
#include "Identifier/Task/Rest/DeleteSecurityPolicyTask.h"
...
{
    FDeleteSecurityPolicyTask::FDeleteSecurityPolicyTask(
    ...
    FDeleteSecurityPolicyTask::FDeleteSecurityPolicyTask(
        const FDeleteSecurityPolicyTask& From
    ): TGs2Future(From), Session(From.Session), Request(From.Request)
    {
    }

    Core::Model::FGs2ErrorPtr FDeleteSecurityPolicyTask::Action(
        const TSharedPtr<Result::FDeleteSecurityPolicyResultPtr> Result
    )
    {
        ...
        if (ResponseCode / 100 == 2)
        {
                ...
                FJsonSerializer::Deserialize(JsonReader, JsonRootObject))
            {
                ...
                *Result = Result::FDeleteSecurityPolicyResult::FromJson(JsonRootObject);
                return nullptr;
            }
            const auto Details = MakeShared<TArray<TSharedPtr<Core::Model::FGs2ErrorDetail>>>();
