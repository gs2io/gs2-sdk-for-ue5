...
#include "MegaField/Task/Rest/FetchPositionFromSystemTask.h"
...
{
    FFetchPositionFromSystemTask::FFetchPositionFromSystemTask(
    ...
    Core::Model::FGs2ErrorPtr FFetchPositionFromSystemTask::Action(
        const TSharedPtr<Result::FFetchPositionFromSystemResultPtr> Result
    )
    {
        ...
        FString ResponseBody;
        {
            ...
            auto Url = Core::FGs2Constant::EndpointHost
                .Replace(TEXT("{service}"), TEXT("mega-field"))
                .Replace(TEXT("{region}"), *this->Session->RegionName())
                .Append("/{namespaceName}/system/area/{areaModelName}/layer/{layerModelName}/spatial/fetch");
            ...
            Url = Url.Replace(
                TEXT("{areaModelName}"),
                !this->Request->GetAreaModelName().IsSet() || this->Request->GetAreaModelName().GetValue().Len() == 0 ?
                    TEXT("null") : ToCStr(*this->Request->GetAreaModelName())
            );
            Url = Url.Replace(
                TEXT("{layerModelName}"),
                !this->Request->GetLayerModelName().IsSet() || this->Request->GetLayerModelName().GetValue().Len() == 0 ?
                    TEXT("null") : ToCStr(*this->Request->GetLayerModelName())
            );
            ...
            if (this->Request->GetUserIds() != nullptr && this->Request->GetUserIds().IsValid())
            {
                TArray<TSharedPtr<FJsonValue>> v;
                for (auto JsonObjectValue : *this->Request->GetUserIds())
                {
        ...
        if (ResponseCode / 100 == 2)
        {
                ...
                FJsonSerializer::Deserialize(JsonReader, JsonRootObject))
            {
                ...
                *Result = Result::FFetchPositionFromSystemResult::FromJson(JsonRootObject);
                return nullptr;
            }
            const auto Details = MakeShared<TArray<TSharedPtr<Core::Model::FGs2ErrorDetail>>>();
