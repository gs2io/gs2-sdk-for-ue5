...
namespace Gs2::MegaField::Domain::Iterator
{
    ...
    FDescribeAreaModelsIterator::FIterator& FDescribeAreaModelsIterator::FIterator::operator++()
    {
        ...
        {
            const auto ListParentKey = Gs2::MegaField::Model::Cache::FAreaModelCache::CreateCacheParentKey(
                Self->NamespaceName,
                TOptional<int32>()
            );
            if (!RangeIteratorOpt)
            {
                Range = Self->Gs2->Cache->TryGetList<Gs2::MegaField::Model::FAreaModel>(ListParentKey);
            ...
            ;
            const auto Future = Self->Client->DescribeAreaModels(Request);
            ...
            Range = R->GetItems().IsValid() ? R->GetItems() : MakeShared<TArray<Gs2::MegaField::Model::FAreaModelPtr>>();
