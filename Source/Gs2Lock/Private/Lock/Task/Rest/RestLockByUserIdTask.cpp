...
#include "Lock/Task/Rest/LockByUserIdTask.h"
...
{
    FLockByUserIdTask::FLockByUserIdTask(
    ...
    Core::Model::FGs2ErrorPtr FLockByUserIdTask::Action(
        const TSharedPtr<Result::FLockByUserIdResultPtr> Result
    )
    {
        ...
        FString ResponseBody;
        {
            ...
            auto Url = Core::FGs2Constant::EndpointHost
                .Replace(TEXT("{service}"), TEXT("lock"))
                .Replace(TEXT("{region}"), *this->Session->RegionName())
                .Append("/{namespaceName}/user/{userId}/mutex/{propertyId}/lock");
            ...
            if (this->Request->GetTransactionId().IsSet() && !this->Request->GetTransactionId().GetValue().IsEmpty())
            {
                JsonRootObject->SetStringField("transactionId", this->Request->GetTransactionId().GetValue());
            }
            if (this->Request->GetTtl().IsSet())
            {
                JsonRootObject->SetStringField("ttl", FString::Printf(TEXT("%lld"), this->Request->GetTtl().GetValue()));
            }
        ...
        if (ResponseCode / 100 == 2)
        {
                ...
                FJsonSerializer::Deserialize(JsonReader, JsonRootObject))
            {
                ...
                *Result = Result::FLockByUserIdResult::FromJson(JsonRootObject);
                return nullptr;
            }
            const auto Details = MakeShared<TArray<TSharedPtr<Core::Model::FGs2ErrorDetail>>>();
