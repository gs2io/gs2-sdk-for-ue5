...
#include "Deploy/Task/Rest/GetStackStatusTask.h"
...
{
    FGetStackStatusTask::FGetStackStatusTask(
    ...
    FGetStackStatusTask::FGetStackStatusTask(
        const FGetStackStatusTask& From
    ): TGs2Future(From), Session(From.Session), Request(From.Request)
    {
    }

    Core::Model::FGs2ErrorPtr FGetStackStatusTask::Action(
        const TSharedPtr<Result::FGetStackStatusResultPtr> Result
    )
    {
        ...
        if (ResponseCode / 100 == 2)
        {
                ...
                FJsonSerializer::Deserialize(JsonReader, JsonRootObject))
            {
                ...
                *Result = Result::FGetStackStatusResult::FromJson(JsonRootObject);
                return nullptr;
            }
            const auto Details = MakeShared<TArray<TSharedPtr<Core::Model::FGs2ErrorDetail>>>();
