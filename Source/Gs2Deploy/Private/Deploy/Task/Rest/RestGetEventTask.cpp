...
#include "Deploy/Task/Rest/GetEventTask.h"
...
namespace Gs2::Deploy::Task::Rest
{
    ...
    FGetEventTask::FGetEventTask(
        const FGetEventTask& From
    ): TGs2Future(From), Session(From.Session), Request(From.Request)
    {
    }

    Core::Model::FGs2ErrorPtr FGetEventTask::Action(
        const TSharedPtr<Result::FGetEventResultPtr> Result
    )
    {
        ...
        FString ResponseBody;
        {
            ...
            auto Url = Core::FGs2Constant::EndpointHost
                ...
                .Append("/stack/{stackName}/event/{eventName}");
            ...
            Url = Url.Replace(
                TEXT("{eventName}"),
                !this->Request->GetEventName().IsSet() || this->Request->GetEventName().GetValue().Len() == 0 ?
                    TEXT("null") : ToCStr(*this->Request->GetEventName())
            );
        ...
        if (ResponseCode / 100 == 2)
        {
                ...
                FJsonSerializer::Deserialize(JsonReader, JsonRootObject))
            {
                ...
                *Result = Result::FGetEventResult::FromJson(JsonRootObject);
                return nullptr;
            }
            const auto Details = MakeShared<TArray<TSharedPtr<Core::Model::FGs2ErrorDetail>>>();
