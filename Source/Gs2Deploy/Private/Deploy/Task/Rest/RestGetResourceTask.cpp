...
#include "Deploy/Task/Rest/GetResourceTask.h"
...
{
    FGetResourceTask::FGetResourceTask(
    ...
    Core::Model::FGs2ErrorPtr FGetResourceTask::Action(
        const TSharedPtr<Result::FGetResourceResultPtr> Result
    )
    {
        ...
        FString ResponseBody;
        {
            ...
            auto Url = Core::FGs2Constant::EndpointHost
                .Replace(TEXT("{service}"), TEXT("deploy"))
                .Replace(TEXT("{region}"), *this->Session->RegionName())
                .Append("/stack/{stackName}/resource/{resourceName}");

            Url = Url.Replace(
                TEXT("{stackName}"),
                !this->Request->GetStackName().IsSet() || this->Request->GetStackName().GetValue().Len() == 0 ?
                    TEXT("null") : ToCStr(*this->Request->GetStackName())
            );
            Url = Url.Replace(
                TEXT("{resourceName}"),
                !this->Request->GetResourceName().IsSet() || this->Request->GetResourceName().GetValue().Len() == 0 ?
                    TEXT("null") : ToCStr(*this->Request->GetResourceName())
            );
        ...
        if (ResponseCode / 100 == 2)
        {
                ...
                FJsonSerializer::Deserialize(JsonReader, JsonRootObject))
            {
                ...
                *Result = Result::FGetResourceResult::FromJson(JsonRootObject);
                return nullptr;
            }
            const auto Details = MakeShared<TArray<TSharedPtr<Core::Model::FGs2ErrorDetail>>>();
