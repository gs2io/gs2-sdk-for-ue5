...
namespace Gs2::Deploy::Domain::Iterator
{
    ...
    FDescribeStacksIterator::FIterator& FDescribeStacksIterator::FIterator::operator++()
    {
        ...
        {
            const auto ListParentKey = Gs2::Deploy::Model::Cache::FStackCache::CreateCacheParentKey(
                TOptional<int32>()
            );
            if (!RangeIteratorOpt)
            {
            ...
            const auto Request =
                MakeShared<Gs2::Deploy::Request::FDescribeStacksRequest>()
                    ...
                    ->WithLimit(FetchSize)
            ;
            const auto Future = Self->Client->DescribeStacks(Request);
            ...
            Range = R->GetItems().IsValid() ? R->GetItems() : MakeShared<TArray<Gs2::Deploy::Model::FStackPtr>>();
