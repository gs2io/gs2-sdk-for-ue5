...
namespace Gs2::Deploy::Domain::Iterator
{
    ...
    FDescribeOutputsIterator::FIterator& FDescribeOutputsIterator::FIterator::operator++()
    {
        ...
        {
            const auto ListParentKey = Gs2::Deploy::Model::Cache::FOutputCache::CreateCacheParentKey(
                Self->StackName,
                TOptional<int32>()
            );
            if (!RangeIteratorOpt)
            {
                Range = Self->Gs2->Cache->TryGetList<Gs2::Deploy::Model::FOutput>(ListParentKey);
            ...
            Range = R->GetItems().IsValid() ? R->GetItems() : MakeShared<TArray<Gs2::Deploy::Model::FOutputPtr>>();
