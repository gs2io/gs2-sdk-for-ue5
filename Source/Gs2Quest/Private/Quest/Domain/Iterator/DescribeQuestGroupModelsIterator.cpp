...
namespace Gs2::Quest::Domain::Iterator
{
    ...
    FDescribeQuestGroupModelsIterator::FIterator& FDescribeQuestGroupModelsIterator::FIterator::operator++()
    {
        ...
        {
            const auto ListParentKey = Gs2::Quest::Model::Cache::FQuestGroupModelCache::CreateCacheParentKey(
                Self->NamespaceName,
                TOptional<int32>()
            );
            if (!RangeIteratorOpt)
            {
                Range = Self->Gs2->Cache->TryGetList<Gs2::Quest::Model::FQuestGroupModel>(ListParentKey);
            ...
            ;
            const auto Future = Self->Client->DescribeQuestGroupModels(Request);
            ...
            Range = R->GetItems().IsValid() ? R->GetItems() : MakeShared<TArray<Gs2::Quest::Model::FQuestGroupModelPtr>>();
