...
namespace Gs2::Enhance::Domain::Model
{
    ...
    )
    {
        ...
        if (ResultModel != nullptr) {
            ...
            if (ResultModel->GetItem() != nullptr)
            {
                ...
                Self->Gs2->Cache->Delete(Gs2::Enhance::Model::FUnleashRateModelMaster::TypeName, ParentKey, Key);
            }
        }
        auto Domain = Self;
    ...
    )
    {
        ...
        TSharedPtr<Gs2::Enhance::Model::FUnleashRateModelMaster> Value;
        auto bCacheHit = Self->Gs2->Cache->TryGet<Gs2::Enhance::Model::FUnleashRateModelMaster>(
            Self->ParentKey,
        ...
        if (!bCacheHit) {
            ...
            if (Future->GetTask().IsError())
            {
                ...
                if (Future->GetTask().Error()->Detail(0)->GetComponent() != "unleashRateModelMaster")
                {
                    return Future->GetTask().Error();
                }
            }
            Self->Gs2->Cache->TryGet<Gs2::Enhance::Model::FUnleashRateModelMaster>(
                Self->ParentKey,
                Gs2::Enhance::Domain::Model::FUnleashRateModelMasterDomain::CreateCacheKey(
                    Self->RateName
                ),
