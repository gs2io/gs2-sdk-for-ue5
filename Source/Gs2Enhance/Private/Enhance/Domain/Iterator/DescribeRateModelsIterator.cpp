...
namespace Gs2::Enhance::Domain::Iterator
{
    ...
    FDescribeRateModelsIterator::FIterator& FDescribeRateModelsIterator::FIterator::operator++()
    {
        ...
        {
            const auto ListParentKey = Gs2::Enhance::Model::Cache::FRateModelCache::CreateCacheParentKey(
                Self->NamespaceName,
                TOptional<int32>()
            );
            if (!RangeIteratorOpt)
            {
                Range = Self->Gs2->Cache->TryGetList<Gs2::Enhance::Model::FRateModel>(ListParentKey);
            ...
            Range = R->GetItems().IsValid() ? R->GetItems() : MakeShared<TArray<Gs2::Enhance::Model::FRateModelPtr>>();
