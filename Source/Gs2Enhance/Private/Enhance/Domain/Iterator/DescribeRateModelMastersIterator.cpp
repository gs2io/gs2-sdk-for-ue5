    ...
    FDescribeRateModelMastersIterator::FIterator& FDescribeRateModelMastersIterator::FIterator::operator++()
    {
        ...
        if (!RangeIteratorOpt || (!*RangeIteratorOpt && !bLast))
        {
            ...
            if (!RangeIteratorOpt)
            {
                ...
                if (Range)
                {
                    Range->RemoveAll([this](const Gs2::Enhance::Model::FRateModelMasterPtr& Item) { return Self->NamePrefix && !Item->GetName()->StartsWith(*Self->NamePrefix); });
                    bLast = true;
            ...
            if (Range)
            {
                Range->RemoveAll([this](const Gs2::Enhance::Model::FRateModelMasterPtr& Item) { return Self->NamePrefix.IsSet() && !Item->GetName()->StartsWith(*Self->NamePrefix); });
            }
            RangeIteratorOpt = Range->CreateIterator();
