...
#include "Enhance/Task/Rest/CreateProgressByUserIdTask.h"
...
{
    FCreateProgressByUserIdTask::FCreateProgressByUserIdTask(
    ...
    Core::Model::FGs2ErrorPtr FCreateProgressByUserIdTask::Action(
        const TSharedPtr<Result::FCreateProgressByUserIdResultPtr> Result
    )
    {
        ...
        FString ResponseBody;
        {
            ...
            auto Url = Core::FGs2Constant::EndpointHost
                ...
                .Append("/{namespaceName}/user/{userId}/progress");
            ...
            if (this->Request->GetRateName().IsSet() && !this->Request->GetRateName().GetValue().IsEmpty())
            {
                JsonRootObject->SetStringField("rateName", this->Request->GetRateName().GetValue());
            }
            if (this->Request->GetTargetItemSetId().IsSet() && !this->Request->GetTargetItemSetId().GetValue().IsEmpty())
            {
            ...
            if (this->Request->GetMaterials() != nullptr && this->Request->GetMaterials().IsValid())
            {
                ...
                for (auto JsonObjectValue : *this->Request->GetMaterials())
                {
        ...
        if (ResponseCode / 100 == 2)
        {
                ...
                FJsonSerializer::Deserialize(JsonReader, JsonRootObject))
            {
                ...
                *Result = Result::FCreateProgressByUserIdResult::FromJson(JsonRootObject);
                return nullptr;
            }
            const auto Details = MakeShared<TArray<TSharedPtr<Core::Model::FGs2ErrorDetail>>>();
