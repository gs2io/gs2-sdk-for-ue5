...
namespace Gs2::Experience::Domain::Iterator
{
    ...
    Gs2::Core::Model::FGs2ErrorPtr FDescribeStatusesByUserIdIterator::FIteratorNextTask::Action(TSharedPtr<TSharedPtr<Gs2::Experience::Model::FStatus>> Result)
    {
        ++Iterator;
    ...
    FDescribeStatusesByUserIdIterator::FIterator& FDescribeStatusesByUserIdIterator::FIterator::operator++()
    {
        ...
        {
            const auto ListParentKey = Gs2::Experience::Model::Cache::FStatusCache::CreateCacheParentKey(
                Self->NamespaceName,
                Self->UserId,
                TOptional<int32>()
            );
            if (!RangeIteratorOpt)
            {
                Range = Self->Gs2->Cache->TryGetList<Gs2::Experience::Model::FStatus>(ListParentKey);
            ...
            const auto Request =
                MakeShared<Gs2::Experience::Request::FDescribeStatusesByUserIdRequest>()
                    ...
                    ->WithLimit(FetchSize)
            ;
            const auto Future = Self->Client->DescribeStatusesByUserId(Request);
            ...
            Range = R->GetItems().IsValid() ? R->GetItems() : MakeShared<TArray<Gs2::Experience::Model::FStatusPtr>>();
            ...
            if (Range.IsValid())
            {
                for (const auto& Item : *Range)
                {
                    ...
                    Gs2::Experience::Model::Cache::FStatusCache::Put(
                        Self->Gs2->Cache,
                        Request->GetNamespaceName(), Request->GetUserId(), Item->GetExperienceName(), Item->GetPropertyId(),
