...
namespace Gs2::Experience::Domain::Iterator
{
    ...
    FDescribeThresholdMastersIterator::FIterator& FDescribeThresholdMastersIterator::FIterator::operator++()
    {
        ...
        {
            const auto ListParentKey = Gs2::Experience::Model::Cache::FThresholdMasterCache::CreateCacheParentKey(
                Self->NamespaceName,
                TOptional<int32>()
            );
            if (!RangeIteratorOpt)
            {
                ...
                if (Range)
                {
                    Range->RemoveAll([this](const Gs2::Experience::Model::FThresholdMasterPtr& Item) { return Self->NamePrefix && !Item->GetName()->StartsWith(*Self->NamePrefix); });
                    bLast = true;
            ...
            const auto Request =
                MakeShared<Gs2::Experience::Request::FDescribeThresholdMastersRequest>()
                    ...
                    ->WithLimit(FetchSize)
            ;
            const auto Future = Self->Client->DescribeThresholdMasters(Request);
            ...
            Range = R->GetItems().IsValid() ? R->GetItems() : MakeShared<TArray<Gs2::Experience::Model::FThresholdMasterPtr>>();
            ...
            if (Range)
            {
                Range->RemoveAll([this](const Gs2::Experience::Model::FThresholdMasterPtr& Item) { return Self->NamePrefix.IsSet() && !Item->GetName()->StartsWith(*Self->NamePrefix); });
            }
            RangeIteratorOpt = Range->CreateIterator();
