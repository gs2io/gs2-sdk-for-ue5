...
namespace Gs2::Experience::Domain::SpeculativeExecutor
{
    ...
    )
    {
        ...
        if (FVerifyRankByUserIdSpeculativeExecutor::Action() == NewVerifyAction->GetAction()) {
            ...
            auto Future = Inverse
                ? FVerifyRankByUserIdSpeculativeExecutor::ExecuteInverse(
                    Domain,
                    Service,
                    AccessToken,
                    Request)
                : FVerifyRankByUserIdSpeculativeExecutor::Execute(
                Domain,
                Service,
                AccessToken,
                Request
        ...
        if (FVerifyRankCapByUserIdSpeculativeExecutor::Action() == NewVerifyAction->GetAction()) {
            TSharedPtr<FJsonObject> RequestModelJson;
            if (const TSharedRef<TJsonReader<>> JsonReader = TJsonReaderFactory<>::Create(NewVerifyAction->GetRequest().IsSet() ? *NewVerifyAction->GetRequest() : "{}");
                !FJsonSerializer::Deserialize(JsonReader, RequestModelJson))
            {
            ...
            auto Future = Inverse
                ? FVerifyRankCapByUserIdSpeculativeExecutor::ExecuteInverse(
                    Domain,
                    Service,
                    AccessToken,
                    Request)
                : FVerifyRankCapByUserIdSpeculativeExecutor::Execute(
                Domain,
                Service,
                AccessToken,
                Request
