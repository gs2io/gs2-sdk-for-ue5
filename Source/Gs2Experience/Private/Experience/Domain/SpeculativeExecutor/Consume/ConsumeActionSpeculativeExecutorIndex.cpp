...
namespace Gs2::Experience::Domain::SpeculativeExecutor
{
    ...
    )
    {
        ...
        if (FSubExperienceByUserIdSpeculativeExecutor::Action() == NewConsumeAction->GetAction()) {
            ...
            auto Request = Request::FSubExperienceByUserIdRequest::FromJson(RequestModelJson);
            Request = FSubExperienceByUserIdSpeculativeExecutor::Rate(Request, Rate);
            auto Future = FSubExperienceByUserIdSpeculativeExecutor::Execute(
                Domain,
                Service,
                AccessToken,
                Request
        ...
        if (FSubRankCapByUserIdSpeculativeExecutor::Action() == NewConsumeAction->GetAction()) {
            TSharedPtr<FJsonObject> RequestModelJson;
            if (const TSharedRef<TJsonReader<>> JsonReader = TJsonReaderFactory<>::Create(NewConsumeAction->GetRequest().IsSet() ? *NewConsumeAction->GetRequest() : "{}");
                !FJsonSerializer::Deserialize(JsonReader, RequestModelJson))
            {
