...
#include "Gateway/Task/Rest/SetFirebaseTokenByUserIdTask.h"
...
{
    FSetFirebaseTokenByUserIdTask::FSetFirebaseTokenByUserIdTask(
    ...
    FSetFirebaseTokenByUserIdTask::FSetFirebaseTokenByUserIdTask(
        const FSetFirebaseTokenByUserIdTask& From
    ): TGs2Future(From), Session(From.Session), Request(From.Request)
    {
    }

    Core::Model::FGs2ErrorPtr FSetFirebaseTokenByUserIdTask::Action(
        const TSharedPtr<Result::FSetFirebaseTokenByUserIdResultPtr> Result
    )
    {
        ...
        const auto Completion = MakeShared<Core::Net::Rest::FRestResponseState, ESPMode::ThreadSafe>();
        {
            ...
            auto Url = Core::FGs2Constant::EndpointHost
                ...
                .Append("/{namespaceName}/user/{userId}/firebase/token");
            ...
            {
                JsonRootObject->SetStringField(TEXT("token"), this->Request->GetToken().GetValue());
            }
            if (this->Request->GetLocale().IsSet())
            {
                JsonRootObject->SetStringField(TEXT("locale"), this->Request->GetLocale().GetValue());
            }
        ...
        if (ResponseCode / 100 == 2)
        {
                ...
                FJsonSerializer::Deserialize(JsonReader, JsonRootObject))
            {
                ...
                *Result = Result::FSetFirebaseTokenByUserIdResult::FromJson(JsonRootObject);
                return nullptr;
            }
            const auto Details = MakeShared<TArray<TSharedPtr<Core::Model::FGs2ErrorDetail>>>();
            return MakeShared<Core::Model::FUnknownError>(Details);
