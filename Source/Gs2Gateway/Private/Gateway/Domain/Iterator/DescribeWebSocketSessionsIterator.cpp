...
namespace Gs2::Gateway::Domain::Iterator
{
    ...
    FDescribeWebSocketSessionsIterator::FIterator& FDescribeWebSocketSessionsIterator::FIterator::operator++()
    {
        ...
        {
            const auto ListParentKey = Gs2::Gateway::Model::Cache::FWebSocketSessionCache::CreateCacheParentKey(
                Self->NamespaceName,
            ...
            const auto Request =
                MakeShared<Gs2::Gateway::Request::FDescribeWebSocketSessionsRequest>()
                    ...
                    ->WithLimit(FetchSize)
            ;
            const auto Future = Self->Client->DescribeWebSocketSessions(Request);
            ...
            Range = R->GetItems().IsValid() ? R->GetItems() : MakeShared<TArray<Gs2::Gateway::Model::FWebSocketSessionPtr>>();
            ...
            if (Range.IsValid())
            {
                for (const auto& Item : *Range)
                {
                    ...
                    Gs2::Gateway::Model::Cache::FWebSocketSessionCache::Put(
                        Self->Gs2->Cache,
                        Request->GetNamespaceName(), CacheOwnerSnapshotUserId,
