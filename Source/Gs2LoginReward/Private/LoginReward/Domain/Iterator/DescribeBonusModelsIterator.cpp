...
namespace Gs2::LoginReward::Domain::Iterator
{
    ...
    FDescribeBonusModelsIterator::FIterator& FDescribeBonusModelsIterator::FIterator::operator++()
    {
        ...
        {
            const auto ListParentKey = Gs2::LoginReward::Model::Cache::FBonusModelCache::CreateCacheParentKey(
                Self->NamespaceName,
                TOptional<int32>()
            );
            if (!RangeIteratorOpt)
            ...
            ;
            const auto Future = Self->Client->DescribeBonusModels(Request);
            ...
            Range = R->GetItems().IsValid() ? R->GetItems() : MakeShared<TArray<Gs2::LoginReward::Model::FBonusModelPtr>>();
