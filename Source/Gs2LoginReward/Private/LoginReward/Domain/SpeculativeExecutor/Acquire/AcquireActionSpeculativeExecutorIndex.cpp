...
namespace Gs2::LoginReward::Domain::SpeculativeExecutor
{
    ...
    )
    {
        ...
        if (FDeleteReceiveStatusByUserIdSpeculativeExecutor::Action() == NewAcquireAction->GetAction()) {
            ...
            auto Request = Request::FDeleteReceiveStatusByUserIdRequest::FromJson(RequestModelJson);
            Request = FDeleteReceiveStatusByUserIdSpeculativeExecutor::Rate(Request, Rate);
            auto Future = FDeleteReceiveStatusByUserIdSpeculativeExecutor::Execute(
                Domain,
                Service,
                AccessToken,
                Request
        ...
        if (FUnmarkReceivedByUserIdSpeculativeExecutor::Action() == NewAcquireAction->GetAction()) {
            TSharedPtr<FJsonObject> RequestModelJson;
            if (const TSharedRef<TJsonReader<>> JsonReader = TJsonReaderFactory<>::Create(NewAcquireAction->GetRequest().IsSet() ? *NewAcquireAction->GetRequest() : "{}");
                !FJsonSerializer::Deserialize(JsonReader, RequestModelJson))
            {
            ...
            auto Request = Request::FUnmarkReceivedByUserIdRequest::FromJson(RequestModelJson);
            Request = FUnmarkReceivedByUserIdSpeculativeExecutor::Rate(Request, Rate);
            auto Future = FUnmarkReceivedByUserIdSpeculativeExecutor::Execute(
                Domain,
                Service,
                AccessToken,
                Request
