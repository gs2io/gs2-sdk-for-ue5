...
#include "StateMachine/Task/WebSocket/ReportTask.h"
...
namespace Gs2::StateMachine::Task::WebSocket
{
    ...
    )
    {
        ...
        UE_LOG(Gs2Log, VeryVerbose, TEXT("[%s:%s:%s] %s"), TEXT("stateMachine"), TEXT("status"), TEXT("report"), ToCStr(RequestPayload->Payload()));
        ...
        *Result = Result::FReportResult::FromJson(WebSocketResult->Body());
