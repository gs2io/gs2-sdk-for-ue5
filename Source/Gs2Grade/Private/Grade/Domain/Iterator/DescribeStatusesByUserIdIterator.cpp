...
namespace Gs2::Grade::Domain::Iterator
{
    ...
    FDescribeStatusesByUserIdIterator::FIterator& FDescribeStatusesByUserIdIterator::FIterator::operator++()
    {
        ...
        {
            const auto ListParentKey = Gs2::Grade::Model::Cache::FStatusCache::CreateCacheParentKey(
                Self->NamespaceName,
                Self->UserId,
                TOptional<int32>()
            );
            ...
            Range = R->GetItems().IsValid() ? R->GetItems() : MakeShared<TArray<Gs2::Grade::Model::FStatusPtr>>();
            ...
            if (Range.IsValid())
            {
                for (const auto& Item : *Range)
                {
                    ...
                    Gs2::Grade::Model::Cache::FStatusCache::Put(
                        Self->Gs2->Cache,
                        Request->GetNamespaceName(), Request->GetUserId(), Item->GetGradeName(), Item->GetPropertyId(),
