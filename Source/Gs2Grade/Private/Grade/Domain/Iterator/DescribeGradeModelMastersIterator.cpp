    ...
    FDescribeGradeModelMastersIterator::FIterator& FDescribeGradeModelMastersIterator::FIterator::operator++()
    {
        ...
        if (!RangeIteratorOpt || (!*RangeIteratorOpt && !bLast))
        {
            ...
            if (!RangeIteratorOpt)
            {
                ...
                if (Range)
                {
                    Range->RemoveAll([this](const Gs2::Grade::Model::FGradeModelMasterPtr& Item) { return Self->NamePrefix && !Item->GetName()->StartsWith(*Self->NamePrefix); });
                    bLast = true;
            ...
            if (Range)
            {
                Range->RemoveAll([this](const Gs2::Grade::Model::FGradeModelMasterPtr& Item) { return Self->NamePrefix.IsSet() && !Item->GetName()->StartsWith(*Self->NamePrefix); });
            }
            RangeIteratorOpt = Range->CreateIterator();
