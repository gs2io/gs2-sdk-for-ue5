...
namespace Gs2::Grade::Domain::SpeculativeExecutor
{
    ...
    )
    {
        ...
        if (FAddGradeByUserIdSpeculativeExecutor::Action() == NewAcquireAction->GetAction()) {
            ...
            auto Request = Request::FAddGradeByUserIdRequest::FromJson(RequestModelJson);
            Request = FAddGradeByUserIdSpeculativeExecutor::Rate(Request, Rate);
            auto Future = FAddGradeByUserIdSpeculativeExecutor::Execute(
                Domain,
                Service,
                AccessToken,
                Request
        ...
        if (FApplyRankCapByUserIdSpeculativeExecutor::Action() == NewAcquireAction->GetAction()) {
            TSharedPtr<FJsonObject> RequestModelJson;
            if (const TSharedRef<TJsonReader<>> JsonReader = TJsonReaderFactory<>::Create(NewAcquireAction->GetRequest().IsSet() ? *NewAcquireAction->GetRequest() : "{}");
                !FJsonSerializer::Deserialize(JsonReader, RequestModelJson))
            {
            ...
            auto Request = Request::FApplyRankCapByUserIdRequest::FromJson(RequestModelJson);
            Request = FApplyRankCapByUserIdSpeculativeExecutor::Rate(Request, Rate);
            auto Future = FApplyRankCapByUserIdSpeculativeExecutor::Execute(
                Domain,
                Service,
                AccessToken,
                Request
