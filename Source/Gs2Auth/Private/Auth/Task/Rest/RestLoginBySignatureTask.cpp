...
#include "Auth/Task/Rest/LoginBySignatureTask.h"
...
{
    FLoginBySignatureTask::FLoginBySignatureTask(
    ...
    Core::Model::FGs2ErrorPtr FLoginBySignatureTask::Action(
        const TSharedPtr<Result::FLoginBySignatureResultPtr> Result
    )
    {
        ...
        FString ResponseBody;
        {
            ...
            auto Url = Core::FGs2Constant::EndpointHost
                .Replace(TEXT("{service}"), TEXT("auth"))
            ...
            if (this->Request->GetBody().IsSet() && !this->Request->GetBody().GetValue().IsEmpty())
            {
                JsonRootObject->SetStringField("body", this->Request->GetBody().GetValue());
            }
            if (this->Request->GetSignature().IsSet() && !this->Request->GetSignature().GetValue().IsEmpty())
            {
        ...
        if (ResponseCode / 100 == 2)
        {
                ...
                FJsonSerializer::Deserialize(JsonReader, JsonRootObject))
            {
                ...
                *Result = Result::FLoginBySignatureResult::FromJson(JsonRootObject);
                return nullptr;
            }
            const auto Details = MakeShared<TArray<TSharedPtr<Core::Model::FGs2ErrorDetail>>>();
