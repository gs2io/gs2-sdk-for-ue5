...
namespace Gs2::Distributor::Domain::Iterator
{
    ...
    FDescribeDistributorModelsIterator::FIterator& FDescribeDistributorModelsIterator::FIterator::operator++()
    {
        ...
        {
            const auto ListParentKey = Gs2::Distributor::Model::Cache::FDistributorModelCache::CreateCacheParentKey(
                Self->NamespaceName,
                TOptional<int32>()
            );
            if (!RangeIteratorOpt)
            {
                Range = Self->Gs2->Cache->TryGetList<Gs2::Distributor::Model::FDistributorModel>(ListParentKey);
            ...
            ;
            const auto Future = Self->Client->DescribeDistributorModels(Request);
            ...
            Range = R->GetItems().IsValid() ? R->GetItems() : MakeShared<TArray<Gs2::Distributor::Model::FDistributorModelPtr>>();
