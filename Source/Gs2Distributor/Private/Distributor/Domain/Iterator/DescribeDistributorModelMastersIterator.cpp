...
namespace Gs2::Distributor::Domain::Iterator
{
    ...
    FDescribeDistributorModelMastersIterator::FIterator& FDescribeDistributorModelMastersIterator::FIterator::operator++()
    {
        ...
        {
            const auto ListParentKey = Gs2::Distributor::Model::Cache::FDistributorModelMasterCache::CreateCacheParentKey(
                Self->NamespaceName,
                TOptional<int32>()
            );
            if (!RangeIteratorOpt)
            {
                ...
                if (Range)
                {
                    Range->RemoveAll([this](const Gs2::Distributor::Model::FDistributorModelMasterPtr& Item) { return Self->NamePrefix && !Item->GetName()->StartsWith(*Self->NamePrefix); });
                    bLast = true;
            ...
            const auto Request =
                MakeShared<Gs2::Distributor::Request::FDescribeDistributorModelMastersRequest>()
                    ...
                    ->WithLimit(FetchSize)
            ;
            const auto Future = Self->Client->DescribeDistributorModelMasters(Request);
            ...
            Range = R->GetItems().IsValid() ? R->GetItems() : MakeShared<TArray<Gs2::Distributor::Model::FDistributorModelMasterPtr>>();
            ...
            if (Range)
            {
                Range->RemoveAll([this](const Gs2::Distributor::Model::FDistributorModelMasterPtr& Item) { return Self->NamePrefix.IsSet() && !Item->GetName()->StartsWith(*Self->NamePrefix); });
            }
            RangeIteratorOpt = Range->CreateIterator();
