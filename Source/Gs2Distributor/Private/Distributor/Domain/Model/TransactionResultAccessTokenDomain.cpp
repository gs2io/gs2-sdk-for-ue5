...
namespace Gs2::Distributor::Domain::Model
{
    ...
    {
        const FString CacheKey = Gs2::Distributor::Domain::Model::FTransactionResultDomain::CreateCacheKey(
            Self->TransactionId
        );
            ...
            [this, Result, CacheKey]() -> Gs2::Core::Model::FGs2ErrorPtr
            {
                ...
                if (!bCacheHit) {
                    ...
                    if (Future->GetTask().IsError())
                    {
                        ...
                        if (!Error->GetErrors().IsValid() || Error->Count() == 0 || !Error->Detail(0).IsValid() || Error->Detail(0)->GetComponent() != "transactionResult")
                        {
                            return Error;
                        }
                    }
    ...
    FTransactionResultAccessTokenDomain::FModelNoCacheTask::FModelNoCacheTask(
        const TSharedPtr<FTransactionResultAccessTokenDomain> Self
    ): Self(Self)
    {
    ...
    Gs2::Core::Model::FGs2ErrorPtr FTransactionResultAccessTokenDomain::FModelNoCacheTask::Action(
        TSharedPtr<TSharedPtr<Gs2::Distributor::Model::FTransactionResult>> Result
    )
    {
        // ReSharper disable once CppLocalVariableMayBeConst
    ...
    TSharedPtr<FAsyncTask<FTransactionResultAccessTokenDomain::FModelNoCacheTask>> FTransactionResultAccessTokenDomain::ModelNoCache() {
        return Gs2::Core::Util::New<FAsyncTask<FTransactionResultAccessTokenDomain::FModelNoCacheTask>>(this->AsShared());
    }
    ...
    )
    {
        ...
        return Gs2->Cache->Subscribe(
            ...
            [WeakGs2, WeakService, RegisteredParentKey, QueryNamespaceName, QueryTransactionId, SourceToken, RegisteredUserId]()
            {
                ...
                const auto Domain = MakeShared<FTransactionResultAccessTokenDomain>(
                    Owner,
                    WeakService.Pin(),
                    QueryNamespaceName,
                    TokenSnapshot,
