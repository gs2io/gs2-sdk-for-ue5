...
#include "Distributor/Task/Rest/RunTransactionTask.h"
...
{
    FRunTransactionTask::FRunTransactionTask(
    ...
    Core::Model::FGs2ErrorPtr FRunTransactionTask::Action(
        const TSharedPtr<Result::FRunTransactionResultPtr> Result
    )
    {
        ...
        FString ResponseBody;
        {
            ...
            auto Url = Core::FGs2Constant::EndpointHost
                ...
                .Append("/system/{ownerId}/{namespaceName}/user/{userId}/transaction/run");

            Url = Url.Replace(
                TEXT("{ownerId}"),
                !this->Request->GetOwnerId().IsSet() || this->Request->GetOwnerId().GetValue().Len() == 0 ?
                    TEXT("null") : ToCStr(*this->Request->GetOwnerId())
            );
            Url = Url.Replace(
                TEXT("{namespaceName}"),
            ...
            if (this->Request->GetTransaction().IsSet())
            {
                JsonRootObject->SetStringField(TEXT("transaction"), this->Request->GetTransaction().GetValue());
            }
        ...
        if (ResponseCode / 100 == 2)
        {
                ...
                FJsonSerializer::Deserialize(JsonReader, JsonRootObject))
            {
                ...
                *Result = Result::FRunTransactionResult::FromJson(JsonRootObject);
                return nullptr;
            }
            const auto Details = MakeShared<TArray<TSharedPtr<Core::Model::FGs2ErrorDetail>>>();
