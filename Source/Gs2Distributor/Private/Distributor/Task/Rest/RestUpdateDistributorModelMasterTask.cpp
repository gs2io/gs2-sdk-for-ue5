...
#include "Distributor/Task/Rest/UpdateDistributorModelMasterTask.h"
...
{
    FUpdateDistributorModelMasterTask::FUpdateDistributorModelMasterTask(
    ...
    Core::Model::FGs2ErrorPtr FUpdateDistributorModelMasterTask::Action(
        const TSharedPtr<Result::FUpdateDistributorModelMasterResultPtr> Result
    )
    {
        ...
        FString ResponseBody;
        {
            ...
            auto Url = Core::FGs2Constant::EndpointHost
                ...
                .Append("/{namespaceName}/master/distributor/{distributorName}");
            ...
            Url = Url.Replace(
                ...
                !this->Request->GetDistributorName().IsSet() || this->Request->GetDistributorName().GetValue().Len() == 0 ?
                    TEXT("null") : ToCStr(*this->Request->GetDistributorName())
            );
            ...
            if (this->Request->GetInboxNamespaceId().IsSet() && !this->Request->GetInboxNamespaceId().GetValue().IsEmpty())
            {
                JsonRootObject->SetStringField("inboxNamespaceId", this->Request->GetInboxNamespaceId().GetValue());
            }
            if (this->Request->GetWhiteListTargetIds() != nullptr && this->Request->GetWhiteListTargetIds().IsValid())
            {
                TArray<TSharedPtr<FJsonValue>> v;
                for (auto JsonObjectValue : *this->Request->GetWhiteListTargetIds())
                {
        ...
        if (ResponseCode / 100 == 2)
        {
                ...
                FJsonSerializer::Deserialize(JsonReader, JsonRootObject))
            {
                ...
                *Result = Result::FUpdateDistributorModelMasterResult::FromJson(JsonRootObject);
                return nullptr;
            }
            const auto Details = MakeShared<TArray<TSharedPtr<Core::Model::FGs2ErrorDetail>>>();
