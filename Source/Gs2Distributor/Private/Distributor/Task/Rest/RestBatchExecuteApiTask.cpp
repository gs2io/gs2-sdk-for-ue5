...
#include "Distributor/Task/Rest/BatchExecuteApiTask.h"
...
{
    FBatchExecuteApiTask::FBatchExecuteApiTask(
    ...
    Core::Model::FGs2ErrorPtr FBatchExecuteApiTask::Action(
        const TSharedPtr<Result::FBatchExecuteApiResultPtr> Result
    )
    {
        ...
        FString ResponseBody;
        {
            ...
            if (this->Request->GetRequestPayloads() != nullptr && this->Request->GetRequestPayloads().IsValid())
            {
                TArray<TSharedPtr<FJsonValue>> v;
                for (auto JsonObjectValue : *this->Request->GetRequestPayloads())
                {
        ...
        if (ResponseCode / 100 == 2)
        {
                ...
                FJsonSerializer::Deserialize(JsonReader, JsonRootObject))
            {
                ...
                *Result = Result::FBatchExecuteApiResult::FromJson(JsonRootObject);
                return nullptr;
            }
            const auto Details = MakeShared<TArray<TSharedPtr<Core::Model::FGs2ErrorDetail>>>();
