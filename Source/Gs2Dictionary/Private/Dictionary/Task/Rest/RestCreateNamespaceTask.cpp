...
#include "Dictionary/Task/Rest/CreateNamespaceTask.h"
...
namespace Gs2::Dictionary::Task::Rest
{
    ...
    )
    {
        ...
        FString ResponseBody;
        {
            ...
            if (this->Request->GetEntryScript() != nullptr && this->Request->GetEntryScript().IsValid())
            {
                JsonRootObject->SetObjectField(TEXT("entryScript"), this->Request->GetEntryScript()->ToJson());
            }
            if (this->Request->GetDuplicateEntryScript().IsSet())
            {
                JsonRootObject->SetStringField(TEXT("duplicateEntryScript"), this->Request->GetDuplicateEntryScript().GetValue());
            }
