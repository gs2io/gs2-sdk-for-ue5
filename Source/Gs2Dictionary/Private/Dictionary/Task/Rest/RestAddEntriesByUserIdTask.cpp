...
#include "Dictionary/Task/Rest/AddEntriesByUserIdTask.h"
...
{
    FAddEntriesByUserIdTask::FAddEntriesByUserIdTask(
    ...
    FAddEntriesByUserIdTask::FAddEntriesByUserIdTask(
        const FAddEntriesByUserIdTask& From
    ): TGs2Future(From), Session(From.Session), Request(From.Request)
    {
    }

    Core::Model::FGs2ErrorPtr FAddEntriesByUserIdTask::Action(
        const TSharedPtr<Result::FAddEntriesByUserIdResultPtr> Result
    )
    {
        ...
        FString ResponseBody;
        {
            ...
            auto Url = Core::FGs2Constant::EndpointHost
                ...
                .Append("/{namespaceName}/user/{userId}/entry");
            ...
            if (this->Request->GetEntryModelNames() != nullptr && this->Request->GetEntryModelNames().IsValid())
            {
                TArray<TSharedPtr<FJsonValue>> v;
                for (auto JsonObjectValue : *this->Request->GetEntryModelNames())
                {
        ...
        if (ResponseCode / 100 == 2)
        {
                ...
                FJsonSerializer::Deserialize(JsonReader, JsonRootObject))
            {
                ...
                *Result = Result::FAddEntriesByUserIdResult::FromJson(JsonRootObject);
                return nullptr;
            }
            const auto Details = MakeShared<TArray<TSharedPtr<Core::Model::FGs2ErrorDetail>>>();
