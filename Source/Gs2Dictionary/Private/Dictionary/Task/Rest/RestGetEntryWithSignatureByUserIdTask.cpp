...
#include "Dictionary/Task/Rest/GetEntryWithSignatureByUserIdTask.h"
...
{
    FGetEntryWithSignatureByUserIdTask::FGetEntryWithSignatureByUserIdTask(
    ...
    Core::Model::FGs2ErrorPtr FGetEntryWithSignatureByUserIdTask::Action(
        const TSharedPtr<Result::FGetEntryWithSignatureByUserIdResultPtr> Result
    )
    {
        ...
        FString ResponseBody;
        {
            ...
            auto Url = Core::FGs2Constant::EndpointHost
                .Replace(TEXT("{service}"), TEXT("dictionary"))
                .Replace(TEXT("{region}"), *this->Session->RegionName())
                .Append("/{namespaceName}/user/{userId}/entry/{entryModelName}/signature");
            ...
            Url = Url.Replace(
                TEXT("{entryModelName}"),
                !this->Request->GetEntryModelName().IsSet() || this->Request->GetEntryModelName().GetValue().Len() == 0 ?
                    TEXT("null") : ToCStr(*this->Request->GetEntryModelName())
            );
        ...
        if (ResponseCode / 100 == 2)
        {
                ...
                FJsonSerializer::Deserialize(JsonReader, JsonRootObject))
            {
                ...
                *Result = Result::FGetEntryWithSignatureByUserIdResult::FromJson(JsonRootObject);
                return nullptr;
            }
            const auto Details = MakeShared<TArray<TSharedPtr<Core::Model::FGs2ErrorDetail>>>();
