...
#include "Guild/Task/Rest/BatchUpdateMemberRoleByGuildNameTask.h"
...
{
    FBatchUpdateMemberRoleByGuildNameTask::FBatchUpdateMemberRoleByGuildNameTask(
    ...
    FBatchUpdateMemberRoleByGuildNameTask::FBatchUpdateMemberRoleByGuildNameTask(
        const FBatchUpdateMemberRoleByGuildNameTask& From
    ): TGs2Future(From), Session(From.Session), Request(From.Request)
    {
    }

    Core::Model::FGs2ErrorPtr FBatchUpdateMemberRoleByGuildNameTask::Action(
        const TSharedPtr<Result::FBatchUpdateMemberRoleByGuildNameResultPtr> Result
    )
    {
        ...
        FString ResponseBody;
        {
            ...
            auto Url = Core::FGs2Constant::EndpointHost
                ...
                .Append("/{namespaceName}/guild/{guildModelName}/{guildName}/batch/member/role");
            ...
            if (this->Request->GetMembers() != nullptr && this->Request->GetMembers().IsValid())
            {
                TArray<TSharedPtr<FJsonValue>> v;
                for (auto JsonObjectValue : *this->Request->GetMembers())
                {
        ...
        if (ResponseCode / 100 == 2)
        {
                ...
                FJsonSerializer::Deserialize(JsonReader, JsonRootObject))
            {
                ...
                *Result = Result::FBatchUpdateMemberRoleByGuildNameResult::FromJson(JsonRootObject);
                return nullptr;
            }
            const auto Details = MakeShared<TArray<TSharedPtr<Core::Model::FGs2ErrorDetail>>>();
