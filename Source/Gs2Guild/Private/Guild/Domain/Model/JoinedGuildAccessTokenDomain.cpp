...
namespace Gs2::Guild::Domain::Model
{
    ...
    FJoinedGuildAccessTokenDomain::FUpdateMemberMetadataTask::FUpdateMemberMetadataTask(
        const TSharedPtr<FJoinedGuildAccessTokenDomain>& Self,
    ...
    Gs2::Core::Model::FGs2ErrorPtr FJoinedGuildAccessTokenDomain::FUpdateMemberMetadataTask::Action(
    ...
    )
    {
        Request
            ...
            ->WithGuildName(Self->GuildName)
            ->WithAccessToken(Self->AccessToken->GetToken());
        const auto Future = Self->Client->UpdateMemberMetadata(
            Request
        );
        Future->StartSynchronousTask();
        if (Future->GetTask().IsError())
        ...
        const auto ResultModel = Future->GetTask().Result();
        Future->EnsureCompletion();
        auto Domain = MakeShared<Gs2::Guild::Domain::Model::FGuildAccessTokenDomain>(
            Self->Gs2,
            Self->Service,
    ...
    TSharedPtr<FAsyncTask<FJoinedGuildAccessTokenDomain::FUpdateMemberMetadataTask>> FJoinedGuildAccessTokenDomain::UpdateMemberMetadata(
    ...
    {
        const FString CacheKey = Gs2::Guild::Domain::Model::FJoinedGuildDomain::CreateCacheKey(
            Self->GuildModelName,
            Self->GuildName
        );
            ...
            [this, Result, CacheKey]() -> Gs2::Core::Model::FGs2ErrorPtr
            {
                ...
                if (!bCacheHit) {
                    ...
                    if (Future->GetTask().IsError())
                    {
                        ...
                        if (!Error->GetErrors().IsValid() || Error->Count() == 0 || !Error->Detail(0).IsValid() || Error->Detail(0)->GetComponent() != "joinedGuild")
                        {
                            return Error;
                        }
                    }
    ...
    )
    {
        ...
        return Gs2->Cache->Subscribe(
            ...
            [WeakGs2, WeakService, RegisteredParentKey, QueryNamespaceName, QueryGuildModelName, QueryGuildName, SourceToken, RegisteredUserId]()
            {
                ...
                const auto Domain = MakeShared<FJoinedGuildAccessTokenDomain>(
                    Owner,
                    WeakService.Pin(),
                    QueryNamespaceName,
                    TokenSnapshot,
