...
namespace Gs2::Guild::Domain::Iterator
{
    ...
    FDescribeGuildModelsIterator::FIterator& FDescribeGuildModelsIterator::FIterator::operator++()
    {
        ...
        {
            const auto ListParentKey = Gs2::Guild::Model::Cache::FGuildModelCache::CreateCacheParentKey(
                Self->NamespaceName,
                TOptional<int32>()
            );
            if (!RangeIteratorOpt)
            ...
            ;
            const auto Future = Self->Client->DescribeGuildModels(Request);
            ...
            Range = R->GetItems().IsValid() ? R->GetItems() : MakeShared<TArray<Gs2::Guild::Model::FGuildModelPtr>>();
