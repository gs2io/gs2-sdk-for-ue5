...
namespace Gs2::Lottery::Domain::Iterator
{
    ...
    FDescribePrizeTableMastersIterator::FIterator& FDescribePrizeTableMastersIterator::FIterator::operator++()
    {
        ...
        {
            const auto ListParentKey = Gs2::Lottery::Model::Cache::FPrizeTableMasterCache::CreateCacheParentKey(
                Self->NamespaceName,
                TOptional<int32>()
            );
            if (!RangeIteratorOpt)
            {
                Range = Self->Gs2->Cache->TryGetList<Gs2::Lottery::Model::FPrizeTableMaster>(ListParentKey);

                if (Range)
                {
                    Range->RemoveAll([this](const Gs2::Lottery::Model::FPrizeTableMasterPtr& Item) { return Self->NamePrefix && !Item->GetName()->StartsWith(*Self->NamePrefix); });
                    bLast = true;
            ...
            const auto Request =
                MakeShared<Gs2::Lottery::Request::FDescribePrizeTableMastersRequest>()
                    ...
                    ->WithLimit(FetchSize)
            ;
            const auto Future = Self->Client->DescribePrizeTableMasters(Request);
            ...
            Range = R->GetItems().IsValid() ? R->GetItems() : MakeShared<TArray<Gs2::Lottery::Model::FPrizeTableMasterPtr>>();
            ...
            if (Range)
            {
                Range->RemoveAll([this](const Gs2::Lottery::Model::FPrizeTableMasterPtr& Item) { return Self->NamePrefix.IsSet() && !Item->GetName()->StartsWith(*Self->NamePrefix); });
            }
            RangeIteratorOpt = Range->CreateIterator();
