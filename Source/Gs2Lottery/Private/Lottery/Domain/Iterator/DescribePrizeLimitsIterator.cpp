...
namespace Gs2::Lottery::Domain::Iterator
{
    ...
    FDescribePrizeLimitsIterator::FIterator::FIterator(
    ...
    FDescribePrizeLimitsIterator::FIterator& FDescribePrizeLimitsIterator::FIterator::operator++()
    {
        ...
        {
            const auto ListParentKey = Gs2::Lottery::Model::Cache::FPrizeLimitCache::CreateCacheParentKey(
                Self->NamespaceName,
                Self->PrizeTableName,
                TOptional<int32>()
            );
            if (!RangeIteratorOpt)
            {
                Range = Self->Gs2->Cache->TryGetList<Gs2::Lottery::Model::FPrizeLimit>(ListParentKey);
            ...
            const auto Request =
                MakeShared<Gs2::Lottery::Request::FDescribePrizeLimitsRequest>()
                    ...
                    ->WithLimit(FetchSize)
            ;
            const auto Future = Self->Client->DescribePrizeLimits(Request);
            ...
            Range = R->GetItems().IsValid() ? R->GetItems() : MakeShared<TArray<Gs2::Lottery::Model::FPrizeLimitPtr>>();
            ...
            if (Range.IsValid())
            {
                for (const auto& Item : *Range)
                {
                    ...
                    Gs2::Lottery::Model::Cache::FPrizeLimitCache::Put(
                        Self->Gs2->Cache,
                        Request->GetNamespaceName(), Request->GetPrizeTableName(), Item->GetPrizeId(),
    ...
    FDescribePrizeLimitsIterator::FIterator FDescribePrizeLimitsIterator::OneBeforeBegin()
    {
        return FIterator::OneBeforeBeginOf(this->AsShared());
    }

    FDescribePrizeLimitsIterator::FIterator FDescribePrizeLimitsIterator::begin()
    {
        return FIterator::BeginOf(this->AsShared());
    }

    FDescribePrizeLimitsIterator::FIterator FDescribePrizeLimitsIterator::end()
    {
