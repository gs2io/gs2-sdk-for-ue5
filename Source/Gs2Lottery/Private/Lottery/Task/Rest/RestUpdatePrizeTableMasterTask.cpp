...
#include "Lottery/Task/Rest/UpdatePrizeTableMasterTask.h"
...
{
    FUpdatePrizeTableMasterTask::FUpdatePrizeTableMasterTask(
    ...
    FUpdatePrizeTableMasterTask::FUpdatePrizeTableMasterTask(
        const FUpdatePrizeTableMasterTask& From
    ): TGs2Future(From), Session(From.Session), Request(From.Request)
    {
    }

    Core::Model::FGs2ErrorPtr FUpdatePrizeTableMasterTask::Action(
        const TSharedPtr<Result::FUpdatePrizeTableMasterResultPtr> Result
    )
    {
        ...
        FString ResponseBody;
        {
            ...
            auto Url = Core::FGs2Constant::EndpointHost
                ...
                .Append("/{namespaceName}/master/table/{prizeTableName}");
            ...
            Url = Url.Replace(
                TEXT("{prizeTableName}"),
                !this->Request->GetPrizeTableName().IsSet() || this->Request->GetPrizeTableName().GetValue().Len() == 0 ?
                    TEXT("null") : ToCStr(*this->Request->GetPrizeTableName())
            );
            ...
            if (this->Request->GetPrizes() != nullptr && this->Request->GetPrizes().IsValid())
            {
                TArray<TSharedPtr<FJsonValue>> v;
                for (auto JsonObjectValue : *this->Request->GetPrizes())
                {
        ...
        if (ResponseCode / 100 == 2)
        {
                ...
                FJsonSerializer::Deserialize(JsonReader, JsonRootObject))
            {
                ...
                *Result = Result::FUpdatePrizeTableMasterResult::FromJson(JsonRootObject);
                return nullptr;
            }
            const auto Details = MakeShared<TArray<TSharedPtr<Core::Model::FGs2ErrorDetail>>>();
