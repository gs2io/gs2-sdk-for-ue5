...
#include "Realtime/Task/Rest/WantRoomTask.h"
...
namespace Gs2::Realtime::Task::Rest
{
    ...
    Core::Model::FGs2ErrorPtr FWantRoomTask::Action(
        const TSharedPtr<Result::FWantRoomResultPtr> Result
    )
    {
        ...
        FString ResponseBody;
        {
            ...
            auto Url = Core::FGs2Constant::EndpointHost
                .Replace(TEXT("{service}"), TEXT("realtime"))
                .Replace(TEXT("{region}"), *this->Session->RegionName())
                .Append("/{namespaceName}/room");
            ...
            if (this->Request->GetNotificationUserIds() != nullptr && this->Request->GetNotificationUserIds().IsValid())
            {
                TArray<TSharedPtr<FJsonValue>> v;
                for (auto JsonObjectValue : *this->Request->GetNotificationUserIds())
                {
        ...
        if (ResponseCode / 100 == 2)
        {
                ...
                FJsonSerializer::Deserialize(JsonReader, JsonRootObject))
            {
                ...
                *Result = Result::FWantRoomResult::FromJson(JsonRootObject);
                return nullptr;
            }
            const auto Details = MakeShared<TArray<TSharedPtr<Core::Model::FGs2ErrorDetail>>>();
