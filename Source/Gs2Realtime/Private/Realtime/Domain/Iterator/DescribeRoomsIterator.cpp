...
namespace Gs2::Realtime::Domain::Iterator
{
    ...
    FDescribeRoomsIterator::FIterator& FDescribeRoomsIterator::FIterator::operator++()
    {
        ...
        {
            const auto ListParentKey = Gs2::Realtime::Model::Cache::FRoomCache::CreateCacheParentKey(
                Self->NamespaceName,
                TOptional<int32>()
            );
            if (!RangeIteratorOpt)
            {
                Range = Self->Gs2->Cache->TryGetList<Gs2::Realtime::Model::FRoom>(ListParentKey);
            ...
            Range = R->GetItems().IsValid() ? R->GetItems() : MakeShared<TArray<Gs2::Realtime::Model::FRoomPtr>>();
