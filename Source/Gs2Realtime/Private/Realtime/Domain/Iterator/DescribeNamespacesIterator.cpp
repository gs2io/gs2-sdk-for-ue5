...
namespace Gs2::Realtime::Domain::Iterator
{
    ...
    Gs2::Core::Model::FGs2ErrorPtr FDescribeNamespacesIterator::FIteratorNextTask::Action(TSharedPtr<TSharedPtr<Gs2::Realtime::Model::FNamespace>> Result)
    {
        ++Iterator;
    ...
    FDescribeNamespacesIterator::FIterator& FDescribeNamespacesIterator::FIterator::operator++()
    {
        ...
        {
            const auto ListParentKey = Gs2::Realtime::Model::Cache::FNamespaceCache::CreateCacheParentKey(
                TOptional<int32>()
            );
            if (!RangeIteratorOpt)
            {
                Range = Self->Gs2->Cache->TryGetList<Gs2::Realtime::Model::FNamespace>(ListParentKey);

                if (Range)
                {
                    Range->RemoveAll([this](const Gs2::Realtime::Model::FNamespacePtr& Item) { return Self->NamePrefix && !Item->GetName()->StartsWith(*Self->NamePrefix); });
                    bLast = true;
            ...
            const auto Request =
                MakeShared<Gs2::Realtime::Request::FDescribeNamespacesRequest>()
            ...
            Range = R->GetItems().IsValid() ? R->GetItems() : MakeShared<TArray<Gs2::Realtime::Model::FNamespacePtr>>();
            ...
            if (Range)
            {
                Range->RemoveAll([this](const Gs2::Realtime::Model::FNamespacePtr& Item) { return Self->NamePrefix.IsSet() && !Item->GetName()->StartsWith(*Self->NamePrefix); });
            }
            RangeIteratorOpt = Range->CreateIterator();
