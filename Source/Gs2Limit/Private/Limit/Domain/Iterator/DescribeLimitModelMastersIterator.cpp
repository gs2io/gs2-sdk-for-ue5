...
namespace Gs2::Limit::Domain::Iterator
{
    ...
    FDescribeLimitModelMastersIterator::FDescribeLimitModelMastersIterator(
        const FDescribeLimitModelMastersIterator& From
    ):
        Gs2(From.Gs2),
        Client(From.Client),
    ...
    FDescribeLimitModelMastersIterator::FIterator& FDescribeLimitModelMastersIterator::FIterator::operator++()
    {
        ...
        {
            const auto ListParentKey = Gs2::Limit::Model::Cache::FLimitModelMasterCache::CreateCacheParentKey(
                Self->NamespaceName,
                TOptional<int32>()
            );
            if (!RangeIteratorOpt)
            {
                Range = Self->Gs2->Cache->TryGetList<Gs2::Limit::Model::FLimitModelMaster>(ListParentKey);

                if (Range)
                {
                    Range->RemoveAll([this](const Gs2::Limit::Model::FLimitModelMasterPtr& Item) { return Self->NamePrefix && !Item->GetName()->StartsWith(*Self->NamePrefix); });
                    bLast = true;
            ...
            const auto Request =
                MakeShared<Gs2::Limit::Request::FDescribeLimitModelMastersRequest>()
                    ...
                    ->WithLimit(FetchSize)
            ;
            const auto Future = Self->Client->DescribeLimitModelMasters(Request);
            ...
            Range = R->GetItems().IsValid() ? R->GetItems() : MakeShared<TArray<Gs2::Limit::Model::FLimitModelMasterPtr>>();
            ...
            if (Range)
            {
                Range->RemoveAll([this](const Gs2::Limit::Model::FLimitModelMasterPtr& Item) { return Self->NamePrefix.IsSet() && !Item->GetName()->StartsWith(*Self->NamePrefix); });
            }
            RangeIteratorOpt = Range->CreateIterator();
