...
namespace Gs2::Limit::Domain::SpeculativeExecutor
{
    ...
    )
    {
        ...
        if (FCountDownByUserIdSpeculativeExecutor::Action() == NewAcquireAction->GetAction()) {
            ...
            auto Request = Request::FCountDownByUserIdRequest::FromJson(RequestModelJson);
            const int64 Count = Request->GetCountDownValue().IsSet() ? static_cast<int64>(*Request->GetCountDownValue()) : 1;
            if (Count == 0)
            {
                Request->WithCountDownValue(0);
            }
            else
            {
                ...
                Request->WithCountDownValue(static_cast<int32>(Scaled));
            }
            auto Future = FCountDownByUserIdSpeculativeExecutor::Execute(
                Domain,
                Service,
                AccessToken,
                Request
        ...
        if (FDeleteCounterByUserIdSpeculativeExecutor::Action() == NewAcquireAction->GetAction()) {
            TSharedPtr<FJsonObject> RequestModelJson;
            if (const TSharedRef<TJsonReader<>> JsonReader = TJsonReaderFactory<>::Create(NewAcquireAction->GetRequest().IsSet() ? *NewAcquireAction->GetRequest() : "{}");
                !FJsonSerializer::Deserialize(JsonReader, RequestModelJson))
            {
            ...
            auto Request = Request::FDeleteCounterByUserIdRequest::FromJson(RequestModelJson);
            Request = FDeleteCounterByUserIdSpeculativeExecutor::Rate(Request, Rate);
            auto Future = FDeleteCounterByUserIdSpeculativeExecutor::Execute(
                Domain,
                Service,
                AccessToken,
                Request
