...
namespace Gs2::Limit::Domain::SpeculativeExecutor
{
    ...
    )
    {
        *Result = nullptr;
        ...
        if (FVerifyCounterByUserIdSpeculativeExecutor::Action() == NewVerifyAction->GetAction()) {
            ...
            auto Future = Inverse
                ? FVerifyCounterByUserIdSpeculativeExecutor::ExecuteInverse(
                    Domain,
                    Service,
                    AccessToken,
                    Request
                )
                : FVerifyCounterByUserIdSpeculativeExecutor::Execute(
                Domain,
                Service,
                AccessToken,
                Request
