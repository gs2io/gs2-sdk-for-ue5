...
namespace Gs2::UE5::Exchange::Model
{
    ...
    Gs2::Exchange::Model::FVerifyActionPtr FEzVerifyAction::ToModel() const
    {
        return MakeShared<Gs2::Exchange::Model::FVerifyAction>()
            ->WithAction(ActionValue)
            ->WithRequest(RequestValue);
    }

    TSharedPtr<FEzVerifyAction> FEzVerifyAction::FromModel(const Gs2::Exchange::Model::FVerifyActionPtr Model)
    {
        if (Model == nullptr)
        {
            return nullptr;
