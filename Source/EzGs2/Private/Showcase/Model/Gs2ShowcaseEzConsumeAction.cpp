...
#include "EzGs2/Public/Showcase/Model/Gs2ShowcaseEzConsumeAction.h"
...
namespace Gs2::UE5::Showcase::Model
{
    ...
    Gs2::Showcase::Model::FConsumeActionPtr FEzConsumeAction::ToModel() const
    {
        return MakeShared<Gs2::Showcase::Model::FConsumeAction>()
            ->WithAction(ActionValue)
            ->WithRequest(RequestValue);
    }

    TSharedPtr<FEzConsumeAction> FEzConsumeAction::FromModel(const Gs2::Showcase::Model::FConsumeActionPtr Model)
    {
        if (Model == nullptr)
        {
            return nullptr;
