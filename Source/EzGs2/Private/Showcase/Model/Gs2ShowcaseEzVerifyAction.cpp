...
namespace Gs2::UE5::Showcase::Model
{
    ...
    Gs2::Showcase::Model::FVerifyActionPtr FEzVerifyAction::ToModel() const
    {
        return MakeShared<Gs2::Showcase::Model::FVerifyAction>()
            ->WithAction(ActionValue)
            ->WithRequest(RequestValue);
    }

    TSharedPtr<FEzVerifyAction> FEzVerifyAction::FromModel(const Gs2::Showcase::Model::FVerifyActionPtr Model)
    {
        if (Model == nullptr)
        {
            return nullptr;
