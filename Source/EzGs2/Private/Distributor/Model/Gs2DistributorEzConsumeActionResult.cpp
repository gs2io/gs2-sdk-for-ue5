...
namespace Gs2::UE5::Distributor::Model
{
    ...
    Gs2::Distributor::Model::FConsumeActionResultPtr FEzConsumeActionResult::ToModel() const
    {
        return MakeShared<Gs2::Distributor::Model::FConsumeActionResult>()
            ->WithConsumeRequest(ConsumeRequestValue)
            ->WithStatusCode(StatusCodeValue)
            ->WithConsumeResult(ConsumeResultValue);
    }

    TSharedPtr<FEzConsumeActionResult> FEzConsumeActionResult::FromModel(const Gs2::Distributor::Model::FConsumeActionResultPtr Model)
    {
        if (Model == nullptr)
        {
            return nullptr;
