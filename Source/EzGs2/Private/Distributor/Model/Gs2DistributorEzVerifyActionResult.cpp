...
namespace Gs2::UE5::Distributor::Model
{
    ...
    Gs2::Distributor::Model::FVerifyActionResultPtr FEzVerifyActionResult::ToModel() const
    {
        return MakeShared<Gs2::Distributor::Model::FVerifyActionResult>()
            ->WithVerifyRequest(VerifyRequestValue)
            ->WithStatusCode(StatusCodeValue)
            ->WithVerifyResult(VerifyResultValue);
    }

    TSharedPtr<FEzVerifyActionResult> FEzVerifyActionResult::FromModel(const Gs2::Distributor::Model::FVerifyActionResultPtr Model)
    {
        if (Model == nullptr)
        {
            return nullptr;
