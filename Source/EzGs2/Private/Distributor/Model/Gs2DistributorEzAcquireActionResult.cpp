...
namespace Gs2::UE5::Distributor::Model
{
    ...
    Gs2::Distributor::Model::FAcquireActionResultPtr FEzAcquireActionResult::ToModel() const
    {
        return MakeShared<Gs2::Distributor::Model::FAcquireActionResult>()
            ->WithAcquireRequest(AcquireRequestValue)
            ->WithStatusCode(StatusCodeValue)
            ->WithAcquireResult(AcquireResultValue);
    }

    TSharedPtr<FEzAcquireActionResult> FEzAcquireActionResult::FromModel(const Gs2::Distributor::Model::FAcquireActionResultPtr Model)
    {
        if (Model == nullptr)
        {
            return nullptr;
