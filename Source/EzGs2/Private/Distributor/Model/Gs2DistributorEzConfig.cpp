...
namespace Gs2::UE5::Distributor::Model
{
    ...
    Gs2::Distributor::Model::FConfigPtr FEzConfig::ToModel() const
    {
        return MakeShared<Gs2::Distributor::Model::FConfig>()
            ->WithKey(KeyValue)
            ->WithValue(ValueValue);
    }

    TSharedPtr<FEzConfig> FEzConfig::FromModel(const Gs2::Distributor::Model::FConfigPtr Model)
    {
        if (Model == nullptr)
        {
            return nullptr;
