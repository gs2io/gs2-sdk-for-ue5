...
namespace Gs2::UE5::Mission::Model
{
    ...
    Gs2::Mission::Model::FConsumeActionPtr FEzConsumeAction::ToModel() const
    {
        return MakeShared<Gs2::Mission::Model::FConsumeAction>()
            ->WithAction(ActionValue)
            ->WithRequest(RequestValue);
    }

    TSharedPtr<FEzConsumeAction> FEzConsumeAction::FromModel(const Gs2::Mission::Model::FConsumeActionPtr Model)
    {
        if (Model == nullptr)
        {
            return nullptr;
