...
namespace Gs2::UE5::Mission::Model
{
    ...
    Gs2::Mission::Model::FVerifyActionPtr FEzVerifyAction::ToModel() const
    {
        return MakeShared<Gs2::Mission::Model::FVerifyAction>()
            ->WithAction(ActionValue)
            ->WithRequest(RequestValue);
    }

    TSharedPtr<FEzVerifyAction> FEzVerifyAction::FromModel(const Gs2::Mission::Model::FVerifyActionPtr Model)
    {
        if (Model == nullptr)
        {
            return nullptr;
