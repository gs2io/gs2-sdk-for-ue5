...
namespace Gs2::UE5::Enhance::Model
{
    ...
    Gs2::Enhance::Model::FConfigPtr FEzConfig::ToModel() const
    {
        return MakeShared<Gs2::Enhance::Model::FConfig>()
            ->WithKey(KeyValue)
            ->WithValue(ValueValue);
    }

    TSharedPtr<FEzConfig> FEzConfig::FromModel(const Gs2::Enhance::Model::FConfigPtr Model)
    {
        if (Model == nullptr)
        {
            return nullptr;
