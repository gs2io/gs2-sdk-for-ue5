...
namespace Gs2::UE5::Inbox::Model
{
    ...
    Gs2::Inbox::Model::FConfigPtr FEzConfig::ToModel() const
    {
        return MakeShared<Gs2::Inbox::Model::FConfig>()
            ->WithKey(KeyValue)
            ->WithValue(ValueValue);
    }

    TSharedPtr<FEzConfig> FEzConfig::FromModel(const Gs2::Inbox::Model::FConfigPtr Model)
    {
        if (Model == nullptr)
        {
            return nullptr;
