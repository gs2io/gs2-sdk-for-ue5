...
namespace Gs2::UE5::Quest::Model
{
    ...
    Gs2::Quest::Model::FVerifyActionPtr FEzVerifyAction::ToModel() const
    {
        return MakeShared<Gs2::Quest::Model::FVerifyAction>()
            ->WithAction(ActionValue)
            ->WithRequest(RequestValue);
    }

    TSharedPtr<FEzVerifyAction> FEzVerifyAction::FromModel(const Gs2::Quest::Model::FVerifyActionPtr Model)
    {
        if (Model == nullptr)
        {
            return nullptr;
