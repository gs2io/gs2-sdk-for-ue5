...
namespace Gs2::UE5::LoginReward::Model
{
    ...
    Gs2::LoginReward::Model::FConfigPtr FEzConfig::ToModel() const
    {
        return MakeShared<Gs2::LoginReward::Model::FConfig>()
            ->WithKey(KeyValue)
            ->WithValue(ValueValue);
    }

    TSharedPtr<FEzConfig> FEzConfig::FromModel(const Gs2::LoginReward::Model::FConfigPtr Model)
    {
        if (Model == nullptr)
        {
            return nullptr;
