...
namespace Gs2::Matchmaking::Domain::SpeculativeExecutor
{
    ...
    )
    {
        ...
        if (FVerifyIncludeParticipantByUserIdSpeculativeExecutor::Action() == NewVerifyAction->GetAction()) {
            ...
            auto Future = Inverse
                ? FVerifyIncludeParticipantByUserIdSpeculativeExecutor::ExecuteInverse(
                    Domain,
                    Service,
                    AccessToken,
                    Request)
                : FVerifyIncludeParticipantByUserIdSpeculativeExecutor::Execute(
                Domain,
                Service,
                AccessToken,
                Request
