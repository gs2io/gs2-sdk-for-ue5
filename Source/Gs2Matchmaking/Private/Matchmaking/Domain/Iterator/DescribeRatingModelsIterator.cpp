...
namespace Gs2::Matchmaking::Domain::Iterator
{
    ...
    FDescribeRatingModelsIterator::FIterator& FDescribeRatingModelsIterator::FIterator::operator++()
    {
        ...
        {
            const auto ListParentKey = Gs2::Matchmaking::Model::Cache::FRatingModelCache::CreateCacheParentKey(
                Self->NamespaceName,
                TOptional<int32>()
            );
            if (!RangeIteratorOpt)
            ...
            ;
            const auto Future = Self->Client->DescribeRatingModels(Request);
            ...
            Range = R->GetItems().IsValid() ? R->GetItems() : MakeShared<TArray<Gs2::Matchmaking::Model::FRatingModelPtr>>();
