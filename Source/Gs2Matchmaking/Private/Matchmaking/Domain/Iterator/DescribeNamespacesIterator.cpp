...
namespace Gs2::Matchmaking::Domain::Iterator
{
    ...
    Gs2::Core::Model::FGs2ErrorPtr FDescribeNamespacesIterator::FIteratorNextTask::Action(TSharedPtr<TSharedPtr<Gs2::Matchmaking::Model::FNamespace>> Result)
    {
        ++Iterator;
    ...
    FDescribeNamespacesIterator::FIterator& FDescribeNamespacesIterator::FIterator::operator++()
    {
        ...
        {
            const auto ListParentKey = Gs2::Matchmaking::Model::Cache::FNamespaceCache::CreateCacheParentKey(
                TOptional<int32>()
            );
            if (!RangeIteratorOpt)
            {
                Range = Self->Gs2->Cache->TryGetList<Gs2::Matchmaking::Model::FNamespace>(ListParentKey);

                if (Range)
                {
                    Range->RemoveAll([this](const Gs2::Matchmaking::Model::FNamespacePtr& Item) { return Self->NamePrefix && !Item->GetName()->StartsWith(*Self->NamePrefix); });
                    bLast = true;
            ...
            const auto Request =
                MakeShared<Gs2::Matchmaking::Request::FDescribeNamespacesRequest>()
            ...
            Range = R->GetItems().IsValid() ? R->GetItems() : MakeShared<TArray<Gs2::Matchmaking::Model::FNamespacePtr>>();
            ...
            if (Range)
            {
                Range->RemoveAll([this](const Gs2::Matchmaking::Model::FNamespacePtr& Item) { return Self->NamePrefix.IsSet() && !Item->GetName()->StartsWith(*Self->NamePrefix); });
            }
            RangeIteratorOpt = Range->CreateIterator();
