...
namespace Gs2::Matchmaking::Domain::Iterator
{
    ...
    FDescribeSeasonModelsIterator::FIterator& FDescribeSeasonModelsIterator::FIterator::operator++()
    {
        ...
        {
            const auto ListParentKey = Gs2::Matchmaking::Model::Cache::FSeasonModelCache::CreateCacheParentKey(
                Self->NamespaceName,
                TOptional<int32>()
            );
            if (!RangeIteratorOpt)
            ...
            ;
            const auto Future = Self->Client->DescribeSeasonModels(Request);
            ...
            Range = R->GetItems().IsValid() ? R->GetItems() : MakeShared<TArray<Gs2::Matchmaking::Model::FSeasonModelPtr>>();
