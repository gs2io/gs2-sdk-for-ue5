...
#include "Matchmaking/Task/Rest/GetGatheringTask.h"
...
{
    FGetGatheringTask::FGetGatheringTask(
    ...
    FGetGatheringTask::FGetGatheringTask(
        const FGetGatheringTask& From
    ): TGs2Future(From), Session(From.Session), Request(From.Request)
    {
    }

    Core::Model::FGs2ErrorPtr FGetGatheringTask::Action(
        const TSharedPtr<Result::FGetGatheringResultPtr> Result
    )
    {
        ...
        if (ResponseCode / 100 == 2)
        {
                ...
                FJsonSerializer::Deserialize(JsonReader, JsonRootObject))
            {
                ...
                *Result = Result::FGetGatheringResult::FromJson(JsonRootObject);
                return nullptr;
            }
            const auto Details = MakeShared<TArray<TSharedPtr<Core::Model::FGs2ErrorDetail>>>();
