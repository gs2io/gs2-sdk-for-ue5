...
#include "Matchmaking/Task/Rest/UpdateRatingModelMasterTask.h"
...
{
    FUpdateRatingModelMasterTask::FUpdateRatingModelMasterTask(
    ...
    Core::Model::FGs2ErrorPtr FUpdateRatingModelMasterTask::Action(
        const TSharedPtr<Result::FUpdateRatingModelMasterResultPtr> Result
    )
    {
        ...
        FString ResponseBody;
        {
            ...
            auto Url = Core::FGs2Constant::EndpointHost
                ...
                .Append("/{namespaceName}/master/rating/{ratingName}");
            ...
            Url = Url.Replace(
                TEXT("{ratingName}"),
                !this->Request->GetRatingName().IsSet() || this->Request->GetRatingName().GetValue().Len() == 0 ?
                    TEXT("null") : ToCStr(*this->Request->GetRatingName())
            );
            ...
            {
                JsonRootObject->SetNumberField(TEXT("initialValue"), this->Request->GetInitialValue().GetValue());
            }
            if (this->Request->GetVolatility().IsSet())
            {
                JsonRootObject->SetNumberField(TEXT("volatility"), this->Request->GetVolatility().GetValue());
            }
        ...
        if (ResponseCode / 100 == 2)
        {
                ...
                FJsonSerializer::Deserialize(JsonReader, JsonRootObject))
            {
                ...
                *Result = Result::FUpdateRatingModelMasterResult::FromJson(JsonRootObject);
                return nullptr;
            }
            const auto Details = MakeShared<TArray<TSharedPtr<Core::Model::FGs2ErrorDetail>>>();
