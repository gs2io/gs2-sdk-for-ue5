...
#include "Key/Task/Rest/GetKeyTask.h"
...
namespace Gs2::Key::Task::Rest
{
    ...
    FGetKeyTask::FGetKeyTask(
        const FGetKeyTask& From
    ): TGs2Future(From), Session(From.Session), Request(From.Request)
    {
    }

    Core::Model::FGs2ErrorPtr FGetKeyTask::Action(
        const TSharedPtr<Result::FGetKeyResultPtr> Result
    )
    {
        ...
        if (ResponseCode / 100 == 2)
        {
                ...
                FJsonSerializer::Deserialize(JsonReader, JsonRootObject))
            {
                ...
                *Result = Result::FGetKeyResult::FromJson(JsonRootObject);
                return nullptr;
            }
            const auto Details = MakeShared<TArray<TSharedPtr<Core::Model::FGs2ErrorDetail>>>();
