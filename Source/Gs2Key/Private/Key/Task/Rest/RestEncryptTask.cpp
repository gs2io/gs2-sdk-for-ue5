...
#include "Key/Task/Rest/EncryptTask.h"
...
namespace Gs2::Key::Task::Rest
{
    ...
    Core::Model::FGs2ErrorPtr FEncryptTask::Action(
        const TSharedPtr<Result::FEncryptResultPtr> Result
    )
    {
        ...
        FString ResponseBody;
        {
            ...
            auto Url = Core::FGs2Constant::EndpointHost
                ...
                .Append("/{namespaceName}/key/{keyName}/encrypt");
            ...
            Url = Url.Replace(
                TEXT("{keyName}"),
                !this->Request->GetKeyName().IsSet() || this->Request->GetKeyName().GetValue().Len() == 0 ?
                    TEXT("null") : ToCStr(*this->Request->GetKeyName())
            );
            ...
            if (this->Request->GetData().IsSet() && !this->Request->GetData().GetValue().IsEmpty())
            {
                JsonRootObject->SetStringField("data", this->Request->GetData().GetValue());
            }
        ...
        if (ResponseCode / 100 == 2)
        {
                ...
                FJsonSerializer::Deserialize(JsonReader, JsonRootObject))
            {
                ...
                *Result = Result::FEncryptResult::FromJson(JsonRootObject);
                return nullptr;
            }
            const auto Details = MakeShared<TArray<TSharedPtr<Core::Model::FGs2ErrorDetail>>>();
