...
namespace Gs2::Key::Domain::Iterator
{
    ...
    FDescribeKeysIterator::FIterator& FDescribeKeysIterator::FIterator::operator++()
    {
        ...
        {
            const auto ListParentKey = Gs2::Key::Model::Cache::FKeyCache::CreateCacheParentKey(
                Self->NamespaceName,
                TOptional<int32>()
            );
            if (!RangeIteratorOpt)
            {
                ...
                if (Range)
                {
                    Range->RemoveAll([this](const Gs2::Key::Model::FKeyPtr& Item) { return Self->NamePrefix && !Item->GetName()->StartsWith(*Self->NamePrefix); });
                    bLast = true;
            ...
            const auto Request =
                MakeShared<Gs2::Key::Request::FDescribeKeysRequest>()
                    ...
                    ->WithLimit(FetchSize)
            ;
            const auto Future = Self->Client->DescribeKeys(Request);
            ...
            Range = R->GetItems().IsValid() ? R->GetItems() : MakeShared<TArray<Gs2::Key::Model::FKeyPtr>>();
            ...
            if (Range)
            {
                Range->RemoveAll([this](const Gs2::Key::Model::FKeyPtr& Item) { return Self->NamePrefix.IsSet() && !Item->GetName()->StartsWith(*Self->NamePrefix); });
            }
            RangeIteratorOpt = Range->CreateIterator();
