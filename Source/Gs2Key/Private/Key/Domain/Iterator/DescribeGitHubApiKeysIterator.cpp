...
namespace Gs2::Key::Domain::Iterator
{
    ...
    FDescribeGitHubApiKeysIterator::FIterator& FDescribeGitHubApiKeysIterator::FIterator::operator++()
    {
        ...
        {
            const auto ListParentKey = Gs2::Key::Model::Cache::FGitHubApiKeyCache::CreateCacheParentKey(
                Self->NamespaceName,
                TOptional<int32>()
            );
            if (!RangeIteratorOpt)
            {
                Range = Self->Gs2->Cache->TryGetList<Gs2::Key::Model::FGitHubApiKey>(ListParentKey);
            ...
            const auto Request =
                MakeShared<Gs2::Key::Request::FDescribeGitHubApiKeysRequest>()
                    ...
                    ->WithLimit(FetchSize)
            ;
            const auto Future = Self->Client->DescribeGitHubApiKeys(Request);
            ...
            Range = R->GetItems().IsValid() ? R->GetItems() : MakeShared<TArray<Gs2::Key::Model::FGitHubApiKeyPtr>>();
