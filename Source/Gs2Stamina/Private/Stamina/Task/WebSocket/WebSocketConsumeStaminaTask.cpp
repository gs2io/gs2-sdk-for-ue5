...
#include "Stamina/Task/WebSocket/ConsumeStaminaTask.h"
...
{
    FConsumeStaminaTask::FConsumeStaminaTask(
    ...
    Core::Model::FGs2ErrorPtr FConsumeStaminaTask::Action(
        const TSharedPtr<Result::FConsumeStaminaResultPtr> Result
    )
    {
        ...
        UE_LOG(Gs2Log, VeryVerbose, TEXT("[%s:%s:%s] %s"), TEXT("stamina"), TEXT("stamina"), TEXT("consumeStamina"), ToCStr(RequestPayload->Payload()));
        ...
        *Result = Result::FConsumeStaminaResult::FromJson(WebSocketResult->Body());
