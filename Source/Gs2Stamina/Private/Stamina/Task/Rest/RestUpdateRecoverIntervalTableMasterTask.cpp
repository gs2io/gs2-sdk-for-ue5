...
#include "Stamina/Task/Rest/UpdateRecoverIntervalTableMasterTask.h"
...
{
    FUpdateRecoverIntervalTableMasterTask::FUpdateRecoverIntervalTableMasterTask(
    ...
    FUpdateRecoverIntervalTableMasterTask::FUpdateRecoverIntervalTableMasterTask(
        const FUpdateRecoverIntervalTableMasterTask& From
    ): TGs2Future(From), Session(From.Session), Request(From.Request)
    {
    }

    Core::Model::FGs2ErrorPtr FUpdateRecoverIntervalTableMasterTask::Action(
        const TSharedPtr<Result::FUpdateRecoverIntervalTableMasterResultPtr> Result
    )
    {
        ...
        FString ResponseBody;
        {
            ...
            auto Url = Core::FGs2Constant::EndpointHost
                ...
                .Append("/{namespaceName}/master/recoverIntervalTable/{recoverIntervalTableName}");
            ...
            Url = Url.Replace(
                TEXT("{recoverIntervalTableName}"),
                !this->Request->GetRecoverIntervalTableName().IsSet() || this->Request->GetRecoverIntervalTableName().GetValue().Len() == 0 ?
                    TEXT("null") : ToCStr(*this->Request->GetRecoverIntervalTableName())
            );
            ...
            if (this->Request->GetValues() != nullptr && this->Request->GetValues().IsValid())
            {
                ...
                for (auto JsonObjectValue : *this->Request->GetValues())
                {
        ...
        if (ResponseCode / 100 == 2)
        {
                ...
                FJsonSerializer::Deserialize(JsonReader, JsonRootObject))
            {
                ...
                *Result = Result::FUpdateRecoverIntervalTableMasterResult::FromJson(JsonRootObject);
                return nullptr;
            }
            const auto Details = MakeShared<TArray<TSharedPtr<Core::Model::FGs2ErrorDetail>>>();
