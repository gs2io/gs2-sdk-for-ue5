...
namespace Gs2::Stamina::Domain::Iterator
{
    ...
    FDescribeMaxStaminaTableMastersIterator::FIterator& FDescribeMaxStaminaTableMastersIterator::FIterator::operator++()
    {
        ...
        {
            const auto ListParentKey = Gs2::Stamina::Model::Cache::FMaxStaminaTableMasterCache::CreateCacheParentKey(
                Self->NamespaceName,
                TOptional<int32>()
            );
            if (!RangeIteratorOpt)
            {
                Range = Self->Gs2->Cache->TryGetList<Gs2::Stamina::Model::FMaxStaminaTableMaster>(ListParentKey);
            ...
            const auto Request =
                MakeShared<Gs2::Stamina::Request::FDescribeMaxStaminaTableMastersRequest>()
                    ...
                    ->WithLimit(FetchSize)
            ;
            const auto Future = Self->Client->DescribeMaxStaminaTableMasters(Request);
            ...
            Range = R->GetItems().IsValid() ? R->GetItems() : MakeShared<TArray<Gs2::Stamina::Model::FMaxStaminaTableMasterPtr>>();
