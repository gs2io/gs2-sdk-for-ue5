...
namespace Gs2::Stamina::Domain::Iterator
{
    ...
    FDescribeRecoverValueTableMastersIterator::FIterator& FDescribeRecoverValueTableMastersIterator::FIterator::operator++()
    {
        ...
        {
            const auto ListParentKey = Gs2::Stamina::Model::Cache::FRecoverValueTableMasterCache::CreateCacheParentKey(
                Self->NamespaceName,
                TOptional<int32>()
            );
            if (!RangeIteratorOpt)
            {
                Range = Self->Gs2->Cache->TryGetList<Gs2::Stamina::Model::FRecoverValueTableMaster>(ListParentKey);

                if (Range)
                {
                    Range->RemoveAll([this](const Gs2::Stamina::Model::FRecoverValueTableMasterPtr& Item) { return Self->NamePrefix && !Item->GetName()->StartsWith(*Self->NamePrefix); });
                    bLast = true;
            ...
            const auto Request =
                MakeShared<Gs2::Stamina::Request::FDescribeRecoverValueTableMastersRequest>()
                    ...
                    ->WithLimit(FetchSize)
            ;
            const auto Future = Self->Client->DescribeRecoverValueTableMasters(Request);
            ...
            Range = R->GetItems().IsValid() ? R->GetItems() : MakeShared<TArray<Gs2::Stamina::Model::FRecoverValueTableMasterPtr>>();
            ...
            if (Range)
            {
                Range->RemoveAll([this](const Gs2::Stamina::Model::FRecoverValueTableMasterPtr& Item) { return Self->NamePrefix.IsSet() && !Item->GetName()->StartsWith(*Self->NamePrefix); });
            }
            RangeIteratorOpt = Range->CreateIterator();
