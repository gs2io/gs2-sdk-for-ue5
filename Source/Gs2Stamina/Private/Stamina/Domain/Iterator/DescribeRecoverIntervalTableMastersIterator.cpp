...
namespace Gs2::Stamina::Domain::Iterator
{
    ...
    FDescribeRecoverIntervalTableMastersIterator::FIterator& FDescribeRecoverIntervalTableMastersIterator::FIterator::operator++()
    {
        ...
        {
            const auto ListParentKey = Gs2::Stamina::Model::Cache::FRecoverIntervalTableMasterCache::CreateCacheParentKey(
                Self->NamespaceName,
                TOptional<int32>()
            );
            if (!RangeIteratorOpt)
            {
                Range = Self->Gs2->Cache->TryGetList<Gs2::Stamina::Model::FRecoverIntervalTableMaster>(ListParentKey);

                if (Range)
                {
                    Range->RemoveAll([this](const Gs2::Stamina::Model::FRecoverIntervalTableMasterPtr& Item) { return Self->NamePrefix && !Item->GetName()->StartsWith(*Self->NamePrefix); });
                    bLast = true;
            ...
            const auto Request =
                MakeShared<Gs2::Stamina::Request::FDescribeRecoverIntervalTableMastersRequest>()
                    ...
                    ->WithLimit(FetchSize)
            ;
            const auto Future = Self->Client->DescribeRecoverIntervalTableMasters(Request);
            ...
            Range = R->GetItems().IsValid() ? R->GetItems() : MakeShared<TArray<Gs2::Stamina::Model::FRecoverIntervalTableMasterPtr>>();
            ...
            if (Range)
            {
                Range->RemoveAll([this](const Gs2::Stamina::Model::FRecoverIntervalTableMasterPtr& Item) { return Self->NamePrefix.IsSet() && !Item->GetName()->StartsWith(*Self->NamePrefix); });
            }
            RangeIteratorOpt = Range->CreateIterator();
