...
namespace Gs2::Stamina::Domain::SpeculativeExecutor
{
    ...
    )
    {
        *Result = nullptr;
        ...
        const auto Prepared = MakeShared<Gs2::Stamina::Request::FRaiseMaxValueByUserIdRequest>(*Request);
        ...
        if (!Prepared->GetUserId().IsSet() || Prepared->GetUserId().Get(FString()) != Token->GetUserId().Get(FString()) ||
            !Prepared->GetRaiseValue().IsSet()) return nullptr;
        ...
        const int64 RaiseValue = Prepared->GetRaiseValue().Get(0);
        *Result = FStaminaMutationSpeculativeCommit::Create(
            ...
            [RaiseValue](const Gs2::Stamina::Model::FStaminaPtr& Current, const TOptional<int32>& MaxCapacity)
            {
                if (!Current->GetMaxValue().IsSet()) return false;
                Current->WithMaxValue(FStaminaMutationSpeculativeCommit::Clamp(static_cast<int64>(Current->GetMaxValue().Get(0)) + RaiseValue, MaxCapacity));
                return true;
            }
        );
        return nullptr;
