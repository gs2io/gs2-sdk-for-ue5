        ...
        if (!RangeIteratorOpt || (!*RangeIteratorOpt && !bLast))
        {
            ...
            const auto Request =
                MakeShared<Gs2::Inventory::Request::FDescribeItemSetsRequest>()
                    ...
                    ->WithLimit(FetchSize)
            ;
            const auto Future = Self->Client->DescribeItemSets(Request);
            ...
            if (Range.IsValid())
            {
                for (const auto& Item : *Range)
                {
                    ...
                    Gs2::Inventory::Model::Cache::FItemSetCache::Put(
                        Self->Gs2->Cache,
                        Request->GetNamespaceName(), CacheOwnerSnapshotUserId, Request->GetInventoryName(), Item->GetItemName(), Item->GetName(),
