...
namespace Gs2::Inventory::Domain::Iterator
{
    ...
    FDescribeSimpleInventoryModelsIterator::FIterator& FDescribeSimpleInventoryModelsIterator::FIterator::operator++()
    {
        ...
        {
            const auto ListParentKey = Gs2::Inventory::Model::Cache::FSimpleInventoryModelCache::CreateCacheParentKey(
                Self->NamespaceName,
                TOptional<int32>()
            );
            if (!RangeIteratorOpt)
            ...
            ;
            const auto Future = Self->Client->DescribeSimpleInventoryModels(Request);
            ...
            Range = R->GetItems().IsValid() ? R->GetItems() : MakeShared<TArray<Gs2::Inventory::Model::FSimpleInventoryModelPtr>>();
