...
namespace Gs2::Inventory::Domain::Iterator
{
    ...
    FDescribeItemSetsByUserIdIterator::FDescribeItemSetsByUserIdIterator(
        const FDescribeItemSetsByUserIdIterator& From
    ):
        Gs2(From.Gs2),
        Client(From.Client),
    ...
    FDescribeItemSetsByUserIdIterator::FIterator& FDescribeItemSetsByUserIdIterator::FIterator::operator++()
    {
        ...
        {
            const auto ListParentKey = Gs2::Inventory::Model::Cache::FItemSetCache::CreateCacheParentKey(
                Self->NamespaceName,
                Self->UserId,
                Self->InventoryName,
                TOptional<int32>()
            ...
            if (!RangeIteratorOpt)
            {
                Range = Self->Gs2->Cache->TryGetList<Gs2::Inventory::Model::FItemSet>(ListParentKey);
            ...
            const auto Request =
                MakeShared<Gs2::Inventory::Request::FDescribeItemSetsByUserIdRequest>()
                    ...
                    ->WithLimit(FetchSize)
            ;
            const auto Future = Self->Client->DescribeItemSetsByUserId(Request);
            ...
            Range = R->GetItems().IsValid() ? R->GetItems() : MakeShared<TArray<Gs2::Inventory::Model::FItemSetPtr>>();
            ...
            if (Range.IsValid())
            {
                for (const auto& Item : *Range)
                {
                    ...
                    Gs2::Inventory::Model::Cache::FItemSetCache::Put(
                        Self->Gs2->Cache,
                        Request->GetNamespaceName(), Request->GetUserId(), Request->GetInventoryName(), Item->GetItemName(), Item->GetName(),
