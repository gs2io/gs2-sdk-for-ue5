...
namespace Gs2::Inventory::Domain::Iterator
{
    ...
    FDescribeItemModelsIterator::FIterator& FDescribeItemModelsIterator::FIterator::operator++()
    {
        ...
        {
            const auto ListParentKey = Gs2::Inventory::Model::Cache::FItemModelCache::CreateCacheParentKey(
                Self->NamespaceName,
                Self->InventoryName,
                TOptional<int32>()
            );
            if (!RangeIteratorOpt)
            {
                Range = Self->Gs2->Cache->TryGetList<Gs2::Inventory::Model::FItemModel>(ListParentKey);
            ...
            ;
            const auto Future = Self->Client->DescribeItemModels(Request);
            ...
            Range = R->GetItems().IsValid() ? R->GetItems() : MakeShared<TArray<Gs2::Inventory::Model::FItemModelPtr>>();
