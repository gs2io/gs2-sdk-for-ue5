...
namespace Gs2::Inventory::Domain::Iterator
{
    ...
    FDescribeSimpleItemModelMastersIterator::FIterator& FDescribeSimpleItemModelMastersIterator::FIterator::operator++()
    {
        ...
        {
            const auto ListParentKey = Gs2::Inventory::Model::Cache::FSimpleItemModelMasterCache::CreateCacheParentKey(
                Self->NamespaceName,
                Self->InventoryName,
                TOptional<int32>()
            );
            ...
            const auto Request =
                MakeShared<Gs2::Inventory::Request::FDescribeSimpleItemModelMastersRequest>()
                    ...
                    ->WithLimit(FetchSize)
            ;
            const auto Future = Self->Client->DescribeSimpleItemModelMasters(Request);
            ...
            Range = R->GetItems().IsValid() ? R->GetItems() : MakeShared<TArray<Gs2::Inventory::Model::FSimpleItemModelMasterPtr>>();
            ...
            if (Range)
            {
                Range->RemoveAll([this](const Gs2::Inventory::Model::FSimpleItemModelMasterPtr& Item) { return Self->NamePrefix.IsSet() && !Item->GetName()->StartsWith(*Self->NamePrefix); });
            }
            RangeIteratorOpt = Range->CreateIterator();
