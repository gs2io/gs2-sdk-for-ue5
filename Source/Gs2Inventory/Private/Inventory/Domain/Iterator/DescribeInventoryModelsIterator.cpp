...
namespace Gs2::Inventory::Domain::Iterator
{
    ...
    FDescribeInventoryModelsIterator::FIterator& FDescribeInventoryModelsIterator::FIterator::operator++()
    {
        ...
        {
            const auto ListParentKey = Gs2::Inventory::Model::Cache::FInventoryModelCache::CreateCacheParentKey(
                Self->NamespaceName,
                TOptional<int32>()
            );
            if (!RangeIteratorOpt)
            {
                Range = Self->Gs2->Cache->TryGetList<Gs2::Inventory::Model::FInventoryModel>(ListParentKey);
            ...
            ;
            const auto Future = Self->Client->DescribeInventoryModels(Request);
            ...
            Range = R->GetItems().IsValid() ? R->GetItems() : MakeShared<TArray<Gs2::Inventory::Model::FInventoryModelPtr>>();
