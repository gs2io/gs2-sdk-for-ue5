...
namespace Gs2::Inventory::Domain::Iterator
{
    ...
    FDescribeBigInventoryModelsIterator::FIterator& FDescribeBigInventoryModelsIterator::FIterator::operator++()
    {
        ...
        {
            const auto ListParentKey = Gs2::Inventory::Model::Cache::FBigInventoryModelCache::CreateCacheParentKey(
                Self->NamespaceName,
                TOptional<int32>()
            );
            if (!RangeIteratorOpt)
            ...
            ;
            const auto Future = Self->Client->DescribeBigInventoryModels(Request);
            ...
            Range = R->GetItems().IsValid() ? R->GetItems() : MakeShared<TArray<Gs2::Inventory::Model::FBigInventoryModelPtr>>();
