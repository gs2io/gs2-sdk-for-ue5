...
Gs2::Core::Model::FGs2ErrorPtr FAcquireSimpleItemsByUserIdSpeculativeExecutor::Transform(
    ...
    const Gs2::Inventory::Request::FAcquireSimpleItemsByUserIdRequestPtr&,
...
FAcquireSimpleItemsByUserIdSpeculativeExecutor::FCommitTask::FCommitTask(const FCommitTask& From):
...
)
{
    *Result = nullptr;
    ...
    Gs2::Inventory::Request::FAcquireSimpleItemsByUserIdRequestPtr Prepared = nullptr;
    if (Request.IsValid()) Prepared = MakeShared<Gs2::Inventory::Request::FAcquireSimpleItemsByUserIdRequest>(*Request);
    if (Prepared.IsValid() && Prepared->GetAcquireCounts().IsValid())
    {
        auto Snapshot = MakeShared<TArray<TSharedPtr<Gs2::Inventory::Model::FAcquireCount>>>();
        for (const auto& Entry : *Prepared->GetAcquireCounts())
        {
            TSharedPtr<Gs2::Inventory::Model::FAcquireCount> Copy = nullptr;
            if (Entry.IsValid()) Copy = MakeShared<Gs2::Inventory::Model::FAcquireCount>(*Entry);
            Snapshot->Add(Copy);
        }
        Prepared->WithAcquireCounts(Snapshot);
    }
    ...
    if (!Prepared->GetUserId().IsSet() || Prepared->GetUserId().Get(FString()) != Token->GetUserId().Get(FString()) ||
        ...
        !Prepared->GetAcquireCounts().IsValid()) return nullptr;

    const auto NamespaceName = Prepared->GetNamespaceName();
    if (Prepared->GetAcquireCounts()->Num() == 0)
    {
    ...
    for (const auto& Entry : *Prepared->GetAcquireCounts())
    {
        ...
        Commits.Add(MakeShared<FSimpleItemMutationSpeculativeCommit>(
            ...
            [Delta](const Gs2::Inventory::Model::FSimpleItemPtr& Current)
            {
                ...
                return MakeShared<Gs2::Inventory::Model::FSimpleItem>(*Current)->WithCount(Base + Delta)->WithRevision(0);
            }
        ));
    }
    if (Prepared->GetAcquireCounts()->Num() > 0 && Commits.Num() == 0) return nullptr;
...
Gs2::Inventory::Request::FAcquireSimpleItemsByUserIdRequestPtr FAcquireSimpleItemsByUserIdSpeculativeExecutor::Rate(
    const Gs2::Inventory::Request::FAcquireSimpleItemsByUserIdRequestPtr& Request, const double Rate)
{
    if (!Request.IsValid() || !Request->GetAcquireCounts().IsValid()) return nullptr;
    for (const auto& Entry : *Request->GetAcquireCounts())
    {
...
Gs2::Inventory::Request::FAcquireSimpleItemsByUserIdRequestPtr FAcquireSimpleItemsByUserIdSpeculativeExecutor::Rate(
    const Gs2::Inventory::Request::FAcquireSimpleItemsByUserIdRequestPtr& Request, TBigInt<1024, false> Rate)
{
    if (!Request.IsValid() || !Request->GetAcquireCounts().IsValid()) return nullptr;
    for (const auto& Entry : *Request->GetAcquireCounts())
    {
