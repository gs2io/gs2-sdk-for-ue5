...
namespace Gs2::Inventory::Domain::SpeculativeExecutor
{
    ...
    )
    {
        ...
        if (FConsumeSimpleItemsByUserIdSpeculativeExecutor::Action() == NewConsumeAction->GetAction()) {
            TSharedPtr<FJsonObject> RequestModelJson;
            if (const TSharedRef<TJsonReader<>> JsonReader = TJsonReaderFactory<>::Create(NewConsumeAction->GetRequest().IsSet() ? *NewConsumeAction->GetRequest() : "{}");
                !FJsonSerializer::Deserialize(JsonReader, RequestModelJson))
            {
            ...
            auto Request = Request::FConsumeSimpleItemsByUserIdRequest::FromJson(RequestModelJson);
            Request = FConsumeSimpleItemsByUserIdSpeculativeExecutor::Rate(Request, Rate);
            auto Future = FConsumeSimpleItemsByUserIdSpeculativeExecutor::Execute(
                Domain,
                Service,
                AccessToken,
                Request
