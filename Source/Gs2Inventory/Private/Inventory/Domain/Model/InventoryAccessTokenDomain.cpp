...
namespace Gs2::Inventory::Domain::Model
{
    ...
    )
    {
        ...
        auto Value = Self->Cache->Get<Gs2::Inventory::Model::FInventory>(
            Self->ParentKey,
        ...
        if (Value == nullptr) {
            ...
            if (Future->GetTask().IsError())
            {
                if (Future->GetTask().Error()->Type() == Gs2::Core::Model::FNotFoundError::TypeString)
                {
                    ...
                    {
                        Self->Cache->Delete<Gs2::Inventory::Model::FInventory>(
                            Self->ParentKey,
                            Gs2::Inventory::Domain::Model::FInventoryDomain::CreateCacheKey(
                                Self->InventoryName
                            )
            ...
            Value = Self->Cache->Get<Gs2::Inventory::Model::FInventory>(
                Self->ParentKey,
                Gs2::Inventory::Domain::Model::FInventoryDomain::CreateCacheKey(
                    Self->InventoryName
                )
