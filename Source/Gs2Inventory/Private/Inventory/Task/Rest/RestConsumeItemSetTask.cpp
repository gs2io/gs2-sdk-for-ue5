...
#include "Inventory/Task/Rest/ConsumeItemSetTask.h"
...
{
    FConsumeItemSetTask::FConsumeItemSetTask(
    ...
    FConsumeItemSetTask::FConsumeItemSetTask(
        const FConsumeItemSetTask& From
    ): TGs2Future(From), Session(From.Session), Request(From.Request)
    {
    }

    Core::Model::FGs2ErrorPtr FConsumeItemSetTask::Action(
        const TSharedPtr<Result::FConsumeItemSetResultPtr> Result
    )
    {
        ...
        FString ResponseBody;
        {
            ...
            auto Url = Core::FGs2Constant::EndpointHost
                ...
                .Append("/{namespaceName}/user/me/inventory/{inventoryName}/item/{itemName}/consume");
            ...
            {
                JsonRootObject->SetStringField(TEXT("consumeCount"), FString::Printf(TEXT("%lld"), this->Request->GetConsumeCount().GetValue()));
            }
        ...
        if (ResponseCode / 100 == 2)
        {
                ...
                FJsonSerializer::Deserialize(JsonReader, JsonRootObject))
            {
                ...
                *Result = Result::FConsumeItemSetResult::FromJson(JsonRootObject);
                return nullptr;
            }
            const auto Details = MakeShared<TArray<TSharedPtr<Core::Model::FGs2ErrorDetail>>>();
    ...
    void FConsumeItemSetTask::OnError(Core::Model::FGs2ErrorPtr Error)
    {
        if (Error->Count() > 0 && Error->Detail(0)->Code() == "itemSet.operation.conflict") {
            TGs2Future<Result::FConsumeItemSetResult>::OnError(MakeShared<Inventory::Error::FConflictError>(Error));
        }
        else if (Error->Count() > 0 && Error->Detail(0)->Code() == "itemSet.count.insufficient") {
            TGs2Future<Result::FConsumeItemSetResult>::OnError(MakeShared<Inventory::Error::FInsufficientError>(Error));
        }
        else {
            TGs2Future<Result::FConsumeItemSetResult>::OnError(Error);
        }
    }
}