...
#include "Inventory/Task/Rest/SetCapacityByUserIdTask.h"
...
{
    FSetCapacityByUserIdTask::FSetCapacityByUserIdTask(
    ...
    FSetCapacityByUserIdTask::FSetCapacityByUserIdTask(
        const FSetCapacityByUserIdTask& From
    ): TGs2Future(From), Session(From.Session), Request(From.Request)
    {
    }

    Core::Model::FGs2ErrorPtr FSetCapacityByUserIdTask::Action(
        const TSharedPtr<Result::FSetCapacityByUserIdResultPtr> Result
    )
    {
        ...
        FString ResponseBody;
        {
            ...
            if (this->Request->GetNewCapacityValue().IsSet())
            {
                JsonRootObject->SetNumberField(TEXT("newCapacityValue"), this->Request->GetNewCapacityValue().GetValue());
            }
        ...
        if (ResponseCode / 100 == 2)
        {
                ...
                FJsonSerializer::Deserialize(JsonReader, JsonRootObject))
            {
                ...
                *Result = Result::FSetCapacityByUserIdResult::FromJson(JsonRootObject);
                return nullptr;
            }
            const auto Details = MakeShared<TArray<TSharedPtr<Core::Model::FGs2ErrorDetail>>>();
