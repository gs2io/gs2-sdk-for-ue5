...
#include "Inventory/Task/Rest/VerifyReferenceOfByUserIdTask.h"
...
{
    FVerifyReferenceOfByUserIdTask::FVerifyReferenceOfByUserIdTask(
    ...
    FVerifyReferenceOfByUserIdTask::FVerifyReferenceOfByUserIdTask(
        const FVerifyReferenceOfByUserIdTask& From
    ): TGs2Future(From), Session(From.Session), Request(From.Request)
    {
    }

    Core::Model::FGs2ErrorPtr FVerifyReferenceOfByUserIdTask::Action(
        const TSharedPtr<Result::FVerifyReferenceOfByUserIdResultPtr> Result
    )
    {
        ...
        FString ResponseBody;
        {
            ...
            auto Url = Core::FGs2Constant::EndpointHost
                ...
                .Append("/{namespaceName}/user/{userId}/inventory/{inventoryName}/item/{itemName}/{itemSetName}/reference/{referenceOf}/verify/{verifyType}");
            ...
            Url = Url.Replace(
                TEXT("{itemSetName}"),
                !this->Request->GetItemSetName().IsSet() || this->Request->GetItemSetName().GetValue().Len() == 0 ?
                    TEXT("null") : ToCStr(*this->Request->GetItemSetName())
            );
            Url = Url.Replace(
                TEXT("{referenceOf}"),
                !this->Request->GetReferenceOf().IsSet() || this->Request->GetReferenceOf().GetValue().Len() == 0 ?
                    TEXT("null") : ToCStr(*this->Request->GetReferenceOf())
            );
            Url = Url.Replace(
                TEXT("{verifyType}"),
        ...
        if (ResponseCode / 100 == 2)
        {
                ...
                FJsonSerializer::Deserialize(JsonReader, JsonRootObject))
            {
                ...
                *Result = Result::FVerifyReferenceOfByUserIdResult::FromJson(JsonRootObject);
                return nullptr;
            }
            const auto Details = MakeShared<TArray<TSharedPtr<Core::Model::FGs2ErrorDetail>>>();
