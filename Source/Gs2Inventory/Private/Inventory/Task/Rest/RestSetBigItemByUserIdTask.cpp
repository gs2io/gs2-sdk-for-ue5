...
#include "Inventory/Task/Rest/SetBigItemByUserIdTask.h"
...
{
    FSetBigItemByUserIdTask::FSetBigItemByUserIdTask(
    ...
    FSetBigItemByUserIdTask::FSetBigItemByUserIdTask(
        const FSetBigItemByUserIdTask& From
    ): TGs2Future(From), Session(From.Session), Request(From.Request)
    {
    }

    Core::Model::FGs2ErrorPtr FSetBigItemByUserIdTask::Action(
        const TSharedPtr<Result::FSetBigItemByUserIdResultPtr> Result
    )
    {
        ...
        FString ResponseBody;
        {
            ...
            auto Url = Core::FGs2Constant::EndpointHost
                ...
                .Append("/{namespaceName}/user/{userId}/big/inventory/{inventoryName}/item/{itemName}");
            ...
            {
                JsonRootObject->SetStringField(TEXT("count"), this->Request->GetCount().GetValue());
            }
        ...
        if (ResponseCode / 100 == 2)
        {
                ...
                FJsonSerializer::Deserialize(JsonReader, JsonRootObject))
            {
                ...
                *Result = Result::FSetBigItemByUserIdResult::FromJson(JsonRootObject);
                return nullptr;
            }
            const auto Details = MakeShared<TArray<TSharedPtr<Core::Model::FGs2ErrorDetail>>>();
    ...
    void FSetBigItemByUserIdTask::OnError(Core::Model::FGs2ErrorPtr Error)
    {
        if (Error->Count() > 0 && Error->Detail(0)->Code() == "itemSet.operation.conflict") {
            TGs2Future<Result::FSetBigItemByUserIdResult>::OnError(MakeShared<Inventory::Error::FConflictError>(Error));
        }
        else {
            TGs2Future<Result::FSetBigItemByUserIdResult>::OnError(Error);
        }
    }
}