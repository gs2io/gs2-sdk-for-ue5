...
#include "Inventory/Task/Rest/SetSimpleItemsByUserIdTask.h"
...
{
    FSetSimpleItemsByUserIdTask::FSetSimpleItemsByUserIdTask(
    ...
    FSetSimpleItemsByUserIdTask::FSetSimpleItemsByUserIdTask(
        const FSetSimpleItemsByUserIdTask& From
    ): TGs2Future(From), Session(From.Session), Request(From.Request)
    {
    }

    Core::Model::FGs2ErrorPtr FSetSimpleItemsByUserIdTask::Action(
        const TSharedPtr<Result::FSetSimpleItemsByUserIdResultPtr> Result
    )
    {
        ...
        FString ResponseBody;
        {
            ...
            auto Url = Core::FGs2Constant::EndpointHost
                ...
                .Append("/{namespaceName}/user/{userId}/simple/inventory/{inventoryName}");
            ...
            if (this->Request->GetCounts() != nullptr && this->Request->GetCounts().IsValid())
            {
                TArray<TSharedPtr<FJsonValue>> v;
                for (auto JsonObjectValue : *this->Request->GetCounts())
                {
                    v.Add(MakeShared<FJsonValueObject>(JsonObjectValue->ToJson()));
                }
                JsonRootObject->SetArrayField(TEXT("counts"), v);
            }
        ...
        if (ResponseCode / 100 == 2)
        {
                ...
                FJsonSerializer::Deserialize(JsonReader, JsonRootObject))
            {
                ...
                *Result = Result::FSetSimpleItemsByUserIdResult::FromJson(JsonRootObject);
                return nullptr;
            }
            const auto Details = MakeShared<TArray<TSharedPtr<Core::Model::FGs2ErrorDetail>>>();
    ...
    void FSetSimpleItemsByUserIdTask::OnError(Core::Model::FGs2ErrorPtr Error)
    {
        if (Error->Count() > 0 && Error->Detail(0)->Code() == "itemSet.operation.conflict") {
            TGs2Future<Result::FSetSimpleItemsByUserIdResult>::OnError(MakeShared<Inventory::Error::FConflictError>(Error));
        }
        else {
            TGs2Future<Result::FSetSimpleItemsByUserIdResult>::OnError(Error);
        }
    }
}