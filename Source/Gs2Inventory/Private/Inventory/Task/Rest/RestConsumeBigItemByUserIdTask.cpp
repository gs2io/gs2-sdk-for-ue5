...
#include "Inventory/Task/Rest/ConsumeBigItemByUserIdTask.h"
...
{
    FConsumeBigItemByUserIdTask::FConsumeBigItemByUserIdTask(
    ...
    Core::Model::FGs2ErrorPtr FConsumeBigItemByUserIdTask::Action(
        const TSharedPtr<Result::FConsumeBigItemByUserIdResultPtr> Result
    )
    {
        ...
        FString ResponseBody;
        {
            ...
            auto Url = Core::FGs2Constant::EndpointHost
                ...
                .Append("/{namespaceName}/user/{userId}/big/inventory/{inventoryName}/item/{itemName}/consume");
            ...
            Url = Url.Replace(
                TEXT("{inventoryName}"),
                !this->Request->GetInventoryName().IsSet() || this->Request->GetInventoryName().GetValue().Len() == 0 ?
                    TEXT("null") : ToCStr(*this->Request->GetInventoryName())
            );
            Url = Url.Replace(
                TEXT("{userId}"),
            ...
            Url = Url.Replace(
                TEXT("{itemName}"),
                !this->Request->GetItemName().IsSet() || this->Request->GetItemName().GetValue().Len() == 0 ?
                    TEXT("null") : ToCStr(*this->Request->GetItemName())
            );
            ...
            if (this->Request->GetConsumeCount().IsSet() && !this->Request->GetConsumeCount().GetValue().IsEmpty())
            {
        ...
        if (ResponseCode / 100 == 2)
        {
                ...
                FJsonSerializer::Deserialize(JsonReader, JsonRootObject))
            {
                ...
                *Result = Result::FConsumeBigItemByUserIdResult::FromJson(JsonRootObject);
                return nullptr;
            }
            const auto Details = MakeShared<TArray<TSharedPtr<Core::Model::FGs2ErrorDetail>>>();
    ...
    void FConsumeBigItemByUserIdTask::OnError(Core::Model::FGs2ErrorPtr Error)
    {
        if (Error->Count() > 0 && Error->Detail(0)->Code() == "itemSet.operation.conflict") {
            TGs2Future<Result::FConsumeBigItemByUserIdResult>::OnError(MakeShared<Inventory::Error::FConflictError>(Error));
        }
        else if (Error->Count() > 0 && Error->Detail(0)->Code() == "itemSet.count.insufficient") {
            TGs2Future<Result::FConsumeBigItemByUserIdResult>::OnError(MakeShared<Inventory::Error::FInsufficientError>(Error));
        }
        else {
            TGs2Future<Result::FConsumeBigItemByUserIdResult>::OnError(Error);
        }
    }
}