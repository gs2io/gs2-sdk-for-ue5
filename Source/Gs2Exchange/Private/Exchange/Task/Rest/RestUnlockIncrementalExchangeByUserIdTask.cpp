...
#include "Exchange/Task/Rest/UnlockIncrementalExchangeByUserIdTask.h"
...
{
    FUnlockIncrementalExchangeByUserIdTask::FUnlockIncrementalExchangeByUserIdTask(
    ...
    FUnlockIncrementalExchangeByUserIdTask::FUnlockIncrementalExchangeByUserIdTask(
        const FUnlockIncrementalExchangeByUserIdTask& From
    ): TGs2Future(From), Session(From.Session), Request(From.Request)
    {
    }

    Core::Model::FGs2ErrorPtr FUnlockIncrementalExchangeByUserIdTask::Action(
        const TSharedPtr<Result::FUnlockIncrementalExchangeByUserIdResultPtr> Result
    )
    {
        ...
        FString ResponseBody;
        {
            ...
            auto Url = Core::FGs2Constant::EndpointHost
                ...
                .Append("/{namespaceName}/user/{userId}/incremental/exchange/{rateName}/unlock");
            ...
            if (this->Request->GetLockTransactionId().IsSet() && !this->Request->GetLockTransactionId().GetValue().IsEmpty())
            {
                JsonRootObject->SetStringField("lockTransactionId", this->Request->GetLockTransactionId().GetValue());
            }
        ...
        if (ResponseCode / 100 == 2)
        {
                ...
                FJsonSerializer::Deserialize(JsonReader, JsonRootObject))
            {
                ...
                *Result = Result::FUnlockIncrementalExchangeByUserIdResult::FromJson(JsonRootObject);
                return nullptr;
            }
            const auto Details = MakeShared<TArray<TSharedPtr<Core::Model::FGs2ErrorDetail>>>();
