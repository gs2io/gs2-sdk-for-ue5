...
namespace Gs2::Exchange::Request
{
    ...
    TSharedPtr<FSkipByUserIdRequest> FSkipByUserIdRequest::FromJson(const TSharedPtr<FJsonObject> Data)
    {
        ...
        return MakeShared<FSkipByUserIdRequest>()
            ...
            ->WithSkipType(Data->HasField(ANSI_TO_TCHAR("skipType")) ? [Data]() -> TOptional<FString>
              {
                  FString v("");
                    if (Data->TryGetStringField(ANSI_TO_TCHAR("skipType"), v))
                  {
                        return TOptional(v);
                  }
                  return TOptional<FString>();
    ...
    TSharedPtr<FJsonObject> FSkipByUserIdRequest::ToJson() const
    {
        ...
        {
            JsonRootObject->SetStringField(TEXT("skipType"), SkipTypeValue.GetValue());
        }
        if (MinutesValue.IsSet())
        {
