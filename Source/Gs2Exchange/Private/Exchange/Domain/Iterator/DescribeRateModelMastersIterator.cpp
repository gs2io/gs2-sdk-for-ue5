...
namespace Gs2::Exchange::Domain::Iterator
{
    ...
    FDescribeRateModelMastersIterator::FDescribeRateModelMastersIterator(
        const FDescribeRateModelMastersIterator& From
    ):
        Gs2(From.Gs2),
        Client(From.Client),
    ...
    Gs2::Core::Model::FGs2ErrorPtr FDescribeRateModelMastersIterator::FIteratorNextTask::Action(TSharedPtr<TSharedPtr<Gs2::Exchange::Model::FRateModelMaster>> Result)
    {
        ++Iterator;
    ...
    FDescribeRateModelMastersIterator::FIterator& FDescribeRateModelMastersIterator::FIterator::operator++()
    {
        ...
        {
            const auto ListParentKey = Gs2::Exchange::Model::Cache::FRateModelMasterCache::CreateCacheParentKey(
                Self->NamespaceName,
                TOptional<int32>()
            );
            if (!RangeIteratorOpt)
            {
                Range = Self->Gs2->Cache->TryGetList<Gs2::Exchange::Model::FRateModelMaster>(ListParentKey);

                if (Range)
                {
                    Range->RemoveAll([this](const Gs2::Exchange::Model::FRateModelMasterPtr& Item) { return Self->NamePrefix && !Item->GetName()->StartsWith(*Self->NamePrefix); });
                    bLast = true;
            ...
            Range = R->GetItems().IsValid() ? R->GetItems() : MakeShared<TArray<Gs2::Exchange::Model::FRateModelMasterPtr>>();
            ...
            if (Range)
            {
                Range->RemoveAll([this](const Gs2::Exchange::Model::FRateModelMasterPtr& Item) { return Self->NamePrefix.IsSet() && !Item->GetName()->StartsWith(*Self->NamePrefix); });
            }
            RangeIteratorOpt = Range->CreateIterator();
