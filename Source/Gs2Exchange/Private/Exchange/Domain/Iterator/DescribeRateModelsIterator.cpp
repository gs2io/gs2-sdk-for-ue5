...
namespace Gs2::Exchange::Domain::Iterator
{
    ...
    FDescribeRateModelsIterator::FDescribeRateModelsIterator(
        const FDescribeRateModelsIterator& From
    ):
        Gs2(From.Gs2),
        Client(From.Client),
    ...
    FDescribeRateModelsIterator::FIterator& FDescribeRateModelsIterator::FIterator::operator++()
    {
        ...
        {
            const auto ListParentKey = Gs2::Exchange::Model::Cache::FRateModelCache::CreateCacheParentKey(
                Self->NamespaceName,
                TOptional<int32>()
            );
            if (!RangeIteratorOpt)
            {
                Range = Self->Gs2->Cache->TryGetList<Gs2::Exchange::Model::FRateModel>(ListParentKey);
            ...
            ;
            const auto Future = Self->Client->DescribeRateModels(Request);
            ...
            Range = R->GetItems().IsValid() ? R->GetItems() : MakeShared<TArray<Gs2::Exchange::Model::FRateModelPtr>>();
