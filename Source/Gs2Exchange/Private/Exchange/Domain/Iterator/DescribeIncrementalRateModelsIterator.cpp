...
namespace Gs2::Exchange::Domain::Iterator
{
    ...
    FDescribeIncrementalRateModelsIterator::FIterator& FDescribeIncrementalRateModelsIterator::FIterator::operator++()
    {
        ...
        {
            const auto ListParentKey = Gs2::Exchange::Model::Cache::FIncrementalRateModelCache::CreateCacheParentKey(
                Self->NamespaceName,
                TOptional<int32>()
            );
            if (!RangeIteratorOpt)
            ...
            ;
            const auto Future = Self->Client->DescribeIncrementalRateModels(Request);
            ...
            Range = R->GetItems().IsValid() ? R->GetItems() : MakeShared<TArray<Gs2::Exchange::Model::FIncrementalRateModelPtr>>();
