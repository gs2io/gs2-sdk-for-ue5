...
namespace Gs2::Money2::Domain::Iterator
{
    ...
    FDescribeStoreSubscriptionContentModelsIterator::FIterator& FDescribeStoreSubscriptionContentModelsIterator::FIterator::operator++()
    {
        ...
        {
            const auto ListParentKey = Gs2::Money2::Model::Cache::FStoreSubscriptionContentModelCache::CreateCacheParentKey(
                Self->NamespaceName,
                TOptional<int32>()
            );
            if (!RangeIteratorOpt)
            ...
            ;
            const auto Future = Self->Client->DescribeStoreSubscriptionContentModels(Request);
            ...
            Range = R->GetItems().IsValid() ? R->GetItems() : MakeShared<TArray<Gs2::Money2::Model::FStoreSubscriptionContentModelPtr>>();
