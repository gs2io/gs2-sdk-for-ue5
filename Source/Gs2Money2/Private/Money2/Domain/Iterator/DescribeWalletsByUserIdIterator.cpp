...
namespace Gs2::Money2::Domain::Iterator
{
    ...
    Gs2::Core::Model::FGs2ErrorPtr FDescribeWalletsByUserIdIterator::FIteratorNextTask::Action(TSharedPtr<TSharedPtr<Gs2::Money2::Model::FWallet>> Result)
    {
        ++Iterator;
    ...
    FDescribeWalletsByUserIdIterator::FIterator& FDescribeWalletsByUserIdIterator::FIterator::operator++()
    {
        ...
        if (!RangeIteratorOpt || (!*RangeIteratorOpt && !bLast))
        {
            ...
            const auto Request =
                MakeShared<Gs2::Money2::Request::FDescribeWalletsByUserIdRequest>()
                    ...
                    ->WithLimit(FetchSize)
            ;
            const auto Future = Self->Client->DescribeWalletsByUserId(Request);
            ...
            if (Range.IsValid())
            {
                for (const auto& Item : *Range)
                {
                    ...
                    Gs2::Money2::Model::Cache::FWalletCache::Put(
                        Self->Gs2->Cache,
                        Request->GetNamespaceName(), Request->GetUserId(), Item->GetSlot(),
