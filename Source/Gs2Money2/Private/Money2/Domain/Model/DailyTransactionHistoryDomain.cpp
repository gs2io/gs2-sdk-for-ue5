        ...
        if (ResultModel != nullptr) {
            ...
            if (ResultModel->GetItem() != nullptr)
            {
                ...
                const auto Key = Gs2::Money2::Domain::Model::FDailyTransactionHistoryDomain::CreateCacheKey(
                    ResultModel->GetItem()->GetYear().IsSet() ? FString::FromInt(*ResultModel->GetItem()->GetYear()) : TOptional<FString>(),
                    ResultModel->GetItem()->GetMonth().IsSet() ? FString::FromInt(*ResultModel->GetItem()->GetMonth()) : TOptional<FString>(),
                    ResultModel->GetItem()->GetDay().IsSet() ? FString::FromInt(*ResultModel->GetItem()->GetDay()) : TOptional<FString>(),
