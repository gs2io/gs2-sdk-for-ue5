...
#include "Money2/Task/Rest/GetDailyTransactionHistoryTask.h"
...
{
    FGetDailyTransactionHistoryTask::FGetDailyTransactionHistoryTask(
    ...
    Core::Model::FGs2ErrorPtr FGetDailyTransactionHistoryTask::Action(
        const TSharedPtr<Result::FGetDailyTransactionHistoryResultPtr> Result
    )
    {
        ...
        FString ResponseBody;
        {
            ...
            auto Url = Core::FGs2Constant::EndpointHost
                ...
                .Append("/{namespaceName}/transaction/daily/{year}/{month}/{day}/currency/{currency}");
            ...
            Url = Url.Replace(
                TEXT("{month}"),
                ToCStr(*this->Request->GetMonthString())
            );
            Url = Url.Replace(
                TEXT("{day}"),
                ToCStr(*this->Request->GetDayString())
            );
            Url = Url.Replace(
                TEXT("{currency}"),
        ...
        if (ResponseCode / 100 == 2)
        {
                ...
                FJsonSerializer::Deserialize(JsonReader, JsonRootObject))
            {
                ...
                *Result = Result::FGetDailyTransactionHistoryResult::FromJson(JsonRootObject);
                return nullptr;
            }
            const auto Details = MakeShared<TArray<TSharedPtr<Core::Model::FGs2ErrorDetail>>>();
