...
#include "Money2/Task/Rest/DepositByUserIdTask.h"
...
namespace Gs2::Money2::Task::Rest
{
    ...
    )
    {
        ...
        FString ResponseBody;
        {
            ...
            auto Url = Core::FGs2Constant::EndpointHost
                ...
                .Append("/{namespaceName}/user/{userId}/wallet/{slot}/deposit");
            ...
            if (this->Request->GetDepositTransactions() != nullptr && this->Request->GetDepositTransactions().IsValid())
            {
                TArray<TSharedPtr<FJsonValue>> v;
                for (auto JsonObjectValue : *this->Request->GetDepositTransactions())
                {
        ...
        if (ResponseCode / 100 == 2)
        {
                ...
                FJsonSerializer::Deserialize(JsonReader, JsonRootObject))
            {
                ...
                *Result = Result::FDepositByUserIdResult::FromJson(JsonRootObject);
                return nullptr;
            }
            const auto Details = MakeShared<TArray<TSharedPtr<Core::Model::FGs2ErrorDetail>>>();
    ...
    void FDepositByUserIdTask::OnError(Core::Model::FGs2ErrorPtr Error)
    {
        if (Error->Count() > 0 && Error->Detail(0)->Code() == "wallet.operation.conflict") {
            TGs2Future<Result::FDepositByUserIdResult>::OnError(MakeShared<Money2::Error::FConflictError>(Error));
        }
        else {
