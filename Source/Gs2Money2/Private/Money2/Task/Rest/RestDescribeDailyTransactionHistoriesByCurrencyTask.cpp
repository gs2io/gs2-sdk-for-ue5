...
#include "Money2/Task/Rest/DescribeDailyTransactionHistoriesByCurrencyTask.h"
...
{
    FDescribeDailyTransactionHistoriesByCurrencyTask::FDescribeDailyTransactionHistoriesByCurrencyTask(
    ...
    FDescribeDailyTransactionHistoriesByCurrencyTask::FDescribeDailyTransactionHistoriesByCurrencyTask(
        const FDescribeDailyTransactionHistoriesByCurrencyTask& From
    ): TGs2Future(From), Session(From.Session), Request(From.Request)
    {
    }

    Core::Model::FGs2ErrorPtr FDescribeDailyTransactionHistoriesByCurrencyTask::Action(
        const TSharedPtr<Result::FDescribeDailyTransactionHistoriesByCurrencyResultPtr> Result
    )
    {
        ...
        FString ResponseBody;
        {
            ...
            auto Url = Core::FGs2Constant::EndpointHost
                ...
                .Append("/{namespaceName}/transaction/daily/currency/{currency}/date/{year}");
            ...
            Url = Url.Replace(
                TEXT("{currency}"),
                !this->Request->GetCurrency().IsSet() || this->Request->GetCurrency().GetValue().Len() == 0 ?
                    TEXT("null") : ToCStr(*this->Request->GetCurrency())
            );
            Url = Url.Replace(
                TEXT("{year}"),
                ToCStr(*this->Request->GetYearString())
            );
            ...
            if (this->Request->GetMonth().IsSet()) {
                queryStrings.Add("month=" + this->Request->GetMonthString());
            }
        ...
        if (ResponseCode / 100 == 2)
        {
                ...
                FJsonSerializer::Deserialize(JsonReader, JsonRootObject))
            {
                ...
                *Result = Result::FDescribeDailyTransactionHistoriesByCurrencyResult::FromJson(JsonRootObject);
                return nullptr;
            }
            const auto Details = MakeShared<TArray<TSharedPtr<Core::Model::FGs2ErrorDetail>>>();
