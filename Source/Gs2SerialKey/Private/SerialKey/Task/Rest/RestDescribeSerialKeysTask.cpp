...
#include "SerialKey/Task/Rest/DescribeSerialKeysTask.h"
...
{
    FDescribeSerialKeysTask::FDescribeSerialKeysTask(
    ...
    FDescribeSerialKeysTask::FDescribeSerialKeysTask(
        const FDescribeSerialKeysTask& From
    ): TGs2Future(From), Session(From.Session), Request(From.Request)
    {
    }

    Core::Model::FGs2ErrorPtr FDescribeSerialKeysTask::Action(
        const TSharedPtr<Result::FDescribeSerialKeysResultPtr> Result
    )
    {
        ...
        FString ResponseBody;
        {
            ...
            auto Url = Core::FGs2Constant::EndpointHost
                ...
                .Append("/{namespaceName}/campaign/{campaignModelName}/issue/{issueJobName}/serialKey");
            ...
            Url = Url.Replace(
                TEXT("{issueJobName}"),
                !this->Request->GetIssueJobName().IsSet() || this->Request->GetIssueJobName().GetValue().Len() == 0 ?
                    TEXT("null") : ToCStr(*this->Request->GetIssueJobName())
            );
        ...
        if (ResponseCode / 100 == 2)
        {
                ...
                FJsonSerializer::Deserialize(JsonReader, JsonRootObject))
            {
                ...
                *Result = Result::FDescribeSerialKeysResult::FromJson(JsonRootObject);
                return nullptr;
            }
            const auto Details = MakeShared<TArray<TSharedPtr<Core::Model::FGs2ErrorDetail>>>();
