...
#include "SerialKey/Task/Rest/UpdateCampaignModelMasterTask.h"
...
{
    FUpdateCampaignModelMasterTask::FUpdateCampaignModelMasterTask(
    ...
    Core::Model::FGs2ErrorPtr FUpdateCampaignModelMasterTask::Action(
        const TSharedPtr<Result::FUpdateCampaignModelMasterResultPtr> Result
    )
    {
        ...
        FString ResponseBody;
        {
            ...
            auto Url = Core::FGs2Constant::EndpointHost
                .Replace(TEXT("{service}"), TEXT("serial-key"))
                .Replace(TEXT("{region}"), *this->Session->RegionName())
                .Append("/{namespaceName}/master/campaign/{campaignModelName}");
            ...
            Url = Url.Replace(
                TEXT("{campaignModelName}"),
                !this->Request->GetCampaignModelName().IsSet() || this->Request->GetCampaignModelName().GetValue().Len() == 0 ?
                    TEXT("null") : ToCStr(*this->Request->GetCampaignModelName())
            );
            ...
            if (this->Request->GetEnableCampaignCode().IsSet())
            {
                JsonRootObject->SetBoolField(TEXT("enableCampaignCode"), this->Request->GetEnableCampaignCode().GetValue());
            }
        ...
        if (ResponseCode / 100 == 2)
        {
                ...
                FJsonSerializer::Deserialize(JsonReader, JsonRootObject))
            {
                ...
                *Result = Result::FUpdateCampaignModelMasterResult::FromJson(JsonRootObject);
                return nullptr;
            }
            const auto Details = MakeShared<TArray<TSharedPtr<Core::Model::FGs2ErrorDetail>>>();
