...
namespace Gs2::SerialKey::Domain::SpeculativeExecutor
{
    ...
    )
    {
        ...
        if (FUseByUserIdSpeculativeExecutor::Action() == NewConsumeAction->GetAction()) {
            ...
            auto Request = Request::FUseByUserIdRequest::FromJson(RequestModelJson);
            Request = FUseByUserIdSpeculativeExecutor::Rate(Request, Rate);
            auto Future = FUseByUserIdSpeculativeExecutor::Execute(
                Domain,
                Service,
                AccessToken,
                Request
