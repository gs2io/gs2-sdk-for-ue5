...
)
{
    ...
    if (FollowUser.Value == nullptr) {
        UE_LOG(BpGs2Log, Error, TEXT("[UGs2FriendFollowAsyncFunction::Follow] FollowUser parameter specification is missing."))
        return Action;
    }
...
{
    if (FollowUser.Value == nullptr) {
        UE_LOG(BpGs2Log, Error, TEXT("[UGs2FriendFollowAsyncFunction] FollowUser parameter specification is missing."))
        return;
    }

    auto Future = FollowUser.Value->Follow(
    );
