...
void UGs2FormationListOfMoldModelAsyncFunction::Activate()
{
    ...
    if (Namespace.Value == nullptr) {
        UE_LOG(BpGs2Log, Error, TEXT("[UGs2FormationListOfMoldModelAsyncFunction::Activate] Namespace parameter specification is missing."))
        return;
    }
    ...
    for (auto v : *It)
    {
        ReturnMoldModels.Add(EzMoldModelToFGs2FormationMoldModelValue(v->Current()));
    }
