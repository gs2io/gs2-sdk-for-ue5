...
namespace Gs2::Mission::Result
{
    ...
    TSharedPtr<FGetMissionGroupModelResult> FGetMissionGroupModelResult::FromJson(const TSharedPtr<FJsonObject> Data)
    {
        ...
        return MakeShared<FGetMissionGroupModelResult>()
            ->WithItem(Data->HasField(ANSI_TO_TCHAR("item")) ? [Data]() -> Model::FMissionGroupModelPtr
                 {
                    if (Data->HasTypedField<EJson::Null>(ANSI_TO_TCHAR("item")))
                    {
                        return nullptr;
                    }
                    return Model::FMissionGroupModel::FromJson(Data->GetObjectField(ANSI_TO_TCHAR("item")));
                 }() : nullptr);
    }
