...
namespace Gs2::Mission::Domain::Iterator
{
    ...
    FDescribeMissionTaskModelMastersIterator::FIterator& FDescribeMissionTaskModelMastersIterator::FIterator::operator++()
    {
        ...
        {
            const auto ListParentKey = Gs2::Mission::Model::Cache::FMissionTaskModelMasterCache::CreateCacheParentKey(
                Self->NamespaceName,
            ...
            if (!RangeIteratorOpt)
            {
                ...
                if (Range)
                {
                    Range->RemoveAll([this](const Gs2::Mission::Model::FMissionTaskModelMasterPtr& Item) { return Self->NamePrefix && !Item->GetName()->StartsWith(*Self->NamePrefix); });
                    bLast = true;
            ...
            const auto Request =
                MakeShared<Gs2::Mission::Request::FDescribeMissionTaskModelMastersRequest>()
                    ...
                    ->WithLimit(FetchSize)
            ;
            const auto Future = Self->Client->DescribeMissionTaskModelMasters(Request);
            ...
            Range = R->GetItems().IsValid() ? R->GetItems() : MakeShared<TArray<Gs2::Mission::Model::FMissionTaskModelMasterPtr>>();
            ...
            if (Range.IsValid())
            {
                for (const auto& Item : *Range)
                {
                    ...
                    Gs2::Mission::Model::Cache::FMissionTaskModelMasterCache::Put(
                        Self->Gs2->Cache,
                        Request->GetNamespaceName(), Request->GetMissionGroupName(), Item->GetName(),
            ...
            if (Range)
            {
                Range->RemoveAll([this](const Gs2::Mission::Model::FMissionTaskModelMasterPtr& Item) { return Self->NamePrefix.IsSet() && !Item->GetName()->StartsWith(*Self->NamePrefix); });
            }
            RangeIteratorOpt = Range->CreateIterator();
