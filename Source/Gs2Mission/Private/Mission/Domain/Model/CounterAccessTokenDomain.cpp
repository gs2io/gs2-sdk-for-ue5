...
namespace Gs2::Mission::Domain::Model
{
    ...
    )
    {
        ...
        if (ResultModel->GetChangedCompletes() != nullptr)
        {
            const auto CompleteParentKey = Gs2::Mission::Domain::Model::FUserDomain::CreateCacheParentKey(
                Self->NamespaceName,
                Self->UserId(),
                "Complete"
            );
            for (const auto& ChangedComplete : *ResultModel->GetChangedCompletes())
            {
                if (ChangedComplete == nullptr) continue;
                Self->Gs2->Cache->Put(
                    Gs2::Mission::Model::FComplete::TypeName,
                    CompleteParentKey,
                    Gs2::Mission::Domain::Model::FCompleteDomain::CreateCacheKey(
                        ChangedComplete->GetMissionGroupName()
                    ),
                    ChangedComplete,
    ...
    {
        const FString CacheKey = Gs2::Mission::Domain::Model::FCounterDomain::CreateCacheKey(
            Self->CounterName
        );
            ...
            [this, Result, CacheKey]() -> Gs2::Core::Model::FGs2ErrorPtr
            {
                ...
                if (!bCacheHit) {
                    ...
                    if (Future->GetTask().IsError())
                    {
                        ...
                        if (!Error->GetErrors().IsValid() || Error->Count() == 0 || !Error->Detail(0).IsValid() || Error->Detail(0)->GetComponent() != "counter")
                        {
                            return Error;
                        }
                    }
    ...
    )
    {
        ...
        return Gs2->Cache->Subscribe(
            ...
            [WeakGs2, WeakService, RegisteredParentKey, QueryNamespaceName, QueryCounterName, SourceToken, RegisteredUserId]()
            {
