...
#include "Mission/Task/Rest/BatchReceiveByUserIdTask.h"
...
{
    FBatchReceiveByUserIdTask::FBatchReceiveByUserIdTask(
    ...
    FBatchReceiveByUserIdTask::FBatchReceiveByUserIdTask(
        const FBatchReceiveByUserIdTask& From
    ): TGs2Future(From), Session(From.Session), Request(From.Request)
    {
    }

    Core::Model::FGs2ErrorPtr FBatchReceiveByUserIdTask::Action(
        const TSharedPtr<Result::FBatchReceiveByUserIdResultPtr> Result
    )
    {
        ...
        FString ResponseBody;
        {
            ...
            auto Url = Core::FGs2Constant::EndpointHost
                ...
                .Append("/{namespaceName}/user/{userId}/complete/group/{missionGroupName}/task/any/receive/batch");
            ...
            if (this->Request->GetMissionTaskNames() != nullptr && this->Request->GetMissionTaskNames().IsValid())
            {
                TArray<TSharedPtr<FJsonValue>> v;
                for (auto JsonObjectValue : *this->Request->GetMissionTaskNames())
                {
                    v.Add(MakeShared<FJsonValueString>(JsonObjectValue));
                }
                JsonRootObject->SetArrayField(TEXT("missionTaskNames"), v);
            }
        ...
        if (ResponseCode / 100 == 2)
        {
                ...
                FJsonSerializer::Deserialize(JsonReader, JsonRootObject))
            {
                ...
                *Result = Result::FBatchReceiveByUserIdResult::FromJson(JsonRootObject);
                return nullptr;
            }
            const auto Details = MakeShared<TArray<TSharedPtr<Core::Model::FGs2ErrorDetail>>>();
