...
#include "Log/Task/Rest/UpdateFacetModelTask.h"
...
{
    FUpdateFacetModelTask::FUpdateFacetModelTask(
    ...
    FUpdateFacetModelTask::FUpdateFacetModelTask(
        const FUpdateFacetModelTask& From
    ): TGs2Future(From), Session(From.Session), Request(From.Request)
    {
    }

    Core::Model::FGs2ErrorPtr FUpdateFacetModelTask::Action(
        const TSharedPtr<Result::FUpdateFacetModelResultPtr> Result
    )
    {
        ...
        FString ResponseBody;
        {
            ...
            auto Url = Core::FGs2Constant::EndpointHost
                ...
                .Append("/{namespaceName}/model/facet/{field}");
            ...
            Url = Url.Replace(
                TEXT("{field}"),
                !this->Request->GetField().IsSet() || this->Request->GetField().GetValue().Len() == 0 ?
                    TEXT("null") : ToCStr(*this->Request->GetField())
            );
            ...
            if (this->Request->GetOrder().IsSet())
            {
                JsonRootObject->SetNumberField(TEXT("order"), this->Request->GetOrder().GetValue());
            }
        ...
        if (ResponseCode / 100 == 2)
        {
                ...
                FJsonSerializer::Deserialize(JsonReader, JsonRootObject))
            {
                ...
                *Result = Result::FUpdateFacetModelResult::FromJson(JsonRootObject);
                return nullptr;
            }
            const auto Details = MakeShared<TArray<TSharedPtr<Core::Model::FGs2ErrorDetail>>>();
