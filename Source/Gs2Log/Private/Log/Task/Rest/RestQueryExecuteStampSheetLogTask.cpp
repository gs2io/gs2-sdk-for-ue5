...
#include "Log/Task/Rest/QueryExecuteStampSheetLogTask.h"
...
{
    FQueryExecuteStampSheetLogTask::FQueryExecuteStampSheetLogTask(
    ...
    FQueryExecuteStampSheetLogTask::FQueryExecuteStampSheetLogTask(
        const FQueryExecuteStampSheetLogTask& From
    ): TGs2Future(From), Session(From.Session), Request(From.Request)
    {
    }

    Core::Model::FGs2ErrorPtr FQueryExecuteStampSheetLogTask::Action(
        const TSharedPtr<Result::FQueryExecuteStampSheetLogResultPtr> Result
    )
    {
        ...
        FString ResponseBody;
        {
            ...
            auto Url = Core::FGs2Constant::EndpointHost
                ...
                .Append("/{namespaceName}/log/execute/stamp/sheet");
            ...
            if (this->Request->GetService().IsSet()) {
                queryStrings.Add("service=" + FGenericPlatformHttp::UrlEncode(this->Request->GetService().GetValue()));
            }
            if (this->Request->GetMethod().IsSet()) {
                queryStrings.Add("method=" + FGenericPlatformHttp::UrlEncode(this->Request->GetMethod().GetValue()));
            }
            if (this->Request->GetUserId().IsSet()) {
                queryStrings.Add("userId=" + FGenericPlatformHttp::UrlEncode(this->Request->GetUserId().GetValue()));
            }
            if (this->Request->GetAction().IsSet()) {
                queryStrings.Add("action=" + FGenericPlatformHttp::UrlEncode(this->Request->GetAction().GetValue()));
            }
            if (this->Request->GetBegin().IsSet()) {
                queryStrings.Add("begin=" + this->Request->GetBeginString());
            }
            if (this->Request->GetEnd().IsSet()) {
                queryStrings.Add("end=" + this->Request->GetEndString());
            }
            if (this->Request->GetLongTerm().IsSet()) {
                queryStrings.Add("longTerm=" + this->Request->GetLongTermString());
            }
        ...
        if (ResponseCode / 100 == 2)
        {
                ...
                FJsonSerializer::Deserialize(JsonReader, JsonRootObject))
            {
                ...
                *Result = Result::FQueryExecuteStampSheetLogResult::FromJson(JsonRootObject);
                return nullptr;
            }
            const auto Details = MakeShared<TArray<TSharedPtr<Core::Model::FGs2ErrorDetail>>>();
