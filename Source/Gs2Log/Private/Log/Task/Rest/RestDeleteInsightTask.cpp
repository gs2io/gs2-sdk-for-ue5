...
#include "Log/Task/Rest/DeleteInsightTask.h"
...
{
    FDeleteInsightTask::FDeleteInsightTask(
    ...
    FDeleteInsightTask::FDeleteInsightTask(
        const FDeleteInsightTask& From
    ): TGs2Future(From), Session(From.Session), Request(From.Request)
    {
    }

    Core::Model::FGs2ErrorPtr FDeleteInsightTask::Action(
        const TSharedPtr<Result::FDeleteInsightResultPtr> Result
    )
    {
        ...
        FString ResponseBody;
        {
            ...
            auto Url = Core::FGs2Constant::EndpointHost
                ...
                .Append("/{namespaceName}/insight/{insightName}");
            ...
            Url = Url.Replace(
                TEXT("{insightName}"),
                !this->Request->GetInsightName().IsSet() || this->Request->GetInsightName().GetValue().Len() == 0 ?
                    TEXT("null") : ToCStr(*this->Request->GetInsightName())
            );
        ...
        if (ResponseCode / 100 == 2)
        {
                ...
                FJsonSerializer::Deserialize(JsonReader, JsonRootObject))
            {
                ...
                *Result = Result::FDeleteInsightResult::FromJson(JsonRootObject);
                return nullptr;
            }
            const auto Details = MakeShared<TArray<TSharedPtr<Core::Model::FGs2ErrorDetail>>>();
