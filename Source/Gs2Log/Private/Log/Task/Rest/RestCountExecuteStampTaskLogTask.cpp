...
#include "Log/Task/Rest/CountExecuteStampTaskLogTask.h"
...
{
    FCountExecuteStampTaskLogTask::FCountExecuteStampTaskLogTask(
    ...
    FCountExecuteStampTaskLogTask::FCountExecuteStampTaskLogTask(
        const FCountExecuteStampTaskLogTask& From
    ): TGs2Future(From), Session(From.Session), Request(From.Request)
    {
    }

    Core::Model::FGs2ErrorPtr FCountExecuteStampTaskLogTask::Action(
        const TSharedPtr<Result::FCountExecuteStampTaskLogResultPtr> Result
    )
    {
        ...
        FString ResponseBody;
        {
            ...
            auto Url = Core::FGs2Constant::EndpointHost
                ...
                .Append("/{namespaceName}/log/execute/stamp/task/count");
            ...
            if (this->Request->GetService().IsSet()) {
                queryStrings.Add("service=" + this->Request->GetServiceString());
            }
            if (this->Request->GetMethod().IsSet()) {
                queryStrings.Add("method=" + this->Request->GetMethodString());
            }
            if (this->Request->GetUserId().IsSet()) {
                queryStrings.Add("userId=" + this->Request->GetUserIdString());
            }
            if (this->Request->GetAction().IsSet()) {
                queryStrings.Add("action=" + this->Request->GetActionString());
            }
        ...
        if (ResponseCode / 100 == 2)
        {
                ...
                FJsonSerializer::Deserialize(JsonReader, JsonRootObject))
            {
                ...
                *Result = Result::FCountExecuteStampTaskLogResult::FromJson(JsonRootObject);
                return nullptr;
            }
            const auto Details = MakeShared<TArray<TSharedPtr<Core::Model::FGs2ErrorDetail>>>();
