...
namespace Gs2::Schedule::Domain::Iterator
{
    ...
    FDescribeTriggersByUserIdIterator::FIterator& FDescribeTriggersByUserIdIterator::FIterator::operator++()
    {
        ...
        {
            const auto ListParentKey = Gs2::Schedule::Model::Cache::FTriggerCache::CreateCacheParentKey(
                Self->NamespaceName,
                Self->UserId,
                TOptional<int32>()
            );
            ...
            const auto Request =
                MakeShared<Gs2::Schedule::Request::FDescribeTriggersByUserIdRequest>()
                    ...
                    ->WithLimit(FetchSize)
            ;
            const auto Future = Self->Client->DescribeTriggersByUserId(Request);
            ...
            Range = R->GetItems().IsValid() ? R->GetItems() : MakeShared<TArray<Gs2::Schedule::Model::FTriggerPtr>>();
