...
namespace Gs2::Schedule::Domain::Iterator
{
    ...
    FDescribeEventMastersIterator::FIterator& FDescribeEventMastersIterator::FIterator::operator++()
    {
        ...
        {
            const auto ListParentKey = Gs2::Schedule::Model::Cache::FEventMasterCache::CreateCacheParentKey(
                Self->NamespaceName,
                TOptional<int32>()
            );
            if (!RangeIteratorOpt)
            {
                Range = Self->Gs2->Cache->TryGetList<Gs2::Schedule::Model::FEventMaster>(ListParentKey);

                if (Range)
                {
                    Range->RemoveAll([this](const Gs2::Schedule::Model::FEventMasterPtr& Item) { return Self->NamePrefix && !Item->GetName()->StartsWith(*Self->NamePrefix); });
                    bLast = true;
            ...
            const auto Request =
                MakeShared<Gs2::Schedule::Request::FDescribeEventMastersRequest>()
                    ...
                    ->WithLimit(FetchSize)
            ;
            const auto Future = Self->Client->DescribeEventMasters(Request);
            ...
            Range = R->GetItems().IsValid() ? R->GetItems() : MakeShared<TArray<Gs2::Schedule::Model::FEventMasterPtr>>();
            ...
            if (Range)
            {
                Range->RemoveAll([this](const Gs2::Schedule::Model::FEventMasterPtr& Item) { return Self->NamePrefix.IsSet() && !Item->GetName()->StartsWith(*Self->NamePrefix); });
            }
            RangeIteratorOpt = Range->CreateIterator();
