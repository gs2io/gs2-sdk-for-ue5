...
#include "Schedule/Task/Rest/GetTriggerTask.h"
...
{
    FGetTriggerTask::FGetTriggerTask(
    ...
    FGetTriggerTask::FGetTriggerTask(
        const FGetTriggerTask& From
    ): TGs2Future(From), Session(From.Session), Request(From.Request)
    {
    }

    Core::Model::FGs2ErrorPtr FGetTriggerTask::Action(
        const TSharedPtr<Result::FGetTriggerResultPtr> Result
    )
    {
        ...
        if (ResponseCode / 100 == 2)
        {
                ...
                FJsonSerializer::Deserialize(JsonReader, JsonRootObject))
            {
                ...
                *Result = Result::FGetTriggerResult::FromJson(JsonRootObject);
                return nullptr;
            }
            const auto Details = MakeShared<TArray<TSharedPtr<Core::Model::FGs2ErrorDetail>>>();
