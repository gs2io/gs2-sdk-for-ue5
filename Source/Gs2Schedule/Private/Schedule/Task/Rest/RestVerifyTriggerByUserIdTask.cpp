...
#include "Schedule/Task/Rest/VerifyTriggerByUserIdTask.h"
...
{
    FVerifyTriggerByUserIdTask::FVerifyTriggerByUserIdTask(
    ...
    Core::Model::FGs2ErrorPtr FVerifyTriggerByUserIdTask::Action(
        const TSharedPtr<Result::FVerifyTriggerByUserIdResultPtr> Result
    )
    {
        ...
        FString ResponseBody;
        {
            ...
            auto Url = Core::FGs2Constant::EndpointHost
                ...
                .Append("/{namespaceName}/user/{userId}/trigger/{triggerName}/verify/{verifyType}");
            ...
            Url = Url.Replace(
                TEXT("{triggerName}"),
                !this->Request->GetTriggerName().IsSet() || this->Request->GetTriggerName().GetValue().Len() == 0 ?
                    TEXT("null") : ToCStr(*this->Request->GetTriggerName())
            );
            Url = Url.Replace(
                TEXT("{verifyType}"),
            ...
            if (this->Request->GetElapsedMinutes().IsSet())
            {
                JsonRootObject->SetNumberField(TEXT("elapsedMinutes"), this->Request->GetElapsedMinutes().GetValue());
            }
        ...
        if (ResponseCode / 100 == 2)
        {
                ...
                FJsonSerializer::Deserialize(JsonReader, JsonRootObject))
            {
                ...
                *Result = Result::FVerifyTriggerByUserIdResult::FromJson(JsonRootObject);
                return nullptr;
            }
            const auto Details = MakeShared<TArray<TSharedPtr<Core::Model::FGs2ErrorDetail>>>();
