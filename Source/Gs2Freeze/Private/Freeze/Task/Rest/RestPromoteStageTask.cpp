...
#include "Freeze/Task/Rest/PromoteStageTask.h"
...
{
    FPromoteStageTask::FPromoteStageTask(
    ...
    FPromoteStageTask::FPromoteStageTask(
        const FPromoteStageTask& From
    ): TGs2Future(From), Session(From.Session), Request(From.Request)
    {
    }

    Core::Model::FGs2ErrorPtr FPromoteStageTask::Action(
        const TSharedPtr<Result::FPromoteStageResultPtr> Result
    )
    {
        ...
        if (ResponseCode / 100 == 2)
        {
                ...
                FJsonSerializer::Deserialize(JsonReader, JsonRootObject))
            {
                ...
                *Result = Result::FPromoteStageResult::FromJson(JsonRootObject);
                return nullptr;
            }
            const auto Details = MakeShared<TArray<TSharedPtr<Core::Model::FGs2ErrorDetail>>>();
