...
#include "Freeze/Task/Rest/GetOutputTask.h"
...
namespace Gs2::Freeze::Task::Rest
{
    ...
    Core::Model::FGs2ErrorPtr FGetOutputTask::Action(
        const TSharedPtr<Result::FGetOutputResultPtr> Result
    )
    {
        ...
        FString ResponseBody;
        {
            ...
            auto Url = Core::FGs2Constant::EndpointHost
                .Replace(TEXT("{service}"), TEXT("freeze"))
                .Replace(TEXT("{region}"), *this->Session->RegionName())
                .Append("/{stageName}/progress/output/{outputName}");

            Url = Url.Replace(
                TEXT("{stageName}"),
                !this->Request->GetStageName().IsSet() || this->Request->GetStageName().GetValue().Len() == 0 ?
                    TEXT("null") : ToCStr(*this->Request->GetStageName())
            );
            Url = Url.Replace(
                TEXT("{outputName}"),
                !this->Request->GetOutputName().IsSet() || this->Request->GetOutputName().GetValue().Len() == 0 ?
                    TEXT("null") : ToCStr(*this->Request->GetOutputName())
            );
        ...
        if (ResponseCode / 100 == 2)
        {
                ...
                FJsonSerializer::Deserialize(JsonReader, JsonRootObject))
            {
                ...
                *Result = Result::FGetOutputResult::FromJson(JsonRootObject);
                return nullptr;
            }
            const auto Details = MakeShared<TArray<TSharedPtr<Core::Model::FGs2ErrorDetail>>>();
