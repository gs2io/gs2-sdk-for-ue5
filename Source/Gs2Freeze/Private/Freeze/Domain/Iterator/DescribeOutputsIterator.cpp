...
namespace Gs2::Freeze::Domain::Iterator
{
    ...
    FDescribeOutputsIterator::FIterator& FDescribeOutputsIterator::FIterator::operator++()
    {
        ...
        {
            const auto ListParentKey = Gs2::Freeze::Model::Cache::FOutputCache::CreateCacheParentKey(
                Self->StageName,
                TOptional<int32>()
            );
            if (!RangeIteratorOpt)
            ...
            Range = R->GetItems().IsValid() ? R->GetItems() : MakeShared<TArray<Gs2::Freeze::Model::FOutputPtr>>();
            ...
            if (Range.IsValid())
            {
                for (const auto& Item : *Range)
                {
                    ...
                    Gs2::Freeze::Model::Cache::FOutputCache::Put(
                        Self->Gs2->Cache,
                        Request->GetStageName(), Item->GetName(),
