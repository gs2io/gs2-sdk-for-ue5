...
#include "Script/Task/Rest/UpdateScriptTask.h"
...
{
    FUpdateScriptTask::FUpdateScriptTask(
    ...
    FUpdateScriptTask::FUpdateScriptTask(
        const FUpdateScriptTask& From
    ): TGs2Future(From), Session(From.Session), Request(From.Request)
    {
    }

    Core::Model::FGs2ErrorPtr FUpdateScriptTask::Action(
        const TSharedPtr<Result::FUpdateScriptResultPtr> Result
    )
    {
        ...
        FString ResponseBody;
        {
            ...
            auto Url = Core::FGs2Constant::EndpointHost
                ...
                .Append("/{namespaceName}/script/{scriptName}");
            ...
            Url = Url.Replace(
                TEXT("{scriptName}"),
                !this->Request->GetScriptName().IsSet() || this->Request->GetScriptName().GetValue().Len() == 0 ?
                    TEXT("null") : ToCStr(*this->Request->GetScriptName())
            );
            ...
            if (this->Request->GetScript().IsSet() && !this->Request->GetScript().GetValue().IsEmpty())
            {
        ...
        if (ResponseCode / 100 == 2)
        {
                ...
                FJsonSerializer::Deserialize(JsonReader, JsonRootObject))
            {
                ...
                *Result = Result::FUpdateScriptResult::FromJson(JsonRootObject);
                return nullptr;
            }
            const auto Details = MakeShared<TArray<TSharedPtr<Core::Model::FGs2ErrorDetail>>>();
