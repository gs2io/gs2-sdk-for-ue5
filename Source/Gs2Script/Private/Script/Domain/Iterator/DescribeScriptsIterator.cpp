...
namespace Gs2::Script::Domain::Iterator
{
    ...
    FDescribeScriptsIterator::FIterator& FDescribeScriptsIterator::FIterator::operator++()
    {
        ...
        {
            const auto ListParentKey = Gs2::Script::Model::Cache::FScriptCache::CreateCacheParentKey(
                Self->NamespaceName,
                TOptional<int32>()
            );
            if (!RangeIteratorOpt)
            {
                Range = Self->Gs2->Cache->TryGetList<Gs2::Script::Model::FScript>(ListParentKey);

                if (Range)
                {
                    Range->RemoveAll([this](const Gs2::Script::Model::FScriptPtr& Item) { return Self->NamePrefix && !Item->GetName()->StartsWith(*Self->NamePrefix); });
                    bLast = true;
            ...
            const auto Request =
                MakeShared<Gs2::Script::Request::FDescribeScriptsRequest>()
                    ...
                    ->WithLimit(FetchSize)
            ;
            const auto Future = Self->Client->DescribeScripts(Request);
            ...
            Range = R->GetItems().IsValid() ? R->GetItems() : MakeShared<TArray<Gs2::Script::Model::FScriptPtr>>();
            ...
            if (Range)
            {
                Range->RemoveAll([this](const Gs2::Script::Model::FScriptPtr& Item) { return Self->NamePrefix.IsSet() && !Item->GetName()->StartsWith(*Self->NamePrefix); });
            }
            RangeIteratorOpt = Range->CreateIterator();
