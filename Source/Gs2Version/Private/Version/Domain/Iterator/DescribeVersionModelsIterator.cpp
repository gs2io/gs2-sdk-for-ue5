...
namespace Gs2::Version::Domain::Iterator
{
    ...
    FDescribeVersionModelsIterator::FIterator& FDescribeVersionModelsIterator::FIterator::operator++()
    {
        ...
        {
            const auto ListParentKey = Gs2::Version::Model::Cache::FVersionModelCache::CreateCacheParentKey(
                Self->NamespaceName,
                TOptional<int32>()
            );
            if (!RangeIteratorOpt)
            {
                Range = Self->Gs2->Cache->TryGetList<Gs2::Version::Model::FVersionModel>(ListParentKey);
            ...
            ;
            const auto Future = Self->Client->DescribeVersionModels(Request);
            ...
            Range = R->GetItems().IsValid() ? R->GetItems() : MakeShared<TArray<Gs2::Version::Model::FVersionModelPtr>>();
