...
#include "Version/Task/Rest/RejectByUserIdTask.h"
...
{
    FRejectByUserIdTask::FRejectByUserIdTask(
    ...
    FRejectByUserIdTask::FRejectByUserIdTask(
        const FRejectByUserIdTask& From
    ): TGs2Future(From), Session(From.Session), Request(From.Request)
    {
    }

    Core::Model::FGs2ErrorPtr FRejectByUserIdTask::Action(
        const TSharedPtr<Result::FRejectByUserIdResultPtr> Result
    )
    {
        ...
        FString ResponseBody;
        {
            ...
            auto Url = Core::FGs2Constant::EndpointHost
                ...
                .Append("/{namespaceName}/user/{userId}/acceptVersion/reject");
            ...
            {
                JsonRootObject->SetStringField(TEXT("versionName"), this->Request->GetVersionName().GetValue());
            }
            if (this->Request->GetVersion() != nullptr && this->Request->GetVersion().IsValid())
            {
                JsonRootObject->SetObjectField(TEXT("version"), this->Request->GetVersion()->ToJson());
            }
        ...
        if (ResponseCode / 100 == 2)
        {
                ...
                FJsonSerializer::Deserialize(JsonReader, JsonRootObject))
            {
                ...
                *Result = Result::FRejectByUserIdResult::FromJson(JsonRootObject);
                return nullptr;
            }
            const auto Details = MakeShared<TArray<TSharedPtr<Core::Model::FGs2ErrorDetail>>>();
    ...
    void FRejectByUserIdTask::OnError(Core::Model::FGs2ErrorPtr Error)
    {
        if (Error->Count() > 0 && Error->Detail(0)->Code() == "version.accept.version.invalid") {
            TGs2Future<Result::FRejectByUserIdResult>::OnError(MakeShared<Version::Error::FAcceptVersionInvalidError>(Error));
        }
        else {
            TGs2Future<Result::FRejectByUserIdResult>::OnError(Error);
        }
    }
}