...
namespace Gs2::SkillTree::Domain::Iterator
{
    ...
    Gs2::Core::Model::FGs2ErrorPtr FDescribeStatusesIterator::FIteratorNextTask::Action(TSharedPtr<TSharedPtr<Gs2::SkillTree::Model::FStatus>> Result)
    {
        ++Iterator;
    ...
    FDescribeStatusesIterator::FIterator& FDescribeStatusesIterator::FIterator::operator++()
    {
        ...
        {
            const auto ListParentKey = Gs2::SkillTree::Model::Cache::FStatusCache::CreateCacheParentKey(
                Self->NamespaceName,
            ...
            if (!RangeIteratorOpt)
            {
                Range = Self->Gs2->Cache->TryGetList<Gs2::SkillTree::Model::FStatus>(ListParentKey);
            ...
            const auto Request =
                MakeShared<Gs2::SkillTree::Request::FDescribeStatusesRequest>()
                    ...
                    ->WithLimit(FetchSize)
            ;
            const auto Future = Self->Client->DescribeStatuses(Request);
            ...
            Range = R->GetItems().IsValid() ? R->GetItems() : MakeShared<TArray<Gs2::SkillTree::Model::FStatusPtr>>();
            ...
            if (Range.IsValid())
            {
                for (const auto& Item : *Range)
                {
                    ...
                    Gs2::SkillTree::Model::Cache::FStatusCache::Put(
                        Self->Gs2->Cache,
                        Request->GetNamespaceName(), CacheOwnerSnapshotUserId, Item->GetPropertyId(),
