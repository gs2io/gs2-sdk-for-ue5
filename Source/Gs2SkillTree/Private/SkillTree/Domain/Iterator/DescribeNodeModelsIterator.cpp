...
namespace Gs2::SkillTree::Domain::Iterator
{
    ...
    FDescribeNodeModelsIterator::FIterator& FDescribeNodeModelsIterator::FIterator::operator++()
    {
        ...
        {
            const auto ListParentKey = Gs2::SkillTree::Model::Cache::FNodeModelCache::CreateCacheParentKey(
                Self->NamespaceName,
                TOptional<int32>()
            );
            if (!RangeIteratorOpt)
            ...
            ;
            const auto Future = Self->Client->DescribeNodeModels(Request);
            ...
            Range = R->GetItems().IsValid() ? R->GetItems() : MakeShared<TArray<Gs2::SkillTree::Model::FNodeModelPtr>>();
