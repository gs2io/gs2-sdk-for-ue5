...
#include "SkillTree/Task/Rest/MarkReleaseByUserIdTask.h"
...
{
    FMarkReleaseByUserIdTask::FMarkReleaseByUserIdTask(
    ...
    FMarkReleaseByUserIdTask::FMarkReleaseByUserIdTask(
        const FMarkReleaseByUserIdTask& From
    ): TGs2Future(From), Session(From.Session), Request(From.Request)
    {
    }

    Core::Model::FGs2ErrorPtr FMarkReleaseByUserIdTask::Action(
        const TSharedPtr<Result::FMarkReleaseByUserIdResultPtr> Result
    )
    {
        ...
        FString ResponseBody;
        {
            ...
            auto Url = Core::FGs2Constant::EndpointHost
                ...
                .Append("/{namespaceName}/user/{userId}/status/{propertyId}/node/release/mark");
            ...
            if (this->Request->GetNodeModelNames() != nullptr && this->Request->GetNodeModelNames().IsValid())
            {
                TArray<TSharedPtr<FJsonValue>> v;
                for (auto JsonObjectValue : *this->Request->GetNodeModelNames())
                {
                    v.Add(MakeShared<FJsonValueString>(JsonObjectValue));
                }
                JsonRootObject->SetArrayField(TEXT("nodeModelNames"), v);
            }
        ...
        if (ResponseCode / 100 == 2)
        {
                ...
                FJsonSerializer::Deserialize(JsonReader, JsonRootObject))
            {
                ...
                *Result = Result::FMarkReleaseByUserIdResult::FromJson(JsonRootObject);
                return nullptr;
            }
            const auto Details = MakeShared<TArray<TSharedPtr<Core::Model::FGs2ErrorDetail>>>();
