...
namespace Gs2::Formation::Domain::Iterator
{
    ...
    FDescribeMoldsByUserIdIterator::FIterator& FDescribeMoldsByUserIdIterator::FIterator::operator++()
    {
        ...
        {
            const auto ListParentKey = Gs2::Formation::Model::Cache::FMoldCache::CreateCacheParentKey(
                Self->NamespaceName,
                Self->UserId,
                TOptional<int32>()
            );
            if (!RangeIteratorOpt)
            {
                Range = Self->Gs2->Cache->TryGetList<Gs2::Formation::Model::FMold>(ListParentKey);
            ...
            const auto Request =
                MakeShared<Gs2::Formation::Request::FDescribeMoldsByUserIdRequest>()
                    ...
                    ->WithLimit(FetchSize)
            ;
            const auto Future = Self->Client->DescribeMoldsByUserId(Request);
            ...
            Range = R->GetItems().IsValid() ? R->GetItems() : MakeShared<TArray<Gs2::Formation::Model::FMoldPtr>>();
