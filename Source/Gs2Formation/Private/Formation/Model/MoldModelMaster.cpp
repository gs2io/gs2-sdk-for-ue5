...
namespace Gs2::Formation::Model
{
    ...
    TSharedPtr<FMoldModelMaster> FMoldModelMaster::WithRevision(
    ...
    TOptional<int64> FMoldModelMaster::GetRevision() const
    {
        return RevisionValue;
    }

    FString FMoldModelMaster::GetRevisionString() const
    {
    ...
    TOptional<FString> FMoldModelMaster::GetMoldModelNameFromGrn(const FString Grn)
    {
        const auto Pattern = FRegexPattern(TEXT("grn:gs2:(?<region>.+):(?<ownerId>.+):formation:(?<namespaceName>.+):model:mold:(?<moldModelName>.+)"));
        FRegexMatcher Matcher(Pattern, Grn);
        while (Matcher.FindNext())
    ...
    TSharedPtr<FJsonObject> FMoldModelMaster::ToJson() const
    {
        ...
        {
            JsonRootObject->SetStringField(TEXT("formModelName"), FormModelNameValue.GetValue());
        }
        if (CreatedAtValue.IsSet())
        {
...
namespace Gs2::Formation::Model::Cache
{
    ...
    FString FMoldModelMasterCache::PutUserData(
        const Gs2::Core::Domain::FCacheDatabasePtr& Cache,
        TOptional<FString> NamespaceName,
        TOptional<FString> UserId,
        TOptional<int32> TimeOffset,
        const Gs2::Formation::Model::FMoldModelMasterPtr& Item
    )
    {
