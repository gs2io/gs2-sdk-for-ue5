...
namespace Gs2::Formation::Model
{
    ...
    TSharedPtr<FSlotWithSignature> FSlotWithSignature::FromJson(const TSharedPtr<FJsonObject> Data)
    {
        ...
        return MakeShared<FSlotWithSignature>()
            ...
            ->WithPropertyType(Data->HasField(ANSI_TO_TCHAR("propertyType")) ? [Data]() -> TOptional<FString>
                {
                    FString v("");
                    if (Data->TryGetStringField(ANSI_TO_TCHAR("propertyType"), v))
                    {
                        return TOptional(v);
                    }
                    return TOptional<FString>();
    ...
    TSharedPtr<FJsonObject> FSlotWithSignature::ToJson() const
    {
        ...
        {
            JsonRootObject->SetStringField(TEXT("propertyType"), PropertyTypeValue.GetValue());
        }
        if (BodyValue.IsSet())
        {
