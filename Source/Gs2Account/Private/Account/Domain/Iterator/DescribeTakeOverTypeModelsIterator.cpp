...
namespace Gs2::Account::Domain::Iterator
{
    ...
    FDescribeTakeOverTypeModelsIterator::FIterator& FDescribeTakeOverTypeModelsIterator::FIterator::operator++()
    {
        ...
        {
            const auto ListParentKey = Gs2::Account::Model::Cache::FTakeOverTypeModelCache::CreateCacheParentKey(
                Self->NamespaceName,
                TOptional<int32>()
            );
            if (!RangeIteratorOpt)
            ...
            ;
            const auto Future = Self->Client->DescribeTakeOverTypeModels(Request);
            ...
            Range = R->GetItems().IsValid() ? R->GetItems() : MakeShared<TArray<Gs2::Account::Model::FTakeOverTypeModelPtr>>();
