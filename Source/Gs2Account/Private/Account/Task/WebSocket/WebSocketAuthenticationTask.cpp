...
#include "Account/Task/WebSocket/AuthenticationTask.h"
...
{
    FAuthenticationTask::FAuthenticationTask(
    ...
    Core::Model::FGs2ErrorPtr FAuthenticationTask::Action(
        const TSharedPtr<Result::FAuthenticationResultPtr> Result
    )
    {
        ...
        UE_LOG(Gs2Log, VeryVerbose, TEXT("[%s:%s:%s] %s"), TEXT("account"), TEXT("account"), TEXT("authentication"), ToCStr(RequestPayload->Payload()));
